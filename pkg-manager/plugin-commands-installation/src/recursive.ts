...
export async function recursive(
  ...
  if (
    ...
    if (
      ...
      manifestsByPath[opts.workspaceDir] != null
    ) {
    ...
    if (opts.dedupePeerDependents) {
      for (const rootDir of Object.keys(opts.allProjectsGraph)) {
        if (
          opts.selectedProjectsGraph[rootDir] ||
          rootDir === opts.workspaceDir
        )
          continue
    ...
    const { updatedProjects: mutatedPkgs } = await mutateModules(
      mutatedImporters,
      {
        ...installOpts,
        storeController: store.ctrl,
    ...
    if (opts.save !== false) {
      await Promise.all(
        mutatedPkgs.map(async ({ originalManifest, manifest }, index) =>
          writeProjectManifests[index](originalManifest ?? manifest)
        )
      )
    }
    return true
...
function calculateRepositoryRoot(workspaceDir: string, projectDirs: string[]) {
