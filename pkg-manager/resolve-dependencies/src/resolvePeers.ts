...
export function resolvePeers<T extends PartialResolvedPackage>(opts: {
  ...
  const rootPkgsByName = opts.resolvePeersFromWorkspaceRoot
  ...
  Object.values(depGraph).forEach((node: T & DependenciesGraphNode): void => {
    node.children = mapValues(
      (childNodeId: string | undefined): string | undefined => {
        return pathsByNodeId.get(childNodeId ?? '') ?? childNodeId;
      },
      node.children ?? {}
    )
  })
  ...
  for (const { directNodeIdsByAlias, id } of opts.projects) {
    dependenciesByProjectId[id] = mapValues(
      (nodeId: string): string => {
        return pathsByNodeId.get(nodeId) ?? '';
      },
      directNodeIdsByAlias
    )
  }
  ...
  if (opts.dedupePeerDependents) {
    ...
    for (const { id } of opts.projects) {
      dependenciesByProjectId[id] = mapValues(
        (depPath) => allDepPathsMap[depPath] ?? depPath,
...
function nodeDepsCount(node: DependenciesGraphNode): number {
  return Object.keys(node.children ?? {}).length + (node.resolvedPeerNames?.size ?? 0)
}
...
function deduplicateAll<T extends PartialResolvedPackage>(
  ...
  Object.values(depGraph).forEach((node) => {
    node.children = mapValues(
      (childDepPath) => {
        return depPathsMap[childDepPath ?? ''] ?? childDepPath;
      },
      node.children ?? {}
    )
  })
...
function deduplicateDepPaths<T extends PartialResolvedPackage>(
  ...
  depGraph: GenericDependenciesGraph<T>
    ...
    remainingDuplicates: Set<string>[];
  } {
  function depCountSorter(depPath1: string, depPath2: string): number {
    return nodeDepsCount(depGraph[depPath1]) - nodeDepsCount(depGraph[depPath2])
  }
...
function isCompatibleAndHasMoreDeps<T extends PartialResolvedPackage>(
  ...
  for (const depPath of (node2.resolvedPeerNames ?? new Set<string>())) {
...
function getRootPkgsByName<T extends PartialResolvedPackage>(
  dependenciesTree: DependenciesTree<T>,
  projects: ProjectToResolve[]
): ParentRefs {
  const rootProject =
...
function createPkgsByName<T extends PartialResolvedPackage>(
  ...
  const parentRefs = toPkgByName(
    Object.keys(directNodeIdsByAlias).map((alias) => {
      return {
        alias,
...
function resolvePeersOfNode<T extends PartialResolvedPackage>(
  ...
  if (node?.depth === -1) {
    return {
      resolvedPeers: new Map<string, string>(),
      missingPeers: new Set<string>(),
    }
  }

  const resolvedPackage = node?.resolvedPackage as T | undefined

  if (
    ctx.purePkgs.has(resolvedPackage?.depPath ?? '') &&
    (ctx.depGraph?.[resolvedPackage?.depPath ?? ''].depth ?? 0) <= (node?.depth ?? 0) &&
  ...
  const parentPkgs =
    Object.keys(children ?? {}).length === 0
      ...
      : Object.assign(
        ...
        toPkgByName(
          Object.entries(children ?? {}).map(([alias, nodeId]: [string, string]): {
            alias: string;
            node: DependenciesTreeNode<T> | undefined;
            nodeId: string;
          } => {
            return {
              alias,
  ...
  const hit = ctx.peersCache.get(resolvedPackage?.depPath ?? '')?.find((cache: PeersCacheItem): boolean => {
    for (const [name, cachedNodeId] of cache.resolvedPeers) {
      ...
      const parentDepPath = (
        ctx.dependenciesTree.get(parentPkgNodeId)?.resolvedPackage as T
      )?.depPath
      ...
      const cachedDepPath = (
        ctx.dependenciesTree.get(cachedNodeId)?.resolvedPackage as T
      )?.depPath
  ...
  if (hit != null) {
    ...
    ctx.depGraph[hit.depPath].depth = Math.min(
      ctx.depGraph?.[hit.depPath].depth ?? 0,
      node?.depth ?? 0
    )
  ...
  if (unknownResolvedPeersOfChildren.size === 0) {
    ...
    const peersFolderSuffix = createPeersFolderSuffix(
      [...unknownResolvedPeersOfChildren.entries()].map(([alias, nodeId]): {
        name: string | undefined;
        version: string | undefined;
      } | null => {
        ...
        const resolvedPackage = ctx.dependenciesTree.get(nodeId)?.resolvedPackage;
  ...
  const isPure = unknownResolvedPeersOfChildren.size === 0 && allMissingPeers.size === 0
  ...
  if (resolvedPackage?.name && (!ctx.depGraph[depPath] || (ctx.depGraph?.[depPath].depth ?? 0) > (node?.depth ?? 0))) {
    ...
    ctx.depGraph[depPath] = {
      ...
      resolvedPeerNames: new Set(unknownResolvedPeersOfChildren.keys()),
    }
  }
...
function resolvePeersOfChildren<T extends PartialResolvedPackage>(
  ...
  const [repeated, notRepeated] = partition(
    ([alias]) => parentPkgs[alias] != null,
  ...
  for (const [, childNodeId] of [...notRepeated, ...repeated]) {
    const { resolvedPeers, missingPeers } = resolvePeersOfNode(
      childNodeId,
      parentPkgs,
      ctx
    )
...
function _resolvePeers<T extends PartialResolvedPackage>(ctx: {
  currentDepth: number
  lockfileDir: string
  nodeId: string
  parentPkgs: ParentRefs
  resolvedPackage: T | undefined
  dependenciesTree: DependenciesTree<T>
  rootDir: string
  peerDependencyIssues: Pick<PeerDependencyIssues, 'bad' | 'missing'>
}): PeersResolution {
...
function getLocationFromNodeIdAndPkg<T>({
  ...
  dependenciesTree: DependenciesTree<T>
  nodeId: string
  pkg?: { name?: string | undefined; version?: string | undefined } | undefined
}) {
  const { projectId, parents } = getLocationFromNodeId({
    dependenciesTree,
    nodeId,
  })
...
function getLocationFromNodeId<T>({
  ...
  nodeId: string
    ...
    projectId: string;
    parents: Pick<DependenciesGraphNode, 'name' | 'version'>[];
  } {
  const parts = splitNodeId(nodeId).slice(0, -1)
  const parents = scan(
    (prevNodeId, pkgId) => createNodeId(prevNodeId, pkgId),
    '>',
    parts
  )
    .slice(2)

    .map((nid: string): Pick<DependenciesGraphNode, 'name' | 'version'> => {
      return pick(
        ['name', 'version'],
        dependenciesTree.get(nid)?.resolvedPackage
      );
    }
    )
  return {
...
function toPkgByName<T extends PartialResolvedPackage>(
  nodes: Array<{ alias: string; nodeId: string; node: DependenciesTreeNode<T> | undefined }>
): ParentRefs {
