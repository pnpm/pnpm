...
import { PartialResolvedPackage, GenericDependenciesGraph, DependenciesByProjectId, ProjectToResolve, ResolvedImporters, LinkedDependency } from '@pnpm/types'

export type DedupeInjectedDepsOptions<T extends PartialResolvedPackage> = {
  lockfileDir: string
...
export type InjectedDepsByProjects = Map<
  string,
  Map<string, { depPath: string; id: string }>
>
...
function getInjectedDepsByProjects<T extends PartialResolvedPackage>(
  ...
  for (const project of opts.projects) {
    for (const [alias, nodeId] of Object.entries(
      project.directNodeIdsByAlias
    )) {
      ...
      if (typeof id !== 'undefined' && opts.projects.some((project) => project.id === id)) {
...
function getDedupeMap<T extends PartialResolvedPackage>(
  ...
  opts: Pick<
    DedupeInjectedDepsOptions<T>,
    'depGraph' | 'dependenciesByProjectId'
  >
): DedupeMap {
  ...
  for (const [id, deps] of injectedDepsByProjects.entries()) {
    ...
    for (const [alias, dep] of deps.entries()) {
      ...
      const isSubset = Object.entries(
        ...
        ([alias, depPath]: [string, string | undefined]): boolean => {
          return opts.dependenciesByProjectId[dep.id][alias] === depPath;
        }
      )
