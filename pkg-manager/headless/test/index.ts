...
test('run pre/postinstall scripts', async () => {
  ...
  await headlessInstall(
    await testDefaults({ lockfileDir: prefix, ignoreScripts: true })
  )
...
test('available packages are used when node_modules is not clean', async () => {
  ...
  await fs.copyFile(
    path.join(hasGlobDir, WANTED_LOCKFILE),
    destLockfileYamlPath
  )
  ...
  await fs.copyFile(
    path.join(hasGlobAndRimrafDir, 'package.json'),
    destPackageJsonPath
  )
  await fs.copyFile(
    path.join(hasGlobAndRimrafDir, WANTED_LOCKFILE),
    destLockfileYamlPath
  )

  const reporter = sinon.spy()
  await headlessInstall(
    await testDefaults({ lockfileDir: projectDir, reporter })
  )
...
test('available packages are relinked during forced install', async () => {
  ...
  const reporter = sinon.spy()
  await headlessInstall(
    await testDefaults({ lockfileDir: projectDir, reporter, force: true })
  )
...
test('installing using passed in lockfile files', async () => {
  ...
  await fs.copyFile(
    path.join(simplePkgPath, 'package.json'),
    path.join(prefix, 'package.json')
  )
  await fs.copyFile(
    path.join(simplePkgPath, WANTED_LOCKFILE),
    path.join(prefix, WANTED_LOCKFILE)
  )

  const wantedLockfile = await readWantedLockfile(simplePkgPath, {
...
test('install peer dependencies that are in prod dependencies', async () => {
  ...
  await project.has(
    '.pnpm/@pnpm.e2e+peer-a@1.0.1/node_modules/@pnpm.e2e/peer-a'
  )
})
...
test('installing with hoistPattern=*', async () => {
  ...
  await headlessInstall(
    await testDefaults({ lockfileDir: prefix, reporter, hoistPattern: '*' })
  )
...
test.each([['isolated'], ['hoisted']])(
  'using side effects cache with nodeLinker=%s',
  async (nodeLinker) => {
    ...
    const cacheIntegrityPath = getFilePathInCafs(
      cafsDir,
...
test.skip('using side effects cache and hoistPattern=*', async () => {
  ...
  const cacheBuildDir = path.join(
    opts.storeDir,
    `localhost+${REGISTRY_MOCK_PORT}/diskusage@1.1.3/side_effects/${ENGINE_DIR}/package/build`
  )
  ...
  expect(
    await exists(
      path.join(
        lockfileDir,
        'node_modules/.pnpm/node_modules/diskusage/build/new-file.txt'
      )
    )
  ).toBeTruthy()
...
test('installing with no symlinks but with PnP', async () => {
  ...
  expect([
    ...(await fs.readdir(path.join(prefix, 'node_modules'))),
  ]).toStrictEqual(['.bin', '.modules.yaml', '.pnpm'])
  expect([
    ...(await fs.readdir(
      path.join(prefix, 'node_modules/.pnpm/rimraf@2.7.1/node_modules')
    )),
...
test('installing with node-linker=hoisted', async () => {
  ...
  expect(realpathSync('node_modules/send')).toBe(
    path.resolve('node_modules/send')
  )
...
test('installing in a workspace with node-linker=hoisted', async () => {
  ...
  await headlessInstall(
    await testDefaults({
      lockfileDir: prefix,
      nodeLinker: 'hoisted',
      projects: [path.join(prefix, 'foo'), path.join(prefix, 'bar')],
    })
  )

  expect(realpathSync('bar/node_modules/foo')).toBe(path.resolve('foo'))
  expect(readPkgVersion(path.join(prefix, 'foo/node_modules/webpack'))).toBe(
    '2.7.0'
  )
  expect(realpathSync('foo/node_modules/express')).toBe(
    path.resolve('foo/node_modules/express')
  )
  expect(readPkgVersion(path.join(prefix, 'foo/node_modules/express'))).toBe(
    '4.17.2'
  )
  expect(readPkgVersion(path.join(prefix, 'node_modules/webpack'))).toBe(
    '5.65.0'
  )
  expect(readPkgVersion(path.join(prefix, 'node_modules/express'))).toBe(
    '2.5.11'
  )
})

function readPkgVersion(dir: string): string {
  return loadJsonFile.sync<{ version: string }>(path.join(dir, 'package.json'))
    .version
}
