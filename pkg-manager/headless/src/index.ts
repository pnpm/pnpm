...
export async function headlessInstall (opts: HeadlessOptions): Promise<InstallationResult> {
  ...
  await validateOnlyBuiltDependenciesForHeadless(graph, {
...
async function validateOnlyBuiltDependenciesForHeadless (
  ...
  const byName = new Map<string, DependenciesGraphNode[]>()
  ...
  const perEntry = await Promise.all(entries.map(async (entry) => {
    ...
    const nodeResults = await Promise.all(
      nodes.map(async (node) => {
        const manifest = await safeReadProjectManifestOnly(node.dir)
        if (manifest == null) return null
        const scripts = manifest.scripts
        const hasScripts = Boolean(scripts && hasLifecycleScript(scripts ?? {}))
        return { node, hasScripts }
      })
    )
  ...
  for (const res of perEntry) {
    if (res) unused.push(res)
  }

  if (!unused.length) return

  const body = unused.map(({ entry, matches }) => {
