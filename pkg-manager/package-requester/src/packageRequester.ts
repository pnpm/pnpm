...
export function createPackageRequester (
  ...
  const fetchPackageToStore = fetchToStore.bind(null, {
    ...
    verifyStorePkgNameAndVersion: opts.verifyStorePkgNameAndVersion,
  })
...
function fetchToStore (
  ...
  Actual package in the store by the given integrity: ${pkgFilesIndex.name}@${pkgFilesIndex.version}.`
            if (ctx.verifyStorePkgNameAndVersion ?? true) {
              throw new PnpmError('UNEXPECTED_PKG_CONTENT_IN_STORE', msg, {
                hint: 'If you want to ignore this issue, set the verify-store-pkg-name-and-version to false',
              })
            } else {
              globalWarn(msg)
            }
