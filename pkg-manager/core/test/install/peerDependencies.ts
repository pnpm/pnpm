...
test('resolve peer dependencies from aliased subdependencies if they are dependencies of a parent package', async () => {
  ...
  await addDependenciesToPackage(
    ...
    await testDefaults({ autoInstallPeers: false, strictPeerDependencies: false, useLockfileV6: true })
  )
...
test('resolve peer dependency from aliased direct dependency', async () => {
  ...
  const opts = await testDefaults({ autoInstallPeers: false, strictPeerDependencies: false, useLockfileV6: true })
...
test('in a subdependency, when there are several aliased dependencies of the same package, pick the one with the highest version to resolve peers', async () => {
  ...
  await addDependenciesToPackage({}, ['@pnpm.e2e/abc-parent-with-aliases-of-same-pkg@1.0.0'], await testDefaults({ autoInstallPeers: false, strictPeerDependencies: false, useLockfileV6: true }))
