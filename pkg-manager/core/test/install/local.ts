...
test('a symlink to a symlink to a local dependency is preserved', async () => {
  ...
  expect(await fs.readlink(path.resolve('node_modules/local-pkg'))).toContain(
    'symlink'
  )
})
...
test('package with a broken symlink', async () => {
  ...
  await addDependenciesToPackage(
    {},
    [f.find('has-broken-symlink/has-broken-symlink.tar.gz')],
...
test('tarball local package', async () => {
  ...
  const manifest = await addDependenciesToPackage(
    {},
    [f.find('tar-pkg/tar-pkg-1.0.0.tgz')],
    await testDefaults({ fastUnpack: false })
  )
  ...
  const lockfile = await project.readLockfile()
  expect(
    lockfile.packages[lockfile.dependencies['tar-pkg'].version]
  ).toStrictEqual({
    dev: false,
    name: 'tar-pkg',
    resolution: {
...
test('update tarball local package when its integrity changes', async () => {
  ...
  f.copy(
    'tar-pkg-with-dep-1/tar-pkg-with-dep-1.0.0.tgz',
  ...
  f.copy(
    'tar-pkg-with-dep-2/tar-pkg-with-dep-1.0.0.tgz',
    path.resolve('..', 'tar.tgz')
  )
  ...
  const manifestOfTarballDep = await import(
    path.resolve('node_modules/tar-pkg-with-dep/package.json')
  )
...
test('re-install should update local file dependency', async () => {
  ...
  await install(manifest, await testDefaults())
  await expect(
    fs.access('./node_modules/local-pkg/add.js')
  ...
  // add dependency
  await expect(
    fs.access('./node_modules/.pnpm/is-positive@1.0.0')
  ).rejects.toThrow()
  await fs.writeFile(
  ...
  await install(manifest, await testDefaults())
  await expect(
    fs.access('./node_modules/.pnpm/is-positive@2.0.0')
  ).resolves.toBeUndefined()
  lockfile = await project.readLockfile()
  expect(lockfile).toMatchObject({
    packages: {
...
test('local directory is not relinked if disableRelinkLocalDirDeps is set to true', async () => {
  ...
  await addDependenciesToPackage(
    manifest,
    ['is-odd@1.0.0'],
    await testDefaults({ disableRelinkLocalDirDeps: true })
  )

  expect(fss.readdirSync('node_modules/pkg').sort()).toStrictEqual([
    'index.js',
    'package.json',
  ])
