...
test("don't override existing spec in package.json on named installation", async () => {
  ...
  expect(project.requireModule('is-positive/package.json').version).toBe(
    '2.0.0'
  )
  expect(project.requireModule('is-negative/package.json').version).toBe(
    '1.0.1'
  )
...
test('dependency should be removed from the old field when installing it as a different type of dependency', async () => {
...

  {
    ...
    expect(Object.keys(lockfile.dependencies)).toStrictEqual([
      '@pnpm.e2e/bar',
      '@pnpm.e2e/foo',
      '@pnpm.e2e/qar',
    ])
  ...
  {
    const lockfile = await project.readCurrentLockfile()
    expect(Object.keys(lockfile.devDependencies)).toStrictEqual([
      '@pnpm.e2e/bar',
      '@pnpm.e2e/foo',
      '@pnpm.e2e/qar',
    ])
...
test('multiple save to package.json with `exact` versions (@rstacruz/tap-spec & rimraf@2.5.1) (in sorted order)', async () => {
  ...
  const manifest = await addDependenciesToPackage(
    {},
    ['is-positive@1.0.0', '@zkochan/foo@latest'],
...
test('save to package.json with save prefix ~', async () => {
  ...
  const manifest = await addDependenciesToPackage(
    ...
    await testDefaults({ pinnedVersion: 'minor' })
  )
