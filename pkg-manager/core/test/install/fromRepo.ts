...
test('a subdependency is from a github repo with different name', async () => {
  ...
  const lockfile = await project.readLockfile()
  expect(
    lockfile.packages['/@pnpm.e2e/has-aliased-git-dependency@1.0.0']
      .dependencies
  ).toStrictEqual({
  ...
  await project.isExecutable(
    '@pnpm.e2e/has-aliased-git-dependency/node_modules/.bin/hi'
  )
  await project.isExecutable(
    '@pnpm.e2e/has-aliased-git-dependency/node_modules/.bin/szia'
  )

  expect(
    await exists(
      path.resolve(
        'node_modules/.pnpm/@pnpm.e2e+has-say-hi-peer@1.0.0_hi@1.0.0/node_modules/@pnpm.e2e/has-say-hi-peer'
      )
    )
  ).toBeTruthy()
})
...
test('from a git repo', async () => {
  ...
  await addDependenciesToPackage(
    {},
    ['git+ssh://git@github.com/kevva/is-negative.git'],
    await testDefaults()
  )
...
test.skip('from a non-github git repo', async () => {
  ...
  await addDependenciesToPackage(
    {},
    ['git+http://ikt.pm2.io/ikt.git#3325a3e39a502418dc2e2e4bf21529cbbde96228'],
    await testDefaults()
  )
...
test.skip('from a github repo that needs to be built. isolated node linker is used', async () => {
  ...
  const manifest = await addDependenciesToPackage(
    ...
    await testDefaults({ ignoreScripts: true }, { ignoreScripts: true })
  )
  ...
  await install(
    manifest,
    await testDefaults(
      { frozenLockfile: true, ignoreScripts: true },
      { ignoreScripts: true }
    )
  )
  await project.hasNot('@pnpm.e2e/prepare-script-works/prepare.txt')
...
test.skip('from a github repo that needs to be built. hoisted node linker is  used', async () => {
  ...
  const manifest = await addDependenciesToPackage(
    ...
    ['pnpm-e2e/prepare-script-works'],
    await testDefaults(
      { ignoreScripts: true, nodeLinker: 'hoisted' },
      { ignoreScripts: true }
    )
  )
  ...
  await install(
    manifest,
    await testDefaults({ preferFrozenLockfile: false, nodeLinker: 'hoisted' })
  )
  await project.has('@pnpm.e2e/prepare-script-works/prepare.txt')
  ...
  await install(
    manifest,
    await testDefaults({ frozenLockfile: true, nodeLinker: 'hoisted' })
  )
  await project.has('@pnpm.e2e/prepare-script-works/prepare.txt')
  ...
  await install(
    manifest,
    await testDefaults(
      { frozenLockfile: true, ignoreScripts: true, nodeLinker: 'hoisted' },
      { ignoreScripts: true }
    )
  )
  await project.hasNot('@pnpm.e2e/prepare-script-works/prepare.txt')
...
test('re-adding a git repo with a different tag', async () => {
  ...
  expect(
    JSON.parse(
      fs.readFileSync('./node_modules/is-negative/package.json', 'utf8')
    ).version
  ).toBe('1.0.0')
...
test('should not update when adding unrelated dependency', async () => {
  ...
  await install(
    ...
    await testDefaults({
      ...
      dir: withGitProtocolDepFixture,
      lockfileDir: withGitProtocolDepFixture,
    })
  )

  expect(
    fs.existsSync(
      './node_modules/.pnpm/github.com+kevva+is-negative@219c424611ff4a2af15f7deeff4f93c62558c43d'
    )
  ).toBe(true)
  ...
  expect(manifest.dependencies['is-negative']).toBe(
    'github:kevva/is-negative#master'
  )
