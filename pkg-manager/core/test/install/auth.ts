...
const skipOnNode17 = ['v14', 'v16'].includes(process.version.split('.')[0])
  ? test
  : test.skip
...
skipOnNode17(
  'a package that need authentication reuses authorization tokens for tarball fetching',
  async () => {
    const project = prepareEmpty()
...
skipOnNode17(
  'a package that need authentication reuses authorization tokens for tarball fetching when meta info is cached',
  async () => {
    const project = prepareEmpty()
