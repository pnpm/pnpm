...
test('using side effects cache', async () => {
  ...
  const filesIndexFile = getFilePathInCafs(cafsDir, getIntegrity('@pnpm.e2e/pre-and-postinstall-scripts-example', '1.0.0'))
...
test('a postinstall script does not modify the original sources added to the store', async () => {
  ...
  const cafsDir = path.join(opts.storeDir, 'files')
  const filesIndexFile = getFilePathInCafs(cafsDir, getIntegrity('@pnpm/postinstall-modifies-source', '1.0.0'))
  const filesIndex = loadJsonFile.sync<PackageFilesIndex>(filesIndexFile)
