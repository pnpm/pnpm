...
test('using side effects cache', async () => {
  ...
  const sideEffectsKey = `${ENGINE_NAME};deps=${hashObject({
    uniquePkgId: `@pnpm.e2e/pre-and-postinstall-scripts-example@1.0.0/${getIntegrity('@pnpm.e2e/pre-and-postinstall-scripts-example', '1.0.0')}`,
    deps: {
      '@pnpm.e2e/hello-world-js-bin': hashObject({
        uniquePkgId: `@pnpm.e2e/hello-world-js-bin@1.0.0/${getIntegrity('@pnpm.e2e/hello-world-js-bin', '1.0.0')}`,
        deps: {},
      }),
    },
  })}`
...
test('a postinstall script does not modify the original sources added to the store', async () => {
  ...
  const patchedFileIntegrity = filesIndex.sideEffects?.[`${ENGINE_NAME};deps=${hashObject({
    uniquePkgId: `@pnpm/postinstall-modifies-source@1.0.0/${getIntegrity('@pnpm/postinstall-modifies-source', '1.0.0')}`,
    deps: {},
