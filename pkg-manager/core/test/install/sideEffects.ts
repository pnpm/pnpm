...
test('using side effects cache', async () => {
  ...
  expect(filesIndex.sideEffects).toHaveProperty([
    sideEffectsKey,
  ...
  ).toBeTruthy() // side effects cache correctly used
  expect(
    await exists(
      path.resolve(
  ...
  ).toBeFalsy() // side effects cache correctly used
})
...
test.skip('readonly side effects cache', async () => {
  ...
  await fs.writeFile(
    path.join(cacheBuildDir, 'new-file.txt'),
    'some new content'
  )
  ...
  expect(
    await exists(
      path.join(
        opts2.storeDir,
        `localhost+${REGISTRY_MOCK_PORT}/diskusage/1.1.2/side_effects/${ENGINE_DIR}/package/build`
      )
    )
  ).toBeFalsy()
})
...
test('a postinstall script does not modify the original sources added to the store', async () => {
  ...
  await addDependenciesToPackage(
    {},
    ['@pnpm/postinstall-modifies-source@1.0.0'],
    opts
  )

  expect(
    readFileSync(
      'node_modules/@pnpm/postinstall-modifies-source/empty-file.txt',
      'utf8'
    )
  ).toContain('hello')
  ...
  const filesIndexFile = getFilePathInCafs(
    cafsDir,
    getIntegrity('@pnpm/postinstall-modifies-source', '1.0.0'),
    'index'
  )
  const filesIndex = await loadJsonFile<PackageFilesIndex>(filesIndexFile)
  const patchedFileIntegrity =
    filesIndex.sideEffects?.[`${ENGINE_NAME}-${hashObject({})}`][
      'empty-file.txt'
    ]?.integrity
  ...
  expect(
    readFileSync(
      getFilePathInCafs(cafsDir, originalFileIntegrity, 'nonexec'),
      'utf8'
    )
  ).toEqual('')
})
