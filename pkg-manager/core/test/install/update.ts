...
test('preserve subdeps on update', async () => {
  ...
  await Promise.all([
    addDistTag({
      package: '@pnpm.e2e/abc-grand-parent-with-c',
      version: '1.0.0',
      distTag: 'latest',
    }),
    addDistTag({
      package: '@pnpm.e2e/abc-parent-with-ab',
      version: '1.0.0',
      distTag: 'latest',
    }),
    addDistTag({
  ...
  const manifest = await addDependenciesToPackage(
    {},
    ['@pnpm.e2e/foobarqar', '@pnpm.e2e/abc-grand-parent-with-c'],
    await testDefaults()
  )
  ...
  expect(
    lockfile.packages['/@pnpm.e2e/foobarqar@1.0.1'].dependencies
  ).toStrictEqual({
...
test('preserve subdeps on update when no node_modules is present', async () => {
  ...
  expect(lockfile.packages).toHaveProperty([
    '/@pnpm.e2e/abc-parent-with-ab@1.0.0(@pnpm.e2e/peer-c@1.0.0)',
  ]) // preserve version of package that has resolved peer deps
  expect(lockfile.packages).toHaveProperty(['/@pnpm.e2e/foobarqar@1.0.1'])
  expect(
    lockfile.packages['/@pnpm.e2e/foobarqar@1.0.1'].dependencies
  ).toStrictEqual({
...
test('update does not fail when package has only peer dependencies', async () => {
  ...
  const manifest = await addDependenciesToPackage(
    {},
    ['@pnpm.e2e/has-pkg-with-peer-only'],
    await testDefaults()
  )
...
test('update dependency when external lockfile directory is used', async () => {
  ...
  await install(
    manifest,
    await testDefaults({ update: true, depth: 0, lockfileDir })
  )
...
test('update only the packages that were requested to be updated when hoisting is on', async () => {
  ...
  manifest = await addDependenciesToPackage(
    manifest,
    ['@pnpm.e2e/foo'],
    await testDefaults({ allowNew: false, update: true, hoistPattern: ['*'] })
  )
...
test('update only the specified package', async () => {
  ...
  expect(
    lockfile.packages['/@pnpm.e2e/foobarqar@1.0.0'].dependencies
  ).toStrictEqual({
    '@pnpm.e2e/bar': '100.0.0',
    '@pnpm.e2e/foo': '100.1.0',
    'is-positive': '3.1.0',
