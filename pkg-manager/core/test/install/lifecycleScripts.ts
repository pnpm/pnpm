...
test('run pre/postinstall scripts', async () => {
...

  {
    ...
    const generatedByPostinstall = project.requireModule(
  ...
  await install(
    manifest,
    await testDefaults({ fastUnpack: false, production: true })
  )
  ...
  expect(
    lockfile.packages['/@pnpm.e2e/pre-and-postinstall-scripts-example@1.0.0']
      .requiresBuild
  )
})
...
test('run pre/postinstall scripts, when PnP is used and no symlinks', async () => {
  ...
  const pkgDir =
    'node_modules/.pnpm/@pnpm.e2e+pre-and-postinstall-scripts-example@1.0.0/node_modules/@pnpm.e2e/pre-and-postinstall-scripts-example'
  expect(
    await exists(path.resolve(pkgDir, 'generated-by-prepare.js'))
  ).toBeFalsy()
  expect(
    await exists(path.resolve(pkgDir, 'generated-by-preinstall.js'))
  ).toBeTruthy()
  expect(
    await exists(path.resolve(pkgDir, 'generated-by-postinstall.js'))
  ).toBeTruthy()
})
...
test('run install scripts', async () => {
  ...
  const generatedByInstall = project.requireModule(
...
test('INIT_CWD is always set to lockfile directory', async () => {
  ...
  await mutateModulesInSingleProject(
    {
      ...
      manifest: {
        ...
        scripts: {
          install:
            'node -e "fs.writeFileSync(\'output.json\', JSON.stringify(process.env.INIT_CWD))"',
        },
      },
      rootDir,
    },
  ...
  const childEnv = await loadJsonFile<{ INIT_CWD: string }>(
    path.join(rootDir, 'node_modules/@pnpm.e2e/write-lifecycle-env/env.json')
  )
...
test('run lifecycle scripts of dependent packages after running scripts of their deps', async () => {
  ...
  expect(
    ...
    )[0] < +project.requireModule('@pnpm.e2e/with-postinstall-a/output.json')[0]
  ).toBeTruthy()
})
...
test('bins are linked even if lifecycle scripts are ignored', async () => {
  ...
  await project.isExecutable(
    '@pnpm.e2e/pkg-with-peer-having-bin/node_modules/.bin/hello-world-js-bin'
  )
...
test('scripts have access to unlisted bins when hoisting is used', async () => {
  ...
  expect(
    project.requireModule(
      '@pnpm.e2e/pkg-that-calls-unlisted-dep-in-hooks/output.json'
    )
...
test('selectively ignore scripts in some dependencies by neverBuiltDependencies', async () => {
  ...
  expect(
    lockfile.packages['/@pnpm.e2e/install-script-example@1.0.0'].requiresBuild
  ).toBeTruthy()
...
test('throw an exception when both neverBuiltDependencies and onlyBuiltDependencies are used', async () => {
  ...
  await expect(
    addDependenciesToPackage(
      ...
      ['@pnpm.e2e/pre-and-postinstall-scripts-example@1.0.0'],
      await testDefaults({
        onlyBuiltDependencies: ['@pnpm.e2e/foo'],
        neverBuiltDependencies: ['@pnpm.e2e/bar'],
      })
    )
...
test('selectively allow scripts in some dependencies by onlyBuiltDependenciesFile', async () => {
  prepareEmpty()
  const onlyBuiltDependenciesFile = path.resolve(
...
test('lockfile is updated if neverBuiltDependencies is changed', async () => {
  ...
  {
    const lockfile = await project.readLockfile()
    expect(lockfile.neverBuiltDependencies).toStrictEqual(
      neverBuiltDependencies
    )
    expect(
      lockfile.packages['/@pnpm.e2e/pre-and-postinstall-scripts-example@1.0.0']
...
test("lifecycle scripts have access to package's own binary by binary name", async () => {
  ...
  await project.isExecutable(
    '.pnpm/@pnpm.e2e+runs-own-bin@1.0.0/node_modules/@pnpm.e2e/runs-own-bin/node_modules/.bin/runs-own-bin'
  )
})
...
test('ignore-dep-scripts', async () => {
  ...
  await install(
    manifest,
    await testDefaults({ fastUnpack: false, ignoreDepScripts: true })
  )
...
test('run pre/postinstall scripts in a workspace that uses node-linker=hoisted', async () => {
  ...
  await projects['project-3'].has(
    '@pnpm.e2e/pre-and-postinstall-scripts-example/generated-by-preinstall.js'
  )
  await projects['project-3'].has(
    '@pnpm.e2e/pre-and-postinstall-scripts-example/generated-by-postinstall.js'
  )
  await projects['project-4'].has(
    '@pnpm.e2e/pre-and-postinstall-scripts-example/generated-by-preinstall.js'
  )
  await projects['project-4'].has(
    '@pnpm.e2e/pre-and-postinstall-scripts-example/generated-by-postinstall.js'
