...
test.skip('ignoring some files in the dependency', async () => {
  ...
  await addDependenciesToPackage(
    {},
    ['is-positive@1.0.0'],
    await testDefaults({}, {}, { ignoreFile })
  )

  // package.json was not ignored
  expect(
    await exists(path.resolve('node_modules', 'is-positive', 'package.json'))
  ).toBeTruthy()
  // readme.md was ignored
  expect(
    await exists(path.resolve('node_modules', 'is-positive', 'readme.md'))
  ).toBeFalsy()
})
...
test('installing a package by specifying a specific dist-tag', async () => {
  ...
  await addDependenciesToPackage(
    {},
    ['@pnpm.e2e/dep-of-pkg-with-1-dep@beta'],
    await testDefaults()
  )
...
test('update a package when installing with a dist-tag', async () => {
  ...
  await addDependenciesToPackage(
    manifest,
    ['@pnpm.e2e/dep-of-pkg-with-1-dep@beta'],
    await testDefaults({ targetDependenciesField: 'devDependencies', reporter })
  )
...
test('multiple scoped modules (@rstacruz/...)', async () => {
  ...
  await addDependenciesToPackage(
    {},
    ['@rstacruz/tap-spec@*', '@rstacruz/travis-encrypt@*'],
    await testDefaults({ fastUnpack: false })
  )

  expect(typeof project.requireModule('@rstacruz/tap-spec')).toBe('function')
  expect(typeof project.requireModule('@rstacruz/travis-encrypt')).toBe(
    'function'
  )
})
...
test('nested scoped modules (test-pnpm-issue219 -> @zkochan/test-pnpm-issue219)', async () => {
  ...
  await addDependenciesToPackage(
    {},
    ['@pnpm.e2e/test-pnpm-issue219@1.0.3'],
    await testDefaults({ fastUnpack: false })
  )
...
test('forcing', async () => {
  ...
  await addDependenciesToPackage(
    manifest,
    ['magic-hook@2.0.0'],
    await testDefaults({ fastUnpack: false, force: true })
  )
...
test.skip('relink package to project if the dependency is not linked from store', async () => {
  ...
  const manifest = await addDependenciesToPackage(
    {},
    ['magic-hook@2.0.0'],
    await testDefaults({ save: true, pinnedVersion: 'patch' })
  )
...
test('concurrent circular deps', async () => {
  ...
  expect(
    await exists(
      path.resolve('node_modules/.pnpm/es6-iterator@2.0.0/node_modules/es5-ext')
    )
  ).toBeTruthy()
  expect(
    await exists(
      path.resolve('node_modules/.pnpm/es6-iterator@2.0.1/node_modules/es5-ext')
    )
  ).toBeTruthy()
  expect(
    await exists(
      path.resolve(
        'node_modules/.pnpm/es5-ext@0.10.31/node_modules/es6-iterator'
      )
    )
  ).toBeTruthy()
  expect(
    await exists(
      path.resolve('node_modules/.pnpm/es5-ext@0.10.31/node_modules/es6-symbol')
    )
  ).toBeTruthy()
})
...
test('bin specified in the directories property linked to .bin folder', async () => {
  ...
  await addDependenciesToPackage(
    {},
    ['@pnpm.e2e/pkg-with-directories-bin'],
    await testDefaults({ fastUnpack: false })
  )
...
test('should update subdep on second install', async () => {
  ...
  await install(
    manifest,
    await testDefaults({ depth: 1, update: true, reporter })
  )
...
test('should install dependency in second project', async () => {
  ...
  await addDependenciesToPackage(
    {},
    ['@pnpm.e2e/pkg-with-1-dep'],
    await testDefaults({ fastUnpack: false, save: true, store: '../store' })
  )
  expect(project1.requireModule('@pnpm.e2e/pkg-with-1-dep')().name).toEqual(
    '@pnpm.e2e/dep-of-pkg-with-1-dep'
  )
  ...
  expect(project2.requireModule('@pnpm.e2e/pkg-with-1-dep')().name).toEqual(
    '@pnpm.e2e/dep-of-pkg-with-1-dep'
  )
})
...
test('should throw error when trying to install using a different store then the previous one', async () => {
  ...
  const manifest = await addDependenciesToPackage(
    {},
    ['is-positive'],
    await testDefaults({ storeDir: 'node_modules/.store1' })
  )
  ...
  await expect(
    addDependenciesToPackage(
      manifest,
      ['is-negative'],
      await testDefaults({ storeDir: 'node_modules/.store2' })
    )
  ).rejects.toThrow(
    new UnexpectedStoreError({
      expectedStorePath: '',
      actualStorePath: '',
      modulesDir: '',
    })
...
test('ignores drive case in store path', async () => {
  ...
  const storePathUpper: string = path
    .resolve('node_modules/.store1')
    .toUpperCase()
  ...
  const manifest = await addDependenciesToPackage(
    {},
    ['rimraf@2.5.1'],
    await testDefaults({ storeDir: storePathUpper }, null, null, {
      ignoreFile: () => {},
  ...
  await addDependenciesToPackage(
    manifest,
    ['is-negative'],
    await testDefaults({ storeDir: storePathLower })
  )
})
...
test('should throw error when trying to install using a different virtual store directory then the previous one', async () => {
  ...
  const manifest = await addDependenciesToPackage(
    {},
    ['is-positive'],
    await testDefaults({ virtualStoreDir: 'pkgs' })
  )
  ...
  await expect(
    addDependenciesToPackage(
      manifest,
      ['is-negative'],
      await testDefaults({ virtualStoreDir: 'pnpm' })
    )
  ).rejects.toThrow(
    new UnexpectedVirtualStoreDirError({
      actual: '',
      expected: '',
      modulesDir: '',
    })
...
test('lockfile locks npm dependencies', async () => {
  ...
  const manifest = await addDependenciesToPackage(
    {},
    ['@pnpm.e2e/pkg-with-1-dep'],
    await testDefaults({ save: true, reporter })
  )
  ...
  const m = project.requireModule(
    '.pnpm/@pnpm.e2e+pkg-with-1-dep@100.0.0/node_modules/@pnpm.e2e/dep-of-pkg-with-1-dep/package.json'
  )
...
test('self-require should work', async () => {
  ...
  await addDependenciesToPackage(
    {},
    ['@pnpm.e2e/uses-pkg-with-self-usage'],
    await testDefaults({ fastUnpack: false })
  )

  expect(
    project.requireModule('@pnpm.e2e/uses-pkg-with-self-usage')
  ).toBeTruthy()
})
...
test('should throw error when trying to install a package without name', async () => {
  ...
  await expect(
    addDependenciesToPackage(
      {},
      [`file:${f.find('missing-pkg-name.tgz')}`],
      await testDefaults()
    )
...
test('reinstalls missing packages to node_modules', async () => {
  const project = prepareEmpty()
  const reporter = sinon.spy()
  const depLocation = path.resolve(
    'node_modules/.pnpm/is-positive@1.0.0/node_modules/is-positive'
  )
  const missingDepLog = {
    level: 'debug',
    missing: depLocation,
...
test('all the subdeps of dependencies are linked when a node_modules is partially up to date', async () => {
  ...
  expect(
    [
      ...(await fs.readdir(
        path.resolve(
          'node_modules/.pnpm/@pnpm.e2e+foobarqar@1.0.1/node_modules/@pnpm.e2e'
        )
      )),
    ].sort()
  ).toStrictEqual(['bar', 'foo', 'foobarqar', 'qar'].sort())
})
...
test('subdep symlinks are updated if the lockfile has new subdep versions specified', async () => {
  ...
  expect(
    await exists(
      path.resolve(
        'node_modules/.pnpm/@pnpm.e2e+pkg-with-1-dep@100.0.0/node_modules/@pnpm.e2e/dep-of-pkg-with-1-dep/package.json'
      )
    )
  ).toBeTruthy()
})
...
test('installing with no symlinks with PnP', async () => {
  ...
  expect([...(await fs.readdir(path.resolve('node_modules')))]).toStrictEqual([
    '.bin',
    '.modules.yaml',
    '.pnpm',
  ])
  expect([
    ...(await fs.readdir(
      path.resolve('node_modules/.pnpm/rimraf@2.7.1/node_modules')
    )),
  ]).toStrictEqual(['rimraf'])
...
test('a package should be able to be a dependency of itself', async () => {
  ...
  const manifest = await addDependenciesToPackage(
    {},
    ['@paul-soporan/test-package-self-require-trap@2.0.0'],
    await testDefaults()
  )

  const subpkg =
    '.pnpm/@paul-soporan+test-package-self-require-trap@2.0.0/node_modules/@paul-soporan/test-package-self-require-trap/node_modules/@paul-soporan/test-package-self-require-trap/package.json'
  {
