...
import { addDependenciesToPackage, type PackageManifest } from '@pnpm/core'
...
test('readPackage, afterAllResolved hooks', async () => {
    ...
    switch (manifest.name) {
      case '@pnpm.e2e/pkg-with-1-dep':
        if (manifest.dependencies == null) {
          throw new Error(
            '@pnpm.e2e/pkg-with-1-dep expected to have a dependencies field'
          )
        }
  ...
  expect(afterAllResolved.mock.calls[0][0].lockfileVersion).toEqual(
    LOCKFILE_VERSION
  )
