...
test('auto install the common peer dependency', async () => {
  ...
  await addDependenciesToPackage(
    {},
    ['@pnpm.e2e/wants-peer-c-1', '@pnpm.e2e/wants-peer-c-1.0.0'],
    await testDefaults({ autoInstallPeers: true })
  )
  const lockfile = await project.readLockfile()
  expect(Object.keys(lockfile.packages)).toStrictEqual([
...
test('do not auto install when there is no common peer dependency range intersection', async () => {
  ...
  await addDependenciesToPackage(
    {},
    ['@pnpm.e2e/wants-peer-c-1', '@pnpm.e2e/wants-peer-c-2'],
    await testDefaults({ autoInstallPeers: true })
  )
  const lockfile = await project.readLockfile()
  expect(Object.keys(lockfile.packages)).toStrictEqual([
...
test("don't fail on linked package, when peers are auto installed", async () => {
  ...
  expect(Object.keys(updatedManifest.dependencies ?? {})).toStrictEqual([
    'linked',
    '@pnpm.e2e/peer-b',
  ])
})
...
test('hoist a peer dependency in order to reuse it by other dependencies, when it satisfies them', async () => {
  ...
  const lockfile = await project.readLockfile()
  const suffix = createPeersFolderSuffix([
    { name: '@pnpm/x', version: '1.0.0' },
    { name: '@pnpm/y', version: '1.0.0' },
    { name: '@pnpm/z', version: '1.0.0' },
  ])
  expect(Object.keys(lockfile.packages)).toStrictEqual([
    '/@pnpm/x@1.0.0',
...
test("don't hoist a peer dependency when there is a root dependency by that name", async () => {
  ...
  const lockfile = await project.readLockfile()
  const suffix1 = createPeersFolderSuffix([
    { name: '@pnpm/y', version: '2.0.0' },
    { name: '@pnpm/z', version: '1.0.0' },
    { name: '@pnpm.e2e/peer-a', version: '1.0.0' },
  ])
  const suffix2 = createPeersFolderSuffix([
    { name: '@pnpm/x', version: '1.0.0' },
    { name: '@pnpm/y', version: '1.0.0' },
    { name: '@pnpm/z', version: '1.0.0' },
  ])
...
test("don't install the same missing peer dependency twice", async () => {
  ...
  expect(Object.keys(lockfile.packages).sort()).toStrictEqual(
    [
      '/@pnpm/y@1.0.0',
      `/@pnpm.e2e/has-has-y-peer-peer@1.0.0${createPeersFolderSuffix([
        { name: '@pnpm/y', version: '1.0.0' },
        { name: '@pnpm.e2e/has-y-peer', version: '1.0.0' },
      ])}`,
...
test('prefer the peer dependency version already used in the root', async () => {
  ...
  const lockfile = await project.readLockfile()
  expect(Object.keys(lockfile.packages).sort()).toStrictEqual(
    ['/@pnpm/y@1.0.0', '/@pnpm.e2e/has-y-peer@1.0.0(@pnpm/y@1.0.0)'].sort()
  )
})
...
test('automatically install root peer dependencies', async () => {
  ...
  await install(
    manifest,
    await testDefaults({ autoInstallPeers: true, frozenLockfile: true })
  )
      ...
      hooks: {
        ...
        readPackage: [
          (pkg: PackageManifest): PackageManifest => {
            ...
            if (
              pkg.name === '@angular-devkit/build-angular' &&
              pkg.dependencies
            ) {
