...
test('install with lockfileOnly = true', async () => {
  ...
  expect(
    await exists(
      path.join(
        opts.cacheDir,
        `metadata/localhost+${REGISTRY_MOCK_PORT}/@pnpm.e2e/pkg-with-1-dep.json`
      )
    )
  ).toBeTruthy()
  ...
  expect(
    await exists(
      path.join(
        opts.cacheDir,
        `metadata/localhost+${REGISTRY_MOCK_PORT}/@pnpm.e2e/dep-of-pkg-with-1-dep.json`
      )
    )
  ).toBeTruthy()
  ...
  console.log(
    `doing repeat install when ${WANTED_LOCKFILE} is available already`
  )
