...
test('fix broken lockfile with --fix-lockfile', async () => {
  ...
  expect(
    lockfile.packages?.['/core-js-pure@3.16.2']?.requiresBuild
  ).toBeTruthy()
...
test('--fix-lockfile should preserve all locked dependencies version', async () => {
  ...
  expect(
    lockfile.packages?.['/@babel/runtime-corejs3@7.15.3']?.resolution
  ).toEqual({
    integrity:
      'sha512-30A3lP+sRL6ml8uhoJSs+8jwpKzbw8CqBvDc1laeptxPm5FahumJxirigcbD2qTs71Sonvj1cyZB0OKGAmxQ+A==',
  })
  expect(
    lockfile.packages?.['/@babel/runtime-corejs3@7.15.3']?.engines
  ).toEqual({
    node: '>=6.9.0',
  })
  ...
  expect(
    lockfile.packages?.['/@babel/runtime-corejs3@7.15.4']?.resolution
  ).toEqual({
    integrity:
      'sha512-lWcAqKeB624/twtTc3w6w/2o9RqJPaNBhPGK6DKLSiwuVWC7WFkypWyNg+CpZoyJH0jVzv1uMtXZ/5/lQOLtCg==',
  })
  expect(
    lockfile.packages?.['/@babel/runtime-corejs3@7.15.4']?.engines
  ).toEqual({
    node: '>=6.9.0',
  })
  ...
  expect(
    lockfile.packages?.['/core-js-pure@3.17.2']?.resolution
  ).toHaveProperty(
    'integrity',
    'sha512-2VV7DlIbooyTI7Bh+yzOOWL9tGwLnQKHno7qATE+fqZzDKYr6llVjVQOzpD/QLZFgXDPb8T71pJokHEZHEYJhQ=='
  )
  expect(
    lockfile.packages?.['/core-js-pure@3.17.2']?.requiresBuild
  ).toBeTruthy()
...
test('--fix-lockfile should install successfully when package has no dependencies but has peer dependencies with version like 1.0.0_@pnpm+y@1.0.0', async () => {
  prepareEmpty()
