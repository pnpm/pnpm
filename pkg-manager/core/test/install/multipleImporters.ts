...
test('some projects were removed from the workspace and the ones that are left depend on them', async () => {
  ...
  await mutateModules(
    importers,
    await testDefaults({ allProjects, workspacePackages })
  )
...
test('headless install is used with an up-to-date lockfile when package references another package via workspace: protocol', async () => {
  ...
  await mutateModules(
    importers,
    await testDefaults({ allProjects, lockfileOnly: true, workspacePackages })
  )

  const reporter = sinon.spy()
  await mutateModules(
    importers,
    await testDefaults({ allProjects, reporter, workspacePackages })
  )
...
test('headless install is used when packages are not linked from the workspace (unless workspace ranges are used)', async () => {
  ...
  preparePackages([
    { location: 'foo', package: foo },
    { location: 'bar', package: bar },
    { location: 'qar', package: qar },
  ])
...
test('current lockfile contains only installed dependencies when adding a new importer to workspace with shared lockfile', async () => {
  ...
  const currentLockfile = await readCurrentLockfile(
...
test('partial installation in a monorepo does not remove dependencies of other workspace projects', async () => {
    ...
    path.resolve('pnpm-lock.yaml'),
    {
      ...
      packages: {
        '/@pnpm.e2e/dep-of-pkg-with-1-dep@100.0.0': {
          ...
          resolution: {
            integrity:
              'sha512-RWObNQIluSr56fVbOwD75Dt5CE2aiPReTMMUblYEMEqUI+iJw5ovTyO7LzUG/VJ4iVL2uUrbkQ6+rq4z4WOdDw==',
          },
        },
        '/is-positive@1.0.0': {
          dev: false,
        ...
        '/@pnpm.e2e/pkg-with-1-dep@100.0.0': {
          ...
          resolution: {
            integrity:
              'sha512-OStTw86MRiQHB1JTSy6wl+9GT46aK8w4ghZT3e8ZN899J+FUsfD1nFl5gANa4Qol1LTBRqXeKomgXIAo9R/RZA==',
          },
        },
      },
    },
  ...
  expect(
    await exists(
      path.resolve(
        'node_modules/.pnpm/is-positive@2.0.0/node_modules/is-positive'
      )
    )
  ).toBeTruthy()
  expect(
    await exists(
      path.resolve(
        'node_modules/.pnpm/@pnpm.e2e+pkg-with-1-dep@100.0.0/node_modules/@pnpm.e2e/pkg-with-1-dep'
      )
    )
  ).toBeTruthy()
  expect(
    await exists(
      path.resolve(
        'node_modules/.pnpm/@pnpm.e2e+dep-of-pkg-with-1-dep@100.1.0/node_modules/@pnpm.e2e/dep-of-pkg-with-1-dep'
      )
    )
  ).toBeTruthy()
})
...
test('remove dependencies of a project that was removed from the workspace (during non-headless install)', async () => {
...

  {
    ...
    expect(Object.keys(currentLockfile.importers)).toStrictEqual([
      'project-1',
      'project-2',
    ])
    expect(Object.keys(currentLockfile.packages)).toStrictEqual([
...
test('do not resolve a subdependency from the workspace by default', async () => {
  ...
  const wantedLockfile = await project.readLockfile()
  expect(
    wantedLockfile.packages['/@pnpm.e2e/pkg-with-1-dep@100.0.0'].dependencies?.[
      '@pnpm.e2e/dep-of-pkg-with-1-dep'
    ]
  ).toBe('100.1.0')
})
...
test('resolve a subdependency from the workspace', async () => {
  ...
  ).toBe('link:@pnpm.e2e/dep-of-pkg-with-1-dep')
...
test('resolve a subdependency from the workspace and use it as a peer', async () => {
  ...
  const suffix1 = createPeersFolderSuffix([
    { name: '@pnpm.e2e/peer-a', version: '@pnpm.e2e+peer-a' },
    { name: '@pnpm.e2e/peer-b', version: '1.0.0' },
  ])
  const suffix2 = createPeersFolderSuffix([
    { name: '@pnpm.e2e/peer-a', version: '@pnpm.e2e+peer-a' },
    { name: '@pnpm.e2e/peer-b', version: '1.0.0' },
    { name: '@pnpm.e2e/peer-c', version: '1.0.1' },
  ])
  ...
  expect(
    wantedLockfile.packages['/@pnpm.e2e/abc-parent-with-ab@1.0.0']
      .dependencies?.['@pnpm.e2e/peer-a']
  ).toBe('link:@pnpm.e2e/peer-a')
  expect(
    wantedLockfile.packages[`/@pnpm.e2e/abc@1.0.0${suffix1}`].dependencies?.[
      '@pnpm.e2e/peer-a'
    ]
  ).toBe('link:@pnpm.e2e/peer-a')
})
...
test('install the dependency that is already present in the workspace when adding a new direct dependency', async () => {
  ...
  expect(
    currentLockfile.importers['project-1'].dependencies?.[
      '@pnpm.e2e/dep-of-pkg-with-1-dep'
    ]
  ).toStrictEqual({
    specifier: '^100.0.0',
  ...
  expect(
    currentLockfile.importers['project-2'].dependencies?.[
      '@pnpm.e2e/dep-of-pkg-with-1-dep'
    ]
  ).toStrictEqual({
    specifier: '^100.0.0',
...
test('symlink local package from the location described in its publishConfig.directory when linkDirectory is true', async () => {
  ...
  {
    const linkedManifest = await loadJsonFile<{ name: string }>(
      'project-2/node_modules/project-1/package.json'
    )
  ...
  await mutateModules(
    importers,
    await testDefaults({ allProjects, frozenLockfile: true, workspacePackages })
  )
