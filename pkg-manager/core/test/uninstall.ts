...
test('uninstall tarball dependency', async () => {
  ...
  let manifest = await addDependenciesToPackage(
    {},
    [`http://localhost:${REGISTRY_MOCK_PORT}/is-array/-/is-array-1.0.1.tgz`],
    opts
  )
  manifest = (
    await mutateModulesInSingleProject(
...
test('uninstall package with dependencies and do not touch other deps', async () => {
  ...
  manifest = (
    await mutateModulesInSingleProject(
      ...
      await testDefaults({ pruneStore: true, save: true })
    )
  ).manifest
...
test('uninstall package with its bin files', async () => {
  ...
  const stat = await existsSymlink(
    path.resolve('node_modules', '.bin', 'sh-hello-world')
  )
  expect(stat).toBeFalsy()

  expect(
    await exists(path.resolve('node_modules', '.bin', 'sh-hello-world'))
  ).toBeFalsy()
  expect(
    await exists(path.resolve('node_modules', '.bin', 'sh-hello-world.cmd'))
  ).toBeFalsy()
  expect(
    await exists(path.resolve('node_modules', '.bin', 'sh-hello-world.ps1'))
  ).toBeFalsy()
})
...
test('relative link is uninstalled', async () => {
  ...
  const manifest = await link(
    ...
    opts as typeof opts & { dir: string; manifest: PackageManifest }
  )
  await mutateModulesInSingleProject(
    {
...
test('uninstall remove modules that is not in package.json', async () => {
  ...
  await writeJsonFile('node_modules/foo/package.json', {
    name: 'foo',
    version: '1.0.0',
  })
