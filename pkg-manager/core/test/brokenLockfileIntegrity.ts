...
test('installation breaks if the lockfile contains the wrong checksum', async () => {
  ...
  ;(
    corruptedLockfile.packages['/@pnpm.e2e/pkg-with-1-dep@100.0.0']
      .resolution as TarballResolution
  ).integrity = (
    corruptedLockfile.packages['/@pnpm.e2e/dep-of-pkg-with-1-dep@100.0.0']
      .resolution as TarballResolution
  ).integrity
  ...
  ).rejects.toThrowError(/Package name mismatch found while reading/)
...
test('installation breaks if the lockfile contains the wrong checksum and the store is clean', async () => {
  ...
  await mutateModulesInSingleProject(
    ...
    await testDefaults(
      { preferFrozenLockfile: false, reporter },
      { retry: { retries: 0 } }
    )
  )
