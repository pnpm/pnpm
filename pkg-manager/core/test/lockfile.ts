...
test('lockfile has correct format', async () => {
  ...
  expect(lockfile.dependencies['@pnpm.e2e/pkg-with-1-dep'].version).toBe(
    '100.0.0'
  )
  ...
  expect(
    (lockfile.packages[id].resolution as { integrity: string }).integrity
  ).toBeTruthy()
  expect(
    (lockfile.packages[id].resolution as TarballResolution).tarball
  ).toBeFalsy()
...
test("lockfile doesn't lock subdependencies that don't satisfy the new specs", async () => {
  ...
  expect(
    project.requireModule(
      '.pnpm/react-datetime@1.3.0/node_modules/react-onclickoutside/package.json'
    ).version
...
test(`respects ${WANTED_LOCKFILE} for top dependencies`, async () => {
  ...
  await Promise.all(
    pkgs.map(async (pkgName) =>
      addDistTag({ package: pkgName, version: '100.0.0', distTag: 'latest' })
    )
  )
  ...
  manifest = await addDependenciesToPackage(
    manifest,
    ['@pnpm.e2e/qar'],
    await testDefaults({ addDependenciesToPackage: 'devDependencies' })
  )
  manifest = await addDependenciesToPackage(
    manifest,
    ['@pnpm.e2e/foobar'],
  ...
  expect(
    (
      await readPackageJsonFromDir(
        path.resolve('node_modules', '@pnpm.e2e/foo')
      )
    ).version
  ).toBe('100.0.0')
  expect(
    (
      await readPackageJsonFromDir(
        path.resolve('node_modules', '@pnpm.e2e/bar')
      )
    ).version
  ).toBe('100.0.0')
  expect(
    (
      await readPackageJsonFromDir(
        path.resolve('node_modules', '@pnpm.e2e/qar')
      )
    ).version
  ).toBe('100.0.0')
  expect(
    (
      await readPackageJsonFromDir(
        path.resolve(
          'node_modules/.pnpm/@pnpm.e2e+foobar@100.0.0/node_modules/@pnpm.e2e/foo'
        )
      )
    ).version
  ).toBe('100.0.0')
  expect(
    (
      await readPackageJsonFromDir(
        path.resolve(
          'node_modules/.pnpm/@pnpm.e2e+foobar@100.0.0/node_modules/@pnpm.e2e/bar'
        )
      )
    ).version
  ).toBe('100.0.0')
  ...
  await Promise.all(
    pkgs.map(async (pkgName) =>
      addDistTag({ package: pkgName, version: '100.1.0', distTag: 'latest' })
    )
  )
...
test('package is not marked dev if it is also a subdep of a regular dependency', async () => {
  ...
  console.log(
    'installed optional dependency which is also a dependency of @pnpm.e2e/pkg-with-1-dep'
  )

  const lockfile = await project.readLockfile()

  expect(
    lockfile.packages['/@pnpm.e2e/dep-of-pkg-with-1-dep@100.0.0'].dev
  ).toBeFalsy()
})
...
test('package is not marked optional if it is also a subdep of a regular dependency', async () => {
  ...
  expect(
    lockfile.packages['/@pnpm.e2e/dep-of-pkg-with-1-dep@100.0.0'].optional
  ).toBeFalsy()
})
  ...
  expect(lockfile).toStrictEqual({
    ...
    packages: {
      ...
      '/@zkochan/foo@1.0.0': {
        ...
        resolution: {
          integrity:
            'sha512-IFvrYpq7E6BqKex7A7czIFnFncPiUVdhSzGhAOWpp8RlkXns4y/9ZdynxaA/e0VkihRxQkihE2pTyvxjfe/wBg==',
        },
      },
      '/is-negative@1.0.0': {
        dev: false,
...
test('packages are placed in devDependencies even if they are present as non-dev as well', async () => {
  ...
  expect(lockfile.devDependencies).toHaveProperty([
    '@pnpm.e2e/dep-of-pkg-with-1-dep',
  ])
...
test('updating package that has a github-hosted dependency', async () => {
  ...
  const manifest = await addDependenciesToPackage(
    {},
    ['@pnpm.e2e/has-github-dep@1'],
    await testDefaults()
  )
  await addDependenciesToPackage(
    manifest,
    ['@pnpm.e2e/has-github-dep@latest'],
    await testDefaults()
  )
})
...
test('updating package that has deps with peers', async () => {
  ...
  const manifest = await addDependenciesToPackage(
    {},
    ['@pnpm.e2e/abc-grand-parent-with-c@0'],
    await testDefaults()
  )
  await addDependenciesToPackage(
    manifest,
    ['@pnpm.e2e/abc-grand-parent-with-c@1'],
    await testDefaults()
  )
})
...
test('dev properties are correctly updated on named install', async () => {
  ...
  const lockfile = await project.readLockfile()
  expect(
    Object.values(lockfile.packages).filter(
      (dep) => typeof dep.dev !== 'undefined'
    )
  ).toStrictEqual([])
})
...
test('optional properties are correctly updated on named install', async () => {
  ...
  expect(
    Object.values(lockfile.packages).filter(
      (dep) => typeof dep.optional !== 'undefined'
    )
  ).toStrictEqual([])
})
...
test('fail when installing with useLockfile: false and lockfileOnly: true', async () => {
  ...
  try {
    await install(
      {},
      await testDefaults({ useLockfile: false, lockfileOnly: true })
    )
...
test("don't remove packages during named install when useLockfile: false", async () => {
  ...
  const manifest = await addDependenciesToPackage(
    {},
    ['is-positive'],
    await testDefaults({ useLockfile: false })
  )
  await addDependenciesToPackage(
    manifest,
    ['is-negative'],
...
test('save tarball URL when it is non-standard', async () => {
  ...
  expect(
    (
      lockfile.packages['/esprima-fb@3001.1.0-dev-harmony-fb']
        .resolution as TarballResolution
    ).tarball
  ).toBe(
    `http://localhost:${REGISTRY_MOCK_PORT}/esprima-fb/-/esprima-fb-3001.0001.0000-dev-harmony-fb.tgz`
  )
})
...
test('lockfile file has correct format when lockfile directory does not equal the prefix directory', async () => {
  ...
  const modules = await readYamlFile<any>(
    path.resolve('node_modules', '.modules.yaml')
  )
  expect(modules).toBeTruthy()
  expect(modules.pendingBuilds.length).toBe(0)
...
test(`doing named installation when shared ${WANTED_LOCKFILE} exists already`, async () => {
  ...
  const currentLockfile = await readYamlFile<LockfileV6>(
...
test(`use current ${WANTED_LOCKFILE} as initial wanted one, when wanted was removed`, async () => {
  ...
  const manifest = await addDependenciesToPackage(
    {},
    ['lodash@4.17.11', 'underscore@1.9.0'],
    await testDefaults()
  )
...
test('existing dependencies are preserved when updating a lockfile to a newer format', async () => {
  ...
  await writeYamlFile(
    WANTED_LOCKFILE,
    { ...initialLockfile, lockfileVersion: 5.01 },
    { lineWidth: 1000 }
  )
...
const isPositiveMeta = loadJsonFile.sync<any>(
  path.join(REGISTRY_MIRROR_DIR, 'is-positive.json')
)
...
>>>>>>> next`,
  ...
  const lockfile = await project.readLockfile()
  expect(
    lockfile.dependencies['@pnpm.e2e/dep-of-pkg-with-1-dep']
  ).toHaveProperty('version', '100.1.0')
})
  ...
  expect(reporter).toBeCalledWith(
    expect.objectContaining({
      ...
      message: expect.stringMatching(
        /^Ignoring broken lockfile at .* duplicated mapping key/
      ),
    })
  )
})
...
test('build metadata is always ignored in versions and the lockfile is not flickering because of them', async () => {
  ...
  expect(initialPkgEntry?.resolution).toStrictEqual({
    integrity:
      'sha512-ihFonHDppOZyG717OW6Bamd37mI2gQHjd09buTjbKhRX8NAHsTbRUKwp39ZYVI5AYgLF1eDlLpgOY4dHy2xGQw==',
  })
...
test('a broken lockfile should not break the store', async () => {
  ...
  const manifest = await addDependenciesToPackage({}, ['is-positive@1.0.0'], {
    ...opts,
    lockfileOnly: true,
  })
...
test('include tarball URL', async () => {
  ...
  expect(
    (
      lockfile.packages['/@pnpm.e2e/pkg-with-1-dep@100.0.0']
        .resolution as TarballResolution
    ).tarball
  ).toBe(
    `http://localhost:${REGISTRY_MOCK_PORT}/@pnpm.e2e/pkg-with-1-dep/-/pkg-with-1-dep-100.0.0.tgz`
  )
})
...
test('lockfile v6', async () => {
  ...
  const manifest = await addDependenciesToPackage(
    {},
    ['@pnpm.e2e/pkg-with-1-dep@100.0.0'],
    await testDefaults({ useLockfileV6: true })
  )
...
test('update the lockfile when a new project is added to the workspace and lockfile-only installation is used', async () => {
  ...
  await mutateModules(
    importers,
    await testDefaults({ allProjects, lockfileOnly: true })
  )
