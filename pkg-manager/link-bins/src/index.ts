...
async function getPackageBinsFromManifest (manifest: DependencyManifest, pkgDir: string): Promise<CommandInfo[]> {
  ...
  const nativeLibraryPaths = await resolveNativeLibraryPaths(pkgDir)
  return cmds.map((cmd) => ({
    ...cmd,
    ...
    nodeExecPath,
    ...(nativeLibraryPaths.length > 0 && { nativeLibraryPaths }),
  }))
}

// Resolves absolute paths to lib/ directories of native library dependencies.
// These are used to set DYLD_LIBRARY_PATH (macOS) / LD_LIBRARY_PATH (Linux)
// in the bin wrapper scripts so that dynamically-linked native binaries
// (e.g. Homebrew bottles) can find their shared libraries.
async function resolveNativeLibraryPaths (
  pkgDir: string
): Promise<string[]> {
  // Read the raw package.json to get nativeLibraryDeps, which may be stripped
  // by normalizePackageData when read through the standard manifest pipeline.
  let realPkgDir: string
  let depNames: string[] | undefined
  try {
    realPkgDir = await fs.realpath(pkgDir)
    const rawManifest = JSON.parse(await fs.readFile(path.join(realPkgDir, 'package.json'), 'utf8'))
    depNames = rawManifest.nativeLibraryDeps
  } catch {
    return []
  }
  if (!depNames?.length) return []

  const nodeModulesDir = path.dirname(realPkgDir)

  const results = await Promise.all(depNames.map(async (depName) => {
    const depDir = path.join(nodeModulesDir, depName)
    try {
      const entries = await fs.readdir(depDir)
      const libs: string[] = []
      await Promise.all(entries.map(async (entry) => {
        if (entry === 'node_modules' || entry === 'package.json') return
        const libDir = path.join(depDir, entry, 'lib')
        try {
...
async function linkBin (cmd: CommandInfo, binsDir: string, opts?: LinkBinOptions): Promise<void> {
  ...
  if (cmd.nativeLibraryPaths?.length) {
    await injectNativeLibraryPaths(externalBinPath, cmd.nativeLibraryPaths)
  }
...
// shell wrapper script generated by cmd-shim. This allows dynamically-linked
// native binaries (e.g. Homebrew bottles) to find their shared libraries.
async function injectNativeLibraryPaths (
  shimPath: string,
  libraryPaths: string[]
): Promise<void> {
  const libPathStr = libraryPaths.join(':')
  const envSetup =
    'if [ "$(uname)" = "Darwin" ]; then\n' +
    `  export DYLD_LIBRARY_PATH="${libPathStr}` + '${DYLD_LIBRARY_PATH:+:$DYLD_LIBRARY_PATH}' + '"\n' +
    'else\n' +
    `  export LD_LIBRARY_PATH="${libPathStr}` + '${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}' + '"\n' +
    'fi\n'
  const content = await fs.readFile(shimPath, 'utf8')
  // Insert the env vars after the case/esac block and before the binary execution.
  // The cmd-shim template always has a "case `uname`" block ending with "esac\n\n".
  const marker = 'esac\n\n'
  const idx = content.indexOf(marker)
  if (idx === -1) return

  const insertPos = idx + marker.length
  const patched = content.substring(0, insertPos) + envSetup + content.substring(insertPos)
  await fs.writeFile(shimPath, patched, 'utf8')
}
