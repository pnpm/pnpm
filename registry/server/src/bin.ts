...
const numWorkers = parseInt(process.env['PNPM_REGISTRY_WORKERS'] ?? String(Math.max(2, os.availableParallelism() - 1)), 10)

if (cluster.isPrimary) {
  console.log(`pnpm-registry server starting on http://localhost:${port}`)
