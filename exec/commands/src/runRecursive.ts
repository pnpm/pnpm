...
export type RecursiveRunOpts = Pick<
  ...
  | 'workspaceDir'
> &
  Pick<ConfigContext, 'rootProjectManifest'> &
  Required<
    Pick<ConfigContext, 'allProjects' | 'selectedProjectsGraph'> &
      Pick<Config, 'workspaceDir' | 'dir'>
  > &
  Partial<
    Pick<
      Config,
...
export async function runRecursive (
  ...
  let packageChunks: ProjectRootDir[][] = opts.reverse
      ...
      selectedScripts.map(async ({ prefix, scriptName }) =>
        limitRun(async () => {
          ...
          try {
            ...
            const groupEnd =
              Boolean(lifecycleOpts.silent) ||
              getWorkspaceConcurrency(opts.workspaceConcurrency) > 1
                ? undefined
                : groupStart(
                  formatSectionName({
