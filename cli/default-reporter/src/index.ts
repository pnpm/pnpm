...
export function toOutput$ (
  ...
  const packageProvenancePushStream = new Rx.Subject<logs.ProvenanceLog>()
  setTimeout(() => {
    opts.streamParser.on('data', (log: logs.Log) => {
      ...
      case 'pnpm:update-check':
        ...
        break
      case 'pnpm:provenance':
        packageProvenancePushStream.next(log)
        break
  ...
  const log$ = {
    ...
    packageProvenance: Rx.from(packageProvenancePushStream),
  }
