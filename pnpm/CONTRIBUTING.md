...
See [`CODE_STYLE_GUIDE.md`](./CODE_STYLE_GUIDE.md). Formatting and lint-level rules are enforced by the [pinned rustfmt fork](../CONTRIBUTING.md#rust-formatting), `taplo format`, and `cargo clippy`; the style guide covers everything those tools cannot enforce.
...
A group caps how many of these runs start at once, not how much memory one of them takes. That cap is separate: every cargo entry point here runs one job per 4 GiB of the machine's memory, never more than its core count, because a full `--all-targets` build of both workspaces holds well over a gigabyte per rustc. Set `CARGO_BUILD_JOBS` to throttle a run further, or to lift the cap on a machine with memory to spare; the test threads follow it unless `NEXTEST_TEST_THREADS` says otherwise.
