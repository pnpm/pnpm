...
- [x] `TypeScript repo: installing/deps-installer/test/install/lifecycleScripts.ts:175` `installation fails if lifecycle script fails` — ported at unit level as `fail_when_failing_postinstall_is_required` in `crates/deps-preparer/src/build_modules/tests.rs`.
...
- [x] `TypeScript repo: installing/deps-installer/test/install/sideEffects.ts:79` `using side effects cache` covers side-effects read/write — ported at unit level as `using_side_effects_cache_skips_rebuild` (read) and `write_path_populates_side_effects_row` (write) in `crates/deps-preparer/src/build_modules/tests.rs`, plus the end-to-end `side_effects_materialized_on_warm_frozen_reinstall` in `crates/cli/tests/side_effects_cache.rs`.
- [x] `TypeScript repo: installing/deps-installer/test/install/sideEffects.ts:166` `uploading errors do not interrupt installation` verifies cache upload errors do not fail install — ported as `upload_error_does_not_interrupt_install` in `crates/deps-preparer/src/build_modules/tests.rs`.
- [x] `TypeScript repo: installing/deps-installer/test/install/sideEffects.ts:189` `a postinstall script does not modify the original sources added to the store` verifies side effects stay separate from original CAFS files — covered by `write_path_populates_side_effects_row` in `crates/deps-preparer/src/build_modules/tests.rs` (drives a source-modifying postinstall and asserts the originals stay intact).
- [x] `TypeScript repo: installing/deps-installer/test/install/sideEffects.ts:225` `a corrupted side-effects cache is ignored` verifies fallback when cache contents are invalid — ported as `corrupt_side_effects_cache_falls_back_to_rebuild` in `crates/deps-preparer/src/build_modules/tests.rs`.
...
- [x] `TypeScript repo: installing/deps-installer/test/lockfile.ts:1288` `a lockfile with duplicate keys is fixed` covers the ignore-and-regenerate path for a broken wanted lockfile on a regular install — ported as `a_broken_wanted_lockfile_is_ignored_and_regenerated` in `crates/cli/tests/current_lockfile.rs`.
