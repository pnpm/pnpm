...
- [x] `TypeScript repo: installing/deps-installer/test/install/hoist.ts:790` `should add extra node paths to command shims` — ported as `should_not_add_private_hoisted_modules_to_command_shims` in `crates/cli/tests/hoist.rs`; private hoisted modules are excluded from command shims to prevent undeclared dependency resolution.
