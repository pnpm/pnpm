...
// This test currently fails: project-2 will not link if pnpm install is executed before deploy
test.skip('install then deploy with node-linker=hoisted', async () => {
  preparePackages([
    {
      name: 'project-1',
      version: '1.0.0',
  ...
  await execPnpm([...config, 'install'])
  const lockfileBeforeDeploy = readYamlFile('pnpm-lock.yaml')

  await execPnpm([...config, '--filter=project-1', '--prod', 'deploy', 'deploy'])
  const lockfileAfterDeploy = readYamlFile('pnpm-lock.yaml')
  ...
  expect(fs.existsSync('deploy/node_modules/project-3/index.js')).toBeTruthy()
  expect(fs.existsSync('deploy/node_modules/project-3/test.js')).toBeFalsy()
  expect(lockfileAfterDeploy).toStrictEqual(lockfileBeforeDeploy)
})
