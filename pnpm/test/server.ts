...
skipOnWindows('installation using pnpm server', async () => {
  ...
  const serverJson = await retryLoadJsonFile<{
  ...
  await expect(
    execPnpm(['install', 'is-positive@1.0.0'])
...
skipOnWindows('store server: headless installation', async () => {
  ...
  await expect(
    execPnpm(['install', 'is-positive@1.0.0', '--lockfile-only'])
  ).resolves.not.toThrow()
...
skipOnWindows(
  'installation using pnpm server that runs in the background',
  async () => {
...
skipOnWindows('installation using pnpm server via TCP', async () => {
  ...
  expect(serverJson).toBeTruthy()
  expect(
    serverJson.connectionOptions.remotePrefix.indexOf('http://localhost:')
  ).toBe(0) // TCP is used for communication'
...
skipOnWindows('pnpm server uses TCP when port specified', async () => {
  ...
  expect(serverJson).toBeTruthy()
  expect(serverJson.connectionOptions.remotePrefix).toBe(
    'http://localhost:7856'
  ) // TCP with specified port is used for communication
})
...
test.skip('pnpm server fails when trying to set --port for IPC protocol', async () => {
  ...
  expect(
    execPnpmSync(['server', 'start', '--protocol', 'ipc', '--port', '7856'])
      .status
  ).toBe(1)
})
...
skipOnWindows(
  'uploading cache can be disabled without breaking install',
  async () => {
    ...
    await expect(
      execPnpm(['add', '--side-effects-cache', 'diskusage@1.1.3'])
    ).resolves.not.toThrow()
    ...
    const cacheDir = path.join(
      storePath,
      `localhost+${REGISTRY_MOCK_PORT}/diskusage/1.1.3/side_effects/${engine}/package`
    )
...
skipOnWindows(
  'installation using store server started in the background',
  async () => {
    const project = prepare()

    await expect(
      execPnpm(['install', 'is-positive@1.0.0', '--use-store-server'])
    ).resolves.not.toThrow()
    ...
    try {
      ...
      await expect(
        execPnpm(['uninstall', 'is-positive'])
      ).resolves.not.toThrow()
    } finally {
      await expect(execPnpm(['server', 'stop'])).resolves.not.toThrow()
      await expect(pathExists(serverJsonPath)).resolves.toBeFalsy()
...
skipOnWindows(
  ...
  async () => {
    ...
    try {
      ...
      await expect(
        execPnpm(['remove', 'is-positive', '--store-dir', '../store2'])
      ).resolves.not.toThrow()
    } finally {
      await expect(
        execPnpm(['server', 'stop', '--store-dir', '../store2'])
      ).resolves.not.toThrow()
      await expect(pathExists(serverJsonPath)).resolves.toBeFalsy()
    }
  }
...
async function testParallelServerStart(options: {
  ...
  for (let i = 0; i < options.n; i++) {
    ...
    byline(item.childProcess.stderr as Readable).on('data', (line: Buffer) => {
      console.log(
        `${item.childProcess.pid?.toString() ?? ''}: ${line.toString()}`
      )
    })
  ...
  async function stopRemainingServers() {
    ...
    await Promise.all(
      serverProcessList.map(async (item) => {
...
skipOnWindows(
  'parallel server starts against the same store should result in only one server process existing after 10 seconds',
  async () => {
    ...
    await expect(
      testParallelServerStart({
        ...
        onProcessClosed: (
          ...
          if (!weAttemptedKill) {
            console.log(
              `the server process ${serverProcess.pid?.toString() ?? ''} exited`
            )
            expect(1).toBe(1)
          }
        },
...
skipOnWindows(
  ...
  async () => {
    ...
    await expect(
      execPnpm(['install', 'is-positive@1.0.0', '--no-use-store-server'])
    ).resolves.not.toThrow()
...
test.skip('fail if the store server is run by a different version of pnpm', async () => {
  ...
  expect(result.stdout.toString()).toMatch(
    /The store server runs on pnpm v2.0.0. The same pnpm version should be used to connect \(current is/
  )
})
