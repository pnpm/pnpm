...
fn dates_a_cached_verdict_relative_to_now() {
    ...
    assert_eq!(
        cached_verdict(Some("2026-07-25T11:59:59.747Z"), now),
        "verified 253ms ago",
    );
    assert_eq!(
        cached_verdict(Some("2026-07-25T11:58:00.000Z"), now),
        "verified 2m ago",
    );
    assert_eq!(
        cached_verdict(Some("2026-07-23T12:00:00.000Z"), now),
        "verified 2d ago",
    );
}
...
fn falls_back_to_the_timeless_wording_without_a_usable_timestamp() {
    ...
    assert_eq!(
        cached_verdict(Some("last tuesday"), now),
        "previously verified",
    );
}
...
fn clamps_a_future_timestamp_to_zero() {
    let now = at("2026-07-25T12:00:00.000Z");
    assert_eq!(
        cached_verdict(Some("2026-07-25T12:00:05.000Z"), now),
        "verified 0ms ago",
    );
}
...
fn only_a_pnpm_home_install_is_told_to_self_update() {
    assert_eq!(
        update_command(PnpmInstallSource::PnpmHome),
        "pnpm self-update",
    );
    assert_eq!(
        update_command(PnpmInstallSource::Corepack),
        standalone_install_command(),
    );
    assert_eq!(
        update_command(PnpmInstallSource::Elsewhere),
        standalone_install_command(),
    );
}
