...
pub(crate) fn build_publish_document(
    ...
    let (name, version) = validated_publish_identity(manifest, access)?;
    ...
    let versions = versions_object(
        manifest,
        &name,
        &version,
        dist_object(dist_hashes, tarball_url),
    );
...
fn dist_object(dist_hashes: &DistHashes<'_>, tarball_url: String) -> Map<String, Value> {
    ...
    dist.insert(
        "integrity".to_owned(),
        Value::String(dist_hashes.integrity.to_owned()),
    );
    dist.insert(
        "shasum".to_owned(),
        Value::String(dist_hashes.shasum.to_owned()),
    );
...
pub(super) fn clean_version(version: &str) -> Result<String, PublishPackedPkgError> {
    ...
    let mut parsed = trimmed
        ...
        .map_err(|_| PublishPackedPkgError::BadSemver {
...
fn validated_publish_identity(
    manifest: &Value,
    access: Option<Access>,
) -> Result<(String, String), PublishPackedPkgError> {
    ...
    if !is_valid_old_npm_package_name(&name) {
        return Err(PublishPackedPkgError::InvalidPackageName {
            name,
        });
    }
    ...
    if !name.starts_with('@') && access == Some(Access::Restricted) {
        return Err(PublishPackedPkgError::UnscopedRestricted {
            name,
        });
    }
