...
async fn transiently_walked_subtree_versions_do_not_bias_optional_peer_hoists() {
    ...
    assert_eq!(
        direct.get("dep"),
        None,
        "no optional-peer hoist may install dep",
    );
...
async fn non_root_importer_hoists_the_root_importers_peer_provider() {
    ...
    assert_eq!(
        graph_versions_of(&result, "react"),
        ["19.2.0"],
        "one react, the root's",
    );
...
async fn root_dep_named_only_by_its_manifest_still_provides_the_peer() {
    ...
    let resolver = RecordingResolver {
        table: HashMap::from_iter([
            (
                ("aliased".to_string(), TARBALL.to_string()),
                unnamed.clone(),
            ),
...
async fn a_workspace_range_root_dep_is_offered_as_a_peer_provider() {
    ...
    let resolver = RecordingResolver {
        table: HashMap::from_iter([
            (
                ("real-peer".to_string(), WORKSPACE_RANGE.to_string()),
                linked,
            ),
            (
                ("real-peer".to_string(), "^1.0.0".to_string()),
    ...
    assert_eq!(
        result.peers.direct_dependencies_by_importer["."]["real-peer"].as_str(),
        LINK,
    );
    assert_eq!(
...
async fn finalized_packages_include_peer_free_cycles() {
    let announced = announced_finalized_packages(
        ...
        HashMap::from_iter([
            table_entry(
                "ping",
                serde_json::json!({ "dependencies": { "pong": "^1.0.0" } }),
            ),
            table_entry(
                "pong",
                serde_json::json!({ "dependencies": { "ping": "^1.0.0" } }),
            ),
        ]),
    )
    .await;
