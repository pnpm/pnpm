...
/// A parent resolved via a nested hit on a blinded verdict depends on
/// that verdict's truncation without any gate of its own having been
/// consulted — the dependence arrives only through the reused frame.
#[test]
fn a_parent_of_a_reused_blinded_verdict_inherits_its_partial_view() {
    let edge = |alias: &str, pkg_id: &str| crate::resolved_tree::ChildEdge {
        alias: alias.to_string(),
        pkg_id: Arc::from(pkg_id),
        optional: false,
    ...
    let graph_keys =
        |entries: &[(&str, usize, &str)]| -> Vec<String> {
            let mut tree = peer_cycle_fixture(
                entries,
                PeerCycleShape {
                    ring_len: 4,
            ...
            // `wrap` sits between `entry01` and the ring, and also hangs
            // off `ring01`, where its `ring02` child reuses the lap's
            // blinded verdict.
            tree.packages.insert(Arc::from("wrap@1.0.0"), package("wrap", "1.0.0", &[], false));
            tree.children_by_id
                .insert(Arc::from("wrap@1.0.0"), Arc::new(vec![edge("ring", "ring02@1.0.0")]));
            let mut ring01_children = (*tree.children_by_id["ring01@1.0.0"]).clone();
            ring01_children.push(edge("wp", "wrap@1.0.0"));
            tree.children_by_id.insert(Arc::from("ring01@1.0.0"), Arc::new(ring01_children));
            let entry01_children: Vec<crate::resolved_tree::ChildEdge> =
                tree.children_by_id["entry01@1.0.0"]
                    .iter()
                    .map(|child| {
                        if child.alias == "ring" {
                            edge("ring", "wrap@1.0.0")
                        } else {
                            child.clone()
                        }
                    })
                    .collect();
            tree.children_by_id.insert(Arc::from("entry01@1.0.0"), Arc::new(entry01_children));
            let result = resolve_peers(&mut tree, ResolvePeersOptions::default());
    ...
    // Full-set equality is not asserted: `wrap` sits inside the ring's
    // closure, so an intact walk re-enters it, and whether that
    // second-lap occurrence finds a context-compatible item from the
    // other importer is the reverse-direction residual this guard
    // leaves in place.
    let first_order = graph_keys(&[("entry00", 0, "1.0.0"), ("entry01", 0, "2.0.0")]);
    let second_order = graph_keys(&[("entry01", 0, "2.0.0"), ("entry00", 0, "1.0.0")]);
    for (name, keys) in [("first", &first_order), ("second", &second_order)] {
        for expected in ["wrap@1.0.0(p@1.0.0)(w@2.0.0)", "ring02@1.0.0(p@1.0.0)(w@2.0.0)"] {
            assert!(
                keys.iter().any(|key| key == expected),
                "entry01's wrap sees the ring intact and must resolve its own w:                  {expected} missing from the {name} order; got {keys:#?}",
            );
        }
    }
}
