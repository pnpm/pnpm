use super::{
    AppliedPatchesLog, Envelope, IgnoredScriptsLog, LogEvent, LogLevel, Pipe, Value, assert_eq,
};
...
#[test]
fn applied_patches_event_wire_shape() {
    let event = LogEvent::AppliedPatches(AppliedPatchesLog {
        level: LogLevel::Debug,
        package_names: vec!["bar@2.0.0".to_string(), "foo@1.0.0".to_string()],
    });
    let envelope = Envelope { time: 1_700_000_000_000, hostname: "host", pid: 4242, event: &event };
    let json: Value = envelope
        .pipe_ref(serde_json::to_string)
        ...
        .expect("parse JSON");
    dbg!(&json);
    assert_eq!(json["name"], "pnpm:applied-patches");
    assert_eq!(json["level"], "debug");
    assert_eq!(json["packageNames"], serde_json::json!(["bar@2.0.0", "foo@1.0.0"]));
}
