...
fn run_gives_build_candidates_a_private_writable_projection() {
    ...
    EVENTS.lock().unwrap().clear();
    assert_private_writable_projection::<RecordingReporter>(
        "build-me",
...
fn run_gives_patched_packages_a_private_writable_projection() {
    assert_private_writable_projection::<pacquet_reporter::SilentReporter>(
        "patch-me",
        r#"{"name":"patch-me","version":"1.0.0"}"#,
        &SnapshotEntry { patched: Some(true), ..SnapshotEntry::default() },
        b"patched",
    );
}

fn assert_private_writable_projection<ReporterT: Reporter>(
    package_name: &str,
    manifest: &str,
    ...
    std::fs::write(&package_json, manifest).expect("write store manifest");
    #[cfg(unix)]
    {
    ...
    let package_id = format!("{package_name}@1.0.0");
    let package_key: PackageKey = package_id.parse().expect("valid snapshot key");
    let layout = crate::VirtualStoreLayout::legacy(
    ...
    .run::<ReporterT>()
    .expect("writable projection import should succeed");

    let target =
        layout.slot_dir(&package_key).join("node_modules").join(package_name).join("package.json");
    std::fs::write(&target, mutation).expect("projection should be writable");
    assert_eq!(
        std::fs::read(&package_json).unwrap(),
        manifest.as_bytes(),
        "writing the projection must not change the store manifest",
    );
