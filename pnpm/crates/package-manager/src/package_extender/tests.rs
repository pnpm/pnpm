...
fn extends_manifest_with_missing_dependency() {
    ...
    extensions.insert(
        ...
        extension(&[("@pnpm.e2e/bar", "100.1.0")]),
    );
...
fn extends_peer_dependencies_and_meta() {
    ...
    assert_eq!(
        manifest.get("peerDependencies").unwrap(),
        &json!({ "react": ">=16" }),
    );
    assert_eq!(
...
fn apply_to_arc_returns_same_arc_when_no_match() {
    ...
    assert_eq!(
        before_ptr,
        Arc::as_ptr(&after),
        "unchanged manifests must keep the same Arc",
    );
}
...
fn apply_to_arc_clones_on_match() {
    ...
    assert_ne!(
        before_ptr,
        Arc::as_ptr(&after),
        "matched manifests must be cloned",
    );
