...
use crate::resolution_policy::{PickPolicy, offline_store_index, pick_package_context};
...
use pnpm_resolving_npm_resolver::{
    InMemoryPackageMetaCache, OfflineStoreAvailability, PackumentFetchLocker, PickPackageError,
    PickPackageOptions, RegistryPackageSpec, pick_package, pick_registry_for_package,
};
use std::{
    collections::HashMap,
    sync::{Arc, OnceLock},
...
pub(crate) struct LatestPicker<'a> {
    ...
    /// Opened on the first offline pick instead of in [`Self::new`] so an
    /// online picker pays nothing; one handle per picker covers every
    /// package it resolves.
    offline_store_index: OnceLock<Option<OfflineStoreAvailability>>,
}
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            registries: config
                ...
                .into_iter()
                .collect(),
            offline_store_index: OnceLock::new(),
        }
    }

    fn offline_store_index(&self) -> Option<&OfflineStoreAvailability> {
        self.offline_store_index.get_or_init(|| offline_store_index(self.config)).as_ref()
    }
