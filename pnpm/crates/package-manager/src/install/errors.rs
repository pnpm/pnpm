...
pub enum InstallError {
    ...
    /// The pnpmfile's `readPackage` hook returned a dependency range that is
    /// not a string, raised with `ERR_PNPM_BAD_READ_PACKAGE_HOOK_RESULT`.
    ...
    fn from(err: pnpm_hooks::HookError) -> Self {
        match err {
            pnpm_hooks::HookError::BadReadPackageResult { .. } => {
                InstallError::BadReadPackageHookResult(err)
            }
            _ => InstallError::ReadPackageHook(err),
        }
    }
}
