...
use super::{super::map_frozen_lockfile_error, InstallError, Reporter, dispatch::Settled};
...
async fn compute_frozen_skip_set<Reporter: self::Reporter + 'static>(
    settled: &Settled<'_, '_>,
    lockfile: &pnpm_lockfile::Lockfile,
    ...
    let seed = read_skipped_seed(&settled.install.context.config.modules_dir);
...
pub(super) async fn verify_frozen_tarballs<Reporter: self::Reporter + 'static>(
    settled: Settled<'_, '_>,
) -> Result<(), InstallError> {
    let lockfile =
    ...
    let skipped = compute_frozen_skip_set::<Reporter>(&settled, lockfile, &importer_ids).await?;
