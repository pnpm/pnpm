...
pub(crate) use recorded::{current_settings, current_settings_with_catalogs};
mod recorded;
...
impl SettingsComparison<'_> {
    ...
    fn linking_drift(&self) -> Option<&'static str> {
        let recorded = self.recorded;
        let live = self.live;
        if let Some(drift) = self.global_virtual_store_drift() {
            return Some(drift);
        }
        return_drift_if!(
            self,
    ...
    fn global_virtual_store_drift(&self) -> Option<&'static str> {
        let recorded = self.recorded;
        let live = self.live;
        ...
        return_drift_if!(
            ...
            enable_global_virtual_store_drift
        );
        None
    }
    ...
    fn workspace_policy_drift(&self) -> Option<&'static str> {
        ...
        self.trust_policy_drift()
    }
    fn trust_policy_drift(&self) -> Option<&'static str> {
        let recorded = self.recorded;
        let live = self.live;
        return_drift_if!(
            self,
            "trustPolicy",
            recorded.trust_policy != live.trust_policy
        );
        return_drift_if!(
            self,
            "trustPolicyExclude",
