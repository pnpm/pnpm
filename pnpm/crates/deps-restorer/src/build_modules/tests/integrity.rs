...
fn explicit_allow_by_dep_path_allows_untrusted_package_identity() {
    ...
    assert_eq!(
        policy.check("foo@git+https://github.com/attacker/foo.git#abc123"),
        None,
    );
}
#[test]
fn explicit_allow_by_git_repo_allows_untrusted_package_identity() {
    ...
    assert_eq!(
        policy.check("foo@git+ssh://git@example.com/org/foo.git#abc123"),
        Some(true),
    );
    assert_eq!(
        policy.check("foo@git+ssh://git@example.com/org/foo.git"),
        Some(true),
    );
    assert_eq!(
    ...
    assert_eq!(
        policy.check("foo@git+ssh://git@example.com/other/foo.git#abc123"),
        None,
    );
    assert_eq!(policy.check("foo@1.0.0"), None);
    assert_eq!(
        policy.check("bar@git+ssh://git@example.com/org/bar.git#abc123"),
        Some(false),
    );
}
#[test]
