...
pub fn shim_link_options(config: &Config, node_linker: NodeLinker) -> LinkBinsOptions {
    ...
    LinkBinsOptions {
        ...
        // The directory every project's modules dir is anchored under, so a
        // shim and its target both live inside it. `modules_dir.parent()`
        // is that directory only while `modulesDir` is a single component.
        relocatable_root: config.modules_dir_anchor().map(Path::to_path_buf),
        project_modules_dir_name: (config.extend_node_path
            && config.modules_dir_relative() != Path::new("node_modules"))
        .then(|| config.modules_dir_name().to_owned()),
        installed_modules_dir: (config.modules_dir_relative() != Path::new("node_modules")).then(
