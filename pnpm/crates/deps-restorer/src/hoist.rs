...
pub use direct_dependencies::build_direct_deps_by_importer;
...
pub fn get_hoisted_dependencies<'a>(input: &'a HoistInputs<'a>) -> Option<HoistResult> {
    ...
    let mut entries: Vec<BfsEntry<'a>> = vec![BfsEntry {
        depth: -1,
        sort_key: String::new(),
...
fn direct_nodes<'a>(
    ...
    for importer_deps in input.direct_deps_by_importer.values() {
        for node_id in importer_deps.values() {
            let Some((graph_key, _)) = input.graph.get_key_value(node_id) else {
                continue;
            };
...
impl<'a> HoistPass<'a> {
    ...
    fn hoist_workspace_packages(&mut self) {
        for (name, dir) in self.input.hoisted_workspace_packages.into_iter().flatten() {
            let Some(hoist_kind) = self.hoist_kind(name) else {
                continue;
            };
    ...
    fn place_child(&mut self, alias: &str, child_node_id: &PackageKey) {
        let Some(hoist_kind) = self.hoist_kind(alias) else {
            return;
        };
        ...
        let Some(node) = self.input.graph.get(child_node_id) else {
            return;
        };
