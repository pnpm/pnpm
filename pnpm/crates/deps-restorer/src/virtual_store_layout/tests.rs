...
fn layout_cache_fingerprint_tracks_every_derivation_input() {
    ...
    let fingerprint = |snapshots: &HashMap<PackageKey, SnapshotEntry>,
                       ...
                       dir: Option<&Path>| {
        super::GvsHasher::new(snapshots, Some(packages), engine, policy, dir).fingerprint(snapshots)
    };
    let baseline =
...
fn layout_cache_load_rejects_a_map_it_cannot_vouch_for() {
    ...
    let file = |fingerprint| super::gvs_layout_cache::CacheFile {
    ...
    assert_eq!(
        super::gvs_layout_cache::load(file("another-fingerprint"), expected),
        None,
