...
fn try_read_returns_manifest_when_present() {
    ...
    let result = try_read_project_manifest(tmp.path(), ManifestFormat::default()).unwrap().unwrap();
...
fn try_read_returns_none_when_missing() {
    let tmp = TempDir::new().unwrap();
    assert!(try_read_project_manifest(tmp.path(), ManifestFormat::default()).unwrap().is_none());
}
...
fn safe_read_returns_none_when_missing() {
    let tmp = TempDir::new().unwrap();
    assert!(
        safe_read_project_manifest_only(tmp.path(), ManifestFormat::default()).unwrap().is_none(),
    );
}
...
fn strict_read_errors_when_missing() {
    let tmp = TempDir::new().unwrap();
    match read_project_manifest_only(tmp.path(), ManifestFormat::default()) {
...
fn try_read_prefers_json5_over_yaml() {
    ...
    write_yaml_manifest(tmp.path(), "name: yaml\n");
    let (name, mut manifest) =
        try_read_project_manifest(tmp.path(), ManifestFormat::default()).unwrap().unwrap();
    ...
    let selected = crate::project_manifest_path(tmp.path(), ManifestFormat::default());
    assert_eq!(selected, tmp.path().join("package.json5"));
}

#[test]
fn try_read_prefers_the_preferred_format() {
    let tmp = TempDir::new().unwrap();
    write_manifest(tmp.path(), r#"{"name": "json"}"#);
    fs::write(tmp.path().join("package.json5"), "{ name: 'json5' }").unwrap();
    write_yaml_manifest(tmp.path(), "name: yaml\n");
    let (name, manifest) =
        try_read_project_manifest(tmp.path(), ManifestFormat::Json5).unwrap().unwrap();
    assert_eq!(name, "package.json5");
    assert_eq!(manifest.value()["name"], "json5");
    assert_eq!(crate::read_project_name(tmp.path(), ManifestFormat::Yaml).as_deref(), Some("yaml"));
}

#[test]
fn try_read_falls_back_when_the_preferred_format_is_missing() {
    let tmp = TempDir::new().unwrap();
    write_manifest(tmp.path(), r#"{"name": "json"}"#);
    write_yaml_manifest(tmp.path(), "name: yaml\n");
    let (name, _) = try_read_project_manifest(tmp.path(), ManifestFormat::Json5).unwrap().unwrap();
...
fn try_read_ignores_invalid_unselected_json5() {
    ...
    assert_eq!(
        crate::project_manifest_path(tmp.path(), ManifestFormat::default()),
...
fn try_read_does_not_fall_back_from_invalid_preferred_json() {
    ...
    fs::write(tmp.path().join("package.json5"), "{ name: 'json5' }").unwrap();
    write_yaml_manifest(tmp.path(), "name: yaml\n");
    let error = try_read_project_manifest(tmp.path(), ManifestFormat::default()).err().unwrap();
