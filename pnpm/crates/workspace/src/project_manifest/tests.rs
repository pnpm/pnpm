use super::{
    ManifestFormat, ReadProjectManifestError, ReadProjectManifestOnlyError,
    read_exact_project_manifest, read_project_manifest_only, safe_read_project_manifest_only,
...
fn try_read_returns_manifest_when_present() {
    ...
    let result = try_read_project_manifest(tmp.path(), None).unwrap().unwrap();
...
fn try_read_returns_none_when_missing() {
    let tmp = TempDir::new().unwrap();
    assert!(try_read_project_manifest(tmp.path(), None).unwrap().is_none());
}

#[test]
fn safe_read_returns_none_when_missing() {
    let tmp = TempDir::new().unwrap();
    assert!(safe_read_project_manifest_only(tmp.path(), None).unwrap().is_none());
}

#[test]
fn strict_read_errors_when_missing() {
    let tmp = TempDir::new().unwrap();
    match read_project_manifest_only(tmp.path(), None) {
...
fn try_read_prefers_json5_over_yaml() {
    ...
    write_yaml_manifest(tmp.path(), "name: yaml\n");
    let (name, mut manifest) = try_read_project_manifest(tmp.path(), None).unwrap().unwrap();
    ...
    let selected = crate::project_manifest_path(tmp.path(), None);
...
fn try_read_ignores_invalid_unselected_json5() {
    ...
    write_yaml_manifest(tmp.path(), "name: yaml\n");
    let (name, manifest) = try_read_project_manifest(tmp.path(), None).unwrap().unwrap();
    assert_eq!(name, "package.json");
    assert_eq!(manifest.value()["name"], "json");
    assert_eq!(crate::project_manifest_path(tmp.path(), None), tmp.path().join("package.json"));
}
...
fn try_read_does_not_fall_back_from_invalid_preferred_json() {
    ...
    fs::write(tmp.path().join("package.json5"), "{ name: 'json5' }").unwrap();
    write_yaml_manifest(tmp.path(), "name: yaml\n");
    let error = try_read_project_manifest(tmp.path(), None).err().unwrap();
...
#[test]
fn preferred_format_wins_over_default_precedence() {
    let tmp = TempDir::new().unwrap();
    write_manifest(tmp.path(), r#"{"name": "json"}"#);
    fs::write(tmp.path().join("package.json5"), "{ name: 'json5' }").unwrap();

    let (name, manifest) =
        try_read_project_manifest(tmp.path(), Some(ManifestFormat::Json5)).unwrap().unwrap();
...
#[test]
fn preferred_format_falls_back_when_absent() {
    let tmp = TempDir::new().unwrap();
    write_manifest(tmp.path(), r#"{"name": "json"}"#);
    write_yaml_manifest(tmp.path(), "name: yaml\n");
    ...
    assert_eq!(name, "package.json", "a missing preferred format must not hide the project");
    assert_eq!(manifest.value()["name"], "json");
}

#[test]
fn preferred_format_redirects_the_write_target() {
    let tmp = TempDir::new().unwrap();
    write_manifest(tmp.path(), r#"{"name": "json"}"#);
    fs::write(tmp.path().join("package.json5"), "{ name: 'json5' }").unwrap();

    let selected = crate::project_manifest_path(tmp.path(), Some(ManifestFormat::Json5));
...
#[test]
fn preferred_format_names_a_new_project_manifest() {
    let tmp = TempDir::new().unwrap();

    let selected = crate::project_manifest_path(tmp.path(), Some(ManifestFormat::Yaml));

    assert_eq!(selected, tmp.path().join("package.yaml"));
}

#[test]
fn default_precedence_is_unchanged_without_a_preference() {
    let tmp = TempDir::new().unwrap();
    write_manifest(tmp.path(), r#"{"name": "json"}"#);
    fs::write(tmp.path().join("package.json5"), "{ name: 'json5' }").unwrap();
    write_yaml_manifest(tmp.path(), "name: yaml\n");

    let (name, _) = try_read_project_manifest(tmp.path(), None).unwrap().unwrap();
