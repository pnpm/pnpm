...
/// `package.yaml` / `package.yml` entry into a single serialized `package/package.json`,
/// and reports all of them as `package.json` in the contents listing.
/// Mirrors the `/^package\/package\.(?:json|json5|yaml|yml)$/` test pnpm
