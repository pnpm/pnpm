...
fn join_checked_rejects_absolute_paths() {
    assert_invalid_input(join_checked(PathBuf::from("/root"), "/etc/passwd").unwrap_err());
}

#[test]
fn join_checked_rejects_parent_dir() {
    assert_invalid_input(join_checked(PathBuf::from("/root"), "../escape").unwrap_err());
    // Even a `..` deep in the path must be refused — otherwise
    // `a/../../escape` would slip through.
    assert_invalid_input(join_checked(PathBuf::from("/root"), "a/../escape").unwrap_err());
}
