...
/// and the host's git configuration because a user-selected ssh command is
/// kept.
#[test]
fn git_runs_with_terminal_and_ssh_prompts_disabled() {
    ...
    let stdout = exec_git_with(&shim, &["fetch", "origin"], None).unwrap();
    ...
    let vars = pnpm_git_utils::non_interactive_git_env(
