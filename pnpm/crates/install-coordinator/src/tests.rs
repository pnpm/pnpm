...
async fn preparation_failure_settles_writers_before_restoring_metadata() {
    ...
    let error = InstallPlan::new(root.path().to_path_buf())
        ...
        .with_task(InstallTask::in_place(vec![metadata.clone()], async {
            bail!("prepare failed")
        }))
    ...
    assert!(
        !visible.exists(),
        "projection must not publish: {visible:?}",
    );
    assert!(
        !resources.exists(),
        "unpublished resources must be cleaned: {resources:?}",
    );
}
...
async fn publishes_only_after_preparation_and_keeps_resources() {
    ...
    assert!(
        resources.is_dir(),
        "published resources must survive: {resources:?}",
    );
}
...
async fn publication_failure_restores_all_attempted_projections_and_metadata() {
    ...
    let error = InstallPlan::new(root.path().to_path_buf())
        ...
        .with_task(InstallTask::new(Vec::new(), async {
            Ok(vec![second, third])
        }))
        .run()
        .await
    ...
    assert_eq!(
        fs::read_to_string(first_visible).unwrap(),
        "old environment",
    );
    assert!(
        !second_visible.exists(),
        "failed publisher must roll back: {second_visible:?}",
    );
    assert!(
        !third_visible.exists(),
        "later publishers must not run: {third_visible:?}",
    );
    for path in resources {
        assert!(
            !path.exists(),
            "rolled-back resources must be cleaned: {path:?}",
        );
    }
}
...
async fn rollback_failure_keeps_resources_and_still_restores_other_participants() {
    ...
    assert!(
        !first_visible.exists(),
        "other rollback must still run: {first_visible:?}",
    );
    assert!(
        !metadata.exists(),
        "metadata rollback must still run: {metadata:?}",
    );
    assert!(
        resources.is_dir(),
        "failed rollback must retain resources: {resources:?}",
    );
}
