...
///
/// - Build output contains symlinks → remove the matching cache entry so
///   the build script recreates them on the next install.
...
pub fn upload(
    ...
    if added.has_symlinks {
        writer.queue_side_effects_invalidation(
