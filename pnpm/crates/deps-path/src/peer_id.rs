...
/// One peer entry fed to [`fn@crate::create_peer_dep_graph_hash`]:
/// either `{ name, version }` or a pre-rendered depPath.
///
...
impl PeerId {
    ...
    /// leading `/` stripped (the `/`-prefix fast path for the absolute /
    /// relative depPath distinction).
    #[must_use]
