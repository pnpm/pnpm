...
/// TS: `current lockfile contains only installed dependencies when adding
/// a new importer to workspace with shared lockfile`
/// (`deps-installer/test/install/multipleImporters.ts:730`). The wanted
/// lockfile covers the whole workspace; the current one covers only what
/// reached disk, so a project that was never materialized contributes no
/// packages to it.
#[test]
fn current_lockfile_covers_only_the_importers_that_were_materialized() {
    ...
    let wanted = fixture.wanted();
    assert!(has_snapshot(&wanted, PARENT, "100.0.0"));
    assert!(has_snapshot(&wanted, HELLO, "1.0.0"));

    fixture.run(["--filter", "installed", "install"]);

    let current = fixture.current();
    assert!(has_link(&installed, HELLO));
    assert!(has_snapshot(&current, HELLO, "1.0.0"));
    assert!(
        !has_snapshot(&current, PARENT, "100.0.0"),
        "a project that was only resolved must not appear in the current lockfile",
    );
    assert!(!has_snapshot(&current, DEP, "100.1.0"));
}
