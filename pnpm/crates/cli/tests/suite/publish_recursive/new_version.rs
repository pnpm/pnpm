...
/// A dependency installed before the bump sits in `node_modules` with the old
/// version. The packed manifest must still reference the bumped workspace
/// version, not the stale installed copy.
#[test]
fn recursive_new_version_packs_workspace_deps_at_the_bumped_version() {
    let CommandTempCwd { pacquet, root, workspace, .. } = CommandTempCwd::init();
    let mut server = mockito::Server::new();
    write_workspace(
        &workspace,
    ...
    write_registry_npmrc(&workspace, &format!("{}/", server.url()));
    let installed = workspace.join("project-2/node_modules/project-1");
    fs::create_dir_all(&installed).expect("create the installed copy");
    fs::write(installed.join("package.json"), r#"{"name":"project-1","version":"1.0.0"}"#)
    ...
    let put_2 = server
        ...
        .match_body(Matcher::PartialJsonString(
            r#"{"versions":{"3.0.0":{"dependencies":{"project-1":"3.0.0"}}}}"#.to_owned(),
        ))
        .with_status(200)
        .with_body("{}")
        .expect(1)
