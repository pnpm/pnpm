...
/// Windows-skipped: the second worktree recompiles instead of restoring the
/// shared Cargo build state. The run there places the cache under the 8.3
/// short form of the temporary directory (`RUNNER~1`) while the worktree
/// carries the long one, which is the first thing to check when picking
/// this up.
#[test]
#[cfg_attr(
    target_os = "windows",
    ignore = "the shared Cargo build state is not restored across worktrees"
)]
