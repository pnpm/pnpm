...
fn audit_signatures_reports_verified_packages() {
    ...
    assert!(
        out.contains("1 package has a verified registry signature"),
        "{out}",
    );
    keys_mock.assert();
...
fn audit_signatures_flags_missing_signature() {
    ...
    assert_eq!(
        output.status.code(),
        Some(1),
        "missing signatures should exit 1",
    );
    let out = stdout(&output);
...
fn audit_signatures_flags_invalid_signature() {
    ...
    assert_eq!(
        output.status.code(),
        Some(1),
        "invalid signatures should exit 1",
    );
    let out = stdout(&output);
...
fn audit_signatures_fails_when_keys_endpoint_errors() {
    ...
    assert!(
        stderr.contains("ERR_PNPM_AUDIT_SIGNATURE_KEYS_FETCH_FAIL"),
        "stderr:\n{stderr}",
    );
    ...
    assert!(
        stderr.contains(r"boom \u{1b}[31m\u{a}"),
        "stderr:\n{stderr}",
    );
...
",
    ...
    assert_failure(&output);
    assert!(
        stderr(&output).contains("ERR_PNPM_AUDIT_NO_PACKAGES"),
        "stderr:\n{}",
        stderr(&output),
    );
}
