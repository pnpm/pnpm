...
fn launched_as_pnpx_injects_the_dlx_subcommand() {
    ...
    assert!(
        via_pnpx.status.success(),
        "`pnpx --help` exited with a failure status",
    );
    assert!(
        via_dlx.status.success(),
        "`dlx --help` (the control) exited with a failure status",
    );
    ...
    assert_eq!(
        pnpx_help, dlx_help,
        "`pnpx --help` should equal `dlx --help` (dlx was injected)",
    );
}
