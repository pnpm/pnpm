...
/// does not match the inner one` (`deps-installer/test/lockfile.ts:368`).
/// The wanted lockfile is authoritative: editing a subdependency's
/// version there must re-materialize it even though the current lockfile
/// still describes the old graph.
#[test]
fn a_wanted_lockfile_edit_updates_subdeps_against_the_current_one() {
    let CommandTempCwd { pacquet, root, workspace, npmrc_info, .. } =
        CommandTempCwd::init().add_mocked_registry();
    let AddMockedRegistry { mock_instance, .. } = npmrc_info;
    ...
    let dep_manifest = workspace.join(
        "node_modules/.pnpm/@pnpm.e2e+pkg-with-1-dep@100.0.0\
         /node_modules/@pnpm.e2e/dep-of-pkg-with-1-dep/package.json",
    );
    assert!(dep_manifest.exists(), "the subdependency must be materialized");
    ...
    fs::write(
        &lockfile_path,
        lockfile
            .replace("dep-of-pkg-with-1-dep@100.0.0", "dep-of-pkg-with-1-dep@100.1.0")
            .replace("'100.0.0'\n      dep", "'100.1.0'\n      dep"),
    )
    .expect("rewrite pnpm-lock.yaml");

    rerun(&workspace).with_arg("install").assert().success();

    let installed: serde_json::Value =
        serde_json::from_str(&fs::read_to_string(&dep_manifest).expect("read the subdep manifest"))
            .expect("parse the subdep manifest");
    assert_eq!(installed["version"], "100.1.0");

    drop((root, mock_instance));
}

/// TS: `repeat install with no inner lockfile should not rewrite
/// packages in node_modules` (`deps-installer/test/lockfile.ts:547`).
/// A missing current lockfile makes the install re-verify rather than
/// tear down: everything already on disk stays linked.
#[test]
fn a_repeat_install_without_a_current_lockfile_keeps_node_modules() {
    let CommandTempCwd { pacquet, root, workspace, npmrc_info, .. } =
        CommandTempCwd::init().add_mocked_registry();
    let AddMockedRegistry { mock_instance, .. } = npmrc_info;
    ...
    pacquet.with_arg("install").assert().success();
    fs::remove_file(workspace.join(CURRENT_LOCKFILE)).expect("remove the current lockfile");

    rerun(&workspace).with_arg("install").assert().success();

    assert!(workspace.join("node_modules/@pnpm.e2e/pkg-with-1-dep/package.json").exists());
    assert!(
        workspace
            .join(
                "node_modules/.pnpm/@pnpm.e2e+pkg-with-1-dep@100.0.0\
            ...
            .exists(),
        "the subdependency's virtual-store slot must survive too",
    );
