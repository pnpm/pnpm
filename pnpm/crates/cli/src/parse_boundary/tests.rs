...
fn a_command_taking_a_foreign_command_line_forwards_everything_after_its_first_positional() {
    for (argv, forwarded) in [
        (
            ["run", "show", "--config.foo=bar"].as_slice(),
            ["--config.foo=bar"].as_slice(),
        ),
        (&["run", "show", "--silent"], &["--silent"]),
        (
            &["run", "show", "-s", "--if-present"],
            &["-s", "--if-present"],
        ),
        (
            &["exec", "node", "show.js", "--config.foo=bar"],
            &["show.js", "--config.foo=bar"],
        ),
...
fn a_value_taking_option_does_not_move_the_boundary_onto_its_value() {
    for (argv, forwarded) in [
        ...
        (
            &["dlx", "--package", "cowsay", "cowsay", "--silent"],
            &["--silent"],
        ),
        (
            &["dlx", "--allow-build", "esbuild", "cowsay", "--moo"],
            &["--moo"],
        ),
        ...
        (
            &["--resume-from", "pkg", "-r", "run", "build", "--silent"],
            &["--silent"],
        ),
    ] {
        let (_, actual) = split(argv);
...
fn with_forwards_only_for_a_version_that_execs_a_child() {
    ...
    assert!(
        before.is_empty(),
        "an override ahead of the script name is pnpm's: {before:?}",
    );
    ...
    for (argv, forwarded) in [
        ...
        (
            &["with", "current", "run", "show", "--silent"],
            &["--silent"],
        ),
        (
            &["with", "current", "recursive", "run", "show", "--silent"],
            &["--silent"],
        ),
