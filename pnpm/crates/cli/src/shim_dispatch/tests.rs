...
fn malformed_legacy_shim_argv_fails_instead_of_running_the_cli() {
    ...
    assert_eq!(
        try_dispatch(&strings(&["pnpm", "--shim", "tool", "/g/bin/tool"])),
        Some(1),
    );
    assert_eq!(
...
fn trust_decisions_round_trip_last_record_wins() {
    ...
    assert_eq!(
        read_trust_decision(&trust_file, "/a", "candidate-a"),
        Some(true),
    );
    assert_eq!(
        read_trust_decision(&trust_file, "/b", "candidate-b"),
        Some(false),
    );
...
fn windows_cmd_shim_candidate_matches_the_global_provider() {
    ...
    assert_eq!(
        local_bin_identity(bin, "tool").unwrap().provider.name,
        "tool",
    );
...
fn local_bin_fingerprint_binds_the_executed_flavor() {
    ...
    fs::write(
        modules.join("tool").join("package.json"),
    ...
    assert_ne!(
        before, after,
        "replacing the executed flavor must invalidate the approval",
    );
}
...
fn shim_target_trailer_round_trips() {
    ...
    assert_eq!(
        read_shim_target_from_content("#!/bin/sh\nexec something\n"),
        None,
    );
}
...
fn local_bin_identity_resolves_symlinks_and_trailers() {
    ...
    assert_eq!(
        local_bin_identity(&linked, "linked").unwrap().provider.name,
        "tool",
    );
    ...
    .unwrap();
    assert_eq!(
        local_bin_identity(&scripted, "scripted").unwrap().provider.name,
        "tool",
    );
...
fn provider_identity_supports_workspace_packages() {
    ...
    fs::write(
        workspace_package.join("package.json"),
        r#"{"name":"tool","version":"1.0.0"}"#,
    )
    .unwrap();
    ...
    assert_eq!(
        provider.package_dir,
        fs::canonicalize(workspace_package).unwrap(),
    );
}
...
fn runtime_promptless_policy_matrix() {
    ...
    assert!(!runtime_runs_promptless(
        ShimPolicy::Auto,
        "node",
        "24.0.0-rc.4"
    ));
