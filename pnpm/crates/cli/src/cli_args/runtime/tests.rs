...
fn set_request_fails_for_unknown_subcommand() {
    ...
    assert_eq!(
        err,
        RuntimeError::UnknownSubcommand {
            subcommand: "foo".to_string()
        },
    );
}
...
fn set_request_rejects_unsupported_runtime_names() {
    ...
    for name in ["python", "node,is-positive", "./evil", "file:./evil"] {
        ...
        assert_eq!(
            err,
            RuntimeError::InvalidRuntimeName {
                name: name.to_string()
            },
        );
    }
...
fn set_request_rejects_a_comma_in_the_version() {
    ...
    assert_eq!(
        err,
        RuntimeError::InvalidRuntimeVersion {
            version: "22,is-positive".to_string()
        },
    );
}
...
async fn run_hands_the_set_request_off_to_add_package() {
    ...
    assert!(
        ...
        "expected the offline Node.js resolver error, got: {err}",
    );
}
...
fn local_runtime_suggests_the_explicit_shim_command() {
    ...
    let config = Config {
        global_bin: Some(dir.path().to_path_buf()),
        ..Config::default()
    };
...
fn local_runtime_does_not_suggest_an_installed_project_aware_shim() {
    ...
    install_native_shim_from(
        ...
        bin_dir,
        "node",
        &ShimTarget::Virtual("node".to_string()),
    )
    .unwrap();
    let config = Config {
        global_bin: Some(bin_dir.to_path_buf()),
        ..Config::default()
    };

    assert_eq!(
        runtime_shim_hint(&config, "node", &GlobalShims::default()),
        None,
    );
}
