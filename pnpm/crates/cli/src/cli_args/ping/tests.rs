...
fn redact_and_sanitize_strips_credentials_and_control_chars() {
    ...
    assert!(
        !clean.contains("user:pass"),
        "credentials must be redacted: {clean:?}",
    );
    assert!(
        !clean.contains('\u{1b}'),
        "escape sequences must be stripped: {clean:?}",
    );
    assert!(
        !clean.contains('\n'),
    ...
    assert!(
        clean.contains("https://host/-/ping"),
        "non-sensitive text is kept: {clean:?}",
    );
}
