...
impl RunArgs {
    ...
    fn run_inner(
        ...
        let Some((script_name, args)) = self.script.split_first() else {
            let manifest = read_project_manifest_only(dir, config.preferred_manifest_format)
                .map_err(RunError::Manifest)?;
            println!("{}", render_project_commands(manifest.value(), None));
            return Ok(());
        };
        let manifest = match read_project_manifest_only(dir, config.preferred_manifest_format) {
