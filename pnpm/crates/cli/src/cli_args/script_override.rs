...
pub(crate) fn resolve(
    ...
    if declares_script(
        safe_read_project_manifest_only(dir, Some(config.preferred_manifest_format))?.as_ref(),
        command_name,
    ) {
    ...
    if let Some(workspace_dir) = config.workspace_dir.as_deref()
        ...
        && declares_script(
            safe_read_project_manifest_only(workspace_dir, Some(config.preferred_manifest_format))?
                .as_ref(),
            command_name,
        )
    {
