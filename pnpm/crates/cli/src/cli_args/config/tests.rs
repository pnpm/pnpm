...
// --- project-manifest skipped settings -------------------------------------
...
/// The refusal has to name a destination that works, not the other config file
/// — which refuses these too.
#[test]
fn set_tells_the_user_where_a_refused_key_belongs() {
    // Refused in a project manifest, but the global config file takes it.
    // `userconfig` is writable globally yet never read back, so it points at
    // the key that actually supplies the user-level `.npmrc`.
    for (key, expected) in [
        ...
        assert!(help.contains(expected), "{key}: {help}");
    }
}
...
fn set_does_not_suggest_a_camel_spelling_the_project_manifest_refuses() {
    ...
    .unwrap_err();
    let help = err.help().unwrap().to_string();
    assert!(!help.contains("pnpmHomeDir"), "{help}");
}
