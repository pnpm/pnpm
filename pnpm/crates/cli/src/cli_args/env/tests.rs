...
fn use_is_global_only() {
    let error = args(false, &["use", "24"]).subcommand(&config_with_global_bin()).unwrap_err();
    assert!(matches!(error, EnvError::LocalUseUnsupported), "{error:?}");
}

#[test]
fn use_requires_a_version() {
    let error = args(true, &["use", "  "]).subcommand(&config_with_global_bin()).unwrap_err();
...
fn list_takes_an_optional_selector() {
    for (params, expected) in [
        ...
        let subcommand = args(false, &params).subcommand(&config_with_global_bin()).unwrap();
