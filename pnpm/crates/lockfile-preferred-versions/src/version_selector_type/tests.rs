...
fn classifies_exact_version_as_version() {
    assert_eq!(
        get_version_selector_type("1.0.0"),
        Some(VersionSelectorType::Version),
    );
    assert_eq!(
        get_version_selector_type("1.2.3-beta.1"),
        Some(VersionSelectorType::Version),
    );
}
...
fn classifies_semver_range_as_range() {
    ...
    assert_eq!(
        get_version_selector_type(">=1.0.0 <2.0.0"),
        Some(VersionSelectorType::Range),
    );
    assert_eq!(
        get_version_selector_type("1.x"),
        Some(VersionSelectorType::Range),
    );
}
...
fn classifies_url_safe_string_as_tag() {
    assert_eq!(
        get_version_selector_type("latest"),
        Some(VersionSelectorType::Tag),
    );
    assert_eq!(
        get_version_selector_type("next"),
        Some(VersionSelectorType::Tag),
    );
    assert_eq!(
        get_version_selector_type("beta-rc.1"),
        Some(VersionSelectorType::Tag),
    );
}
...
fn rejects_unknown_specs() {
    assert_eq!(
        get_version_selector_type("git+ssh://example.com/repo.git"),
        None,
    );
