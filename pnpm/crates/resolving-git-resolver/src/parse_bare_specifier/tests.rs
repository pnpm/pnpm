...
fn correct_url_keeps_bracketed_ipv6_host() {
    ...
    assert_eq!(
        correct_url("ssh://[::1]:2222/repo.git"),
        "ssh://[::1]:2222/repo.git",
    );
    assert_eq!(
        correct_url("ssh://[::1]:team/repo.git"),
        "ssh://[::1]/team/repo.git",
    );
    assert_eq!(
        correct_url("ssh://git@[::1]/repo.git"),
        "ssh://git@[::1]/repo.git",
    );
    assert_eq!(
        correct_url("ssh://git@[::1]:team/repo.git"),
...
fn finalize_hosted_auth_url_kept_verbatim_without_archive_eligibility() {
    ...
    assert!(
        spec.hosted.is_none(),
        "credentialed URL must never resolve to a host archive",
    );
}
...
fn every_representation_of_a_hosted_specifier_keeps_its_committish() {
    ...
    let cases: [(String, String, Option<&str>, Option<&str>); 7] = [
        (
            "foo/bar#develop".into(),
            "github:foo/bar#develop".into(),
            Some("develop"),
            None,
        ),
        (
            "foo/bar#v1.0.0".into(),
            "github:foo/bar#v1.0.0".into(),
            Some("v1.0.0"),
            None,
        ),
        (
    ...
    for (input, expected_specifier, expected_committish, expected_range) in &cases {
        ...
        assert!(
            !spec.fetch_spec.contains('#'),
            "ls-remote target keeps a committish: {input}",
        );
        assert_eq!(
            (
...
fn fetch_spec_for_inputs_without_user_info() {
    let cases: &[(&str, &str)] = &[
        ...
        (
            "git+ssh://git.example.com/team/repo.git",
            "ssh://git.example.com/team/repo.git",
        ),
        (
            "git+ssh://git.example.com:team/repo.git",
...
fn path_extracted_from_scp_style_inputs() {
    let cases: &[(&str, Option<&str>)] = &[
        (
            "ssh://username:password@example.com:repo.git#path:/a/@b",
            Some("/a/@b"),
        ),
        (
            "ssh://username:password@example.com:repo/@foo.git#path:/a/@b",
            Some("/a/@b"),
        ),
        (
            "ssh://username:password@example.com:22/repo/@foo.git#path:/a/@b",
        ...
        (
            "ssh://username:password@example.com:22repo/@foo.git#path:/a/@b",
            Some("/a/@b"),
        ),
        (
            "ssh://username:password@example.com:22/repo/@foo.git#path:/a/@b&dev",
        ...
        (
            "git+ssh://username:password@example.com:repo.git#path:/a/@b",
            Some("/a/@b"),
        ),
        (
            "git+ssh://username:password@example.com:repo/@foo.git#path:/a/@b",
            Some("/a/@b"),
        ),
        (
            "git+ssh://username:password@example.com:22/repo/@foo.git#path:/a/@b",
        ...
        (
            "ssh://username:password@example.com:22/repo/@foo.git#dev",
            None,
        ),
        ("git+ssh://username:password@example.com:repo.git", None),
        (
            "git+ssh://username:password@example.com:22/repo/@foo.git#dev",
            None,
        ),
    ];
