//! Strip control and formatting characters from text that reaches the
//! terminal. Rust counterpart of the TypeScript `@pnpm/text.sanitize`
//! helpers: store-derived strings (package descriptions, registry error
//! bodies, usernames) can carry bidi overrides or zero-width joiners that
//! make rendered output misrepresent what is installed, so every such
//! string is filtered before it is printed.
