...
/// Per-dialect SQL operations behind [`SqlAuth`]. Used only as a
/// generic bound (`Db: AuthSqlBackend`), so the methods return
/// `impl Future + Send` (RPITIT) and dispatch statically — no boxed
/// futures or trait objects.
