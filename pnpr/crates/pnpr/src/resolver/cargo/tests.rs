...
async fn cached_deletes_stale_entries() {
    ...
    let result = IndexFetcher::cached_with_ttl(&stale_path, Duration::from_mins(1)).await;
...
async fn cached_returns_fresh_entries() {
    ...
    let result = IndexFetcher::cached_with_ttl(&fresh_path, Duration::from_hours(1)).await;
