...
async fn batch_publish_writes_every_package_in_one_request() {
    ...
    let response = app
        .clone()
        .oneshot(put_json_with_token("/npm/-/pnpm/v1/publish", &body, &token))
        .await
        .unwrap();
    assert_eq!(response.status(), StatusCode::CREATED);
    let payload = body_json(response.into_body()).await;
        ...
        [("batch-a", "1.0.0", bytes_a.as_slice()), ("batch-b", "2.0.0", bytes_b.as_slice())]
    {
        ...
        let served = app
            .clone()
            .oneshot(Request::get(format!("/npm/{name}")).body(Body::empty()).unwrap())
            .await
            .unwrap();
...
async fn batch_publish_supports_scoped_packages_with_libnpmpublish_attachment_names() {
    ...
    let packument_response = app
        .clone()
        .oneshot(Request::get("/npm/@scope/wire-form").body(Body::empty()).unwrap())
        .await
        .unwrap();
    ...
    assert_eq!(
        ...
        "http://example.test/npm/@scope/wire-form/-/wire-form-1.0.0.tgz",
    );

    let served = app
        .oneshot(
            Request::get("/npm/@scope/wire-form/-/wire-form-1.0.0.tgz")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn anonymous_batch_publish_is_rejected() {
    ...
    let response = app.oneshot(put_json("/npm/-/pnpm/v1/publish", &body)).await.unwrap();
...
async fn batch_publish_rolls_back_every_package_when_one_fails_integrity() {
    ...
    let response =
        app.oneshot(put_json_with_token("/npm/-/pnpm/v1/publish", &body, &token)).await.unwrap();
...
async fn batch_publish_merges_with_previously_published_versions() {
    ...
    let response = app
        .clone()
        .oneshot(put_json_with_token("/npm/-/pnpm/v1/publish", &first, &token))
        .await
        .unwrap();
    assert_eq!(response.status(), StatusCode::CREATED);
    ...
    let response =
        app.oneshot(put_json_with_token("/npm/-/pnpm/v1/publish", &second, &token)).await.unwrap();
    assert_eq!(response.status(), StatusCode::CREATED);
