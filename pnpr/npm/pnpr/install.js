#!/usr/bin/env node
// Preinstall step: replace the placeholder `bin/pnpr` with the platform's
// native binary so the `pnpr` command runs the binary directly, with no
// Node.js launcher in the way.
//
// `bin/pnpr` is published as a non-executable placeholder rather than a Node
// launcher on purpose. A launcher would force every `pnpr` invocation through
// Node startup (~170ms) just to spawn the native binary, and on Windows the
// `.bin` shim generated from its `#!/usr/bin/env node` shebang would hardcode a
// `node bin/pnpr` call this script could not undo (npm does not re-read
// package.json after preinstall). So the binary has to be in place before the
// command is ever resolved.
//
// Consequence: when this script does not run — `--ignore-scripts`, or pnpm/Bun
// blocking build scripts until `@pnpm/pnpr` is allow-listed — `bin/pnpr` stays
...
function setup() {
  ...
  // already filtered the `@pnpm/pnpr.*` packages by their `os`/`cpu`/`libc`
  ...
  if (platform === "win32") {
    // The `.bin` shim already points at the original `bin/pnpr` name and npm
    ...
    // 0o755: the swapped file is what the `.bin/pnpr` entry resolves to.
 ...
 * second copy of the binary on disk) and falls back to a copy across
...
function rewriteBin(binValue) {
  ...
  // `pnpr` runnable; the rewrite only helps shims regenerated later.
  try {
