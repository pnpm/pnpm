...
const UNSAVED_DEPENDENCIES = chalk.cyanBright(
...
const fixtureWithNoPkgNameAndNoVersion = f.find(
  'fixture-with-no-pkg-name-and-no-version'
)
const fixtureWithNoPkgVersion = f.find('fixture-with-no-pkg-version')
const fixtureWithExternalLockfile = path.join(
  f.find('fixture-with-external-shrinkwrap'),
  'pkg'
)
...
test('list all deps of a package that has an external lockfile', async () => {
  expect(
    await list([fixtureWithExternalLockfile], {
...
test('list with default parameters in pkg that has no name and version', async () => {
  expect(
    await list([fixtureWithNoPkgNameAndNoVersion], {
      lockfileDir: fixtureWithNoPkgNameAndNoVersion,
    })
  ).toBe(`${LEGEND}
...
test('list with default parameters in pkg that has no version', async () => {
  expect(
    await list([fixtureWithNoPkgVersion], {
      lockfileDir: fixtureWithNoPkgVersion,
    })
  ).toBe(`${LEGEND}
...
test('list with depth 1', async () => {
  expect(await list([fixture], { depth: 1, lockfileDir: fixture }))
    .toBe(`${LEGEND}
...
test('list with depth -1', async () => {
  expect(await list([fixture], { depth: -1, lockfileDir: fixture })).toBe(
    `${boldHighlighted(`fixture@1.0.0 ${fixture}`)}`
  )
})
...
test('list with depth 1 and selected packages', async () => {
  expect(
    await listForPackages(
      ['make-dir', 'pify@2', 'sort-keys@2', 'is-negative'],
      [fixture],
      { depth: 1, lockfileDir: fixture }
    )
  ).toBe(`${LEGEND}
...
test('list in long format', async () => {
  expect(await list([fixture], { long: true, lockfileDir: fixture }))
    .toBe(`${LEGEND}
...
test('JSON list with depth 1', async () => {
  expect(
    await list([fixture], { reportAs: 'json', depth: 1, lockfileDir: fixture })
  ).toBe(
    JSON.stringify(
      [
        {
          ...
          dependencies: {
            'write-json-file': {
              ...
              resolved:
                'https://registry.npmjs.org/write-json-file/-/write-json-file-2.3.0.tgz',
              path: path.join(
                fixture,
                'node_modules/.pnpm/write-json-file@2.3.0/node_modules/write-json-file'
              ),
              ...
              dependencies: {
                'detect-indent': {
                  ...
                  path: path.join(
                    fixture,
                    'node_modules/.pnpm/detect-indent@5.0.0/node_modules/detect-indent'
                  ),
                },
                'graceful-fs': {
                  from: 'graceful-fs',
                  ...
                  path: path.join(
                    fixture,
                    'node_modules/.pnpm/graceful-fs@4.2.2/node_modules/graceful-fs'
                  ),
                },
                'make-dir': {
                  from: 'make-dir',
                  ...
                  path: path.join(
                    fixture,
                    'node_modules/.pnpm/make-dir@1.3.0/node_modules/make-dir'
                  ),
                },
                pify: {
                  from: 'pify',
                  ...
                  path: path.join(
                    fixture,
                    'node_modules/.pnpm/pify@3.0.0/node_modules/pify'
                  ),
                },
                'sort-keys': {
                  from: 'sort-keys',
                  ...
                  path: path.join(
                    fixture,
                    'node_modules/.pnpm/sort-keys@2.0.0/node_modules/sort-keys'
                  ),
                },
                'write-file-atomic': {
                  ...
                  path: path.join(
                    fixture,
                    'node_modules/.pnpm/write-file-atomic@2.4.3/node_modules/write-file-atomic'
                  ),
                },
              },
            },
          ...
          devDependencies: {
            'is-positive': {
              ...
              path: path.join(
                fixture,
                'node_modules/.pnpm/is-positive@3.1.0/node_modules/is-positive'
              ),
            },
          },
          optionalDependencies: {
            'is-negative': {
              ...
              resolved:
                'https://registry.npmjs.org/is-negative/-/is-negative-2.1.0.tgz',
              path: path.join(
                fixture,
                'node_modules/.pnpm/is-negative@2.1.0/node_modules/is-negative'
              ),
            },
          },
        },
...
test('JSON list with aliased dep', async () => {
  expect(
    await list([fixtureWithAliasedDep], {
      reportAs: 'json',
      lockfileDir: fixtureWithAliasedDep,
    })
  ).toBe(
    JSON.stringify(
      [
...
test('print empty', async () => {
  expect(await list([emptyFixture], { lockfileDir: emptyFixture })).toBe(
...
test("don't print empty", async () => {
  expect(
    await list([emptyFixture], {
      alwaysPrintRootPackage: false,
      lockfileDir: emptyFixture,
    })
  ).toBe('')
})
...
test('peer dependencies are marked when searching', async () => {
  const fixture = f.find('with-peer')
  const output = await listForPackages(['ajv'], [fixture], {
    depth: 1,
    lockfileDir: fixture,
  })
  expect(output).toBe(`${LEGEND}
