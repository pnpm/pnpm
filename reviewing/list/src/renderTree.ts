...
const sortPackages = sortBy.default(ramdaPath.default(['name']) as (pkg: PackageNode | PackageInfo) => Ord)
...
async function renderTreeForPackage(
  ...
  const output = (
    await Promise.all(
      dependenciesFields.map(async (dependenciesField: DependenciesField | 'unsavedDependencies'): Promise<string | null> => {
...
function printLabel(getPkgColor: GetPkgColor, node: PackageNode | PackageInfo) {
...
function getPkgColor(node: PackageNode | PackageInfo): chalk.Chalk | ((s: string) => string) {
  if (node.dev === true) {
