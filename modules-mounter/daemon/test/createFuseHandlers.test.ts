...
describe('FUSE handlers', () => {
  ...
  beforeAll(async () => {
    ...
    handlers = await createFuseHandlers(
      fixture,
      path.join(fixture, 'store/v3/files')
    )
  })
  ...
  it('readdir', () => {
    ...
    handlers.readdir(
      '/.pnpm/is-positive@1.0.0/node_modules',
      (returnCode, files) => {
        expect(returnCode).toBe(0)
    ...
    handlers.readdir(
      '/.pnpm/@zkochan+git-config@0.1.0/node_modules/@zkochan',
      (returnCode, files) => {
        expect(returnCode).toBe(0)
    ...
    handlers.readdir(
      '/.pnpm/@zkochan+git-config@0.1.0/node_modules/@zkochan/git-config/does-not-exist',
      (returnCode, files) => {
    ...
    handlers.readdir(
      '/.pnpm/@zkochan+git-config@0.1.0/node_modules/@types',
      (returnCode) => {
        expect(returnCode).toBe(Fuse.ENOENT)
      }
    )
