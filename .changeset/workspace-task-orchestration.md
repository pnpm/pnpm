...
"@pnpm/workspace.projects-sorter": minor
...
- A dependency cycle among the tasks of a run is now an error naming the participating tasks (`ERR_PNPM_TASK_CYCLE`) instead of silently running in an arbitrary order.
...
- With `--bail` (the default), tasks already running when a failure happens finish and report their outcome; nothing new is dispatched.
- `pnpm -r run --dry-run <script>` prints the task graph that would execute without running anything; `--json` emits the tasks and their resolved dependency edges.
