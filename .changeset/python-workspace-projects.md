...
A build backend runs code from the index, so it is approved the way a dependency's build scripts are. Name it under `allowBuilds` in `pnpm-workspace.yaml`, as `hatchling: true`. An install that has not approved a backend does not build the projects that need it.
