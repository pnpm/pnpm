...
`--ignore-workspace` now also applies to the `packageManager` check that runs ahead of every command. That check read the ignored workspace's `pnpm-workspace.yaml`, so an unrecognized setting in it failed the command with `ERR_PNPM_UNRECOGNIZED_WORKSPACE_SETTINGS`, and the pin came from the workspace root rather than the project.
