...
- `pnpm install` now reads dynamic Python project metadata from the build backend. Projects with only a `requirements.txt` file now get a Python environment and lockfile. Related to [pnpm/pnpm#14945](https://github.com/pnpm/pnpm/issues/14945).
...
- Fixed `pnpm audit` hanging on dependency graphs with many shared dependencies [pnpm/pnpm#15005](https://github.com/pnpm/pnpm/issues/15005).
...
- `pnpm install` now downloads the Rust standard library's dependencies when Cargo configuration enables `build-std` [pnpm/pnpm#14944](https://github.com/pnpm/pnpm/issues/14944).
...
- `pnpm deploy` now links commands exposed by workspace dependencies into the deployed project's `node_modules/.bin` directory [pnpm/pnpm#14899](https://github.com/pnpm/pnpm/issues/14899).
...
- `pnpm install` now accepts Rust git dependencies that omit a package version. pnpm also generates Cargo lockfiles for workspaces with git dependencies [pnpm/pnpm#14944](https://github.com/pnpm/pnpm/issues/14944).
...
- `pnpm install` now installs wheels whose `RECORD` hashes disagree with their contents. The wheel archive's locked SHA-256 hash remains verified. pnpm writes correct hashes to the installed `RECORD` [pnpm/pnpm#15061](https://github.com/pnpm/pnpm/issues/15061).
...
- pnpm rejects a `registries` entry named `pkg` in any case, such as `PKG` or `Pkg`.
