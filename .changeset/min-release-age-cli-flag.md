...
Also hardened `getPublishedByPolicy()` so `minimumReleaseAge` rejects `NaN`, `Infinity`, and negative values (which previously disabled the supply-chain maturity gate or crashed on `toISOString()`), and a single `--minimum-release-age-exclude` flag value is coerced to an array so the exclude matcher works for the common single-value invocation.
