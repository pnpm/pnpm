...
No setting, error code, or lockfile field changes. The `registries` and `namedRegistries` settings are read under the names users write them. A `preResolution` hook still reads `ctx.registries`, which is the name pacquet passes as well. An `updateConfig` hook receives the `Config` object itself, so it reads and sets the renamed fields: `config.registriesByScope` and `config.registriesByPrefix`.
