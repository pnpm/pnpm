...
The first time a workspace's `pnpmExecCommand` runs under a user — and whenever the command changes — pnpm prints a notice to stderr showing the command and the binary it resolved, so a project cannot silently redirect which pnpm runs. Repeat runs with an unchanged command are silent.
