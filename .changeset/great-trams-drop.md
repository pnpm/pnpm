...
Add support for a global YAML config file named `rc.yaml`.
...
- pnpm-specific settings which can only be loaded from YAML files such as global `rc.yaml` and local `pnpm-workspace.yaml`.
