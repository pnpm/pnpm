...
test('print cat index file content', async () => {
...

  {
    ...
    const output = await catIndex.handler(
      config as catIndex.CatIndexCommandOptions,
      ['bytes@3.1.2']
    )

    expect(output).toBeTruthy()
    expect(
      typeof JSON.parse(output).files['package.json'].checkedAt
    ).toBeTruthy()
  }
})
...
test('prints index file error', async () => {
  ...
  try {
    ...
    await catIndex.handler(config as catIndex.CatIndexCommandOptions, [
      'bytes@3.1.1',
    ])
