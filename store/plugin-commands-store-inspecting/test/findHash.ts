...
test('print index file path with hash', async () => {
...

  {
    ...
    expect(output)
      .toBe(`${PACKAGE_INFO_CLR('lodash')}@${PACKAGE_INFO_CLR('4.17.19')}  ${INDEX_PATH_CLR('/24/dbddf17111f46417d2fdaa260b1a37f9b3142340e4145efe3f0937d77eb56c862d2a1d2901ca16271dc0d6335b0237c2346768a3ec1a3d579018f1fc5f7a0d-index.json')}
...
test('print index file path with hash error', async () => {
  ...
  try {
    ...
    await findHash.handler(config as findHash.FindHashCommandOptions, [
      'sha512-fXs1pWlUdqT2j',
    ])
  } catch (_err: unknown) {
    // @ts-ignore
    err = _err
