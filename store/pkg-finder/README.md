...
Resolves a package's file index from the content-addressable store (CALVES) and returns a uniform `Map<string, string>` mapping filenames to absolute paths on disk.
...
- **Integrity** (`integrity` field): looks up the index file in CALVES by integrity hash.
...
This function returns only the original package files. Files added or removed by post-install scripts (side effects) are not included. Use the raw `PackageFilesIndex` from `@pnpm/store.calves` if you need side-effect files.
