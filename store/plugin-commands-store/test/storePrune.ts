...
test.skip('remove packages that are used by project that no longer exist', async () => {
  ...
  await cafsHas(
    ssri.fromHex('f0d86377aa15a64c34961f38ac2a9be2b40a1187', 'sha1').toString()
  )
...
test('keep dependencies used by others', async () => {
  ...
  await execa(
    'node',
    [pnpmBin, 'remove', 'camelcase-keys', '--store-dir', storeDir],
    { env: { npm_config_registry: REGISTRY } }
  )
...
test('keep dependency used by package', async () => {
  ...
  await execa(
    'node',
    [pnpmBin, 'remove', 'is-not-positive', '--store-dir', storeDir],
    { env: { npm_config_registry: REGISTRY } }
  )
...
test('prune will skip scanning non-directory in storeDir', async () => {
  ...
  fs.writeFileSync(
    path.join(storeDir, STORE_VERSION, 'files/.DS_store'),
    'foobar'
  )
