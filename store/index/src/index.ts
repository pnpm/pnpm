 ...
 *
 * Git-hosted tarballs (`resolution.type === 'git-tarball'`) are addressed
 * by `gitHostedStoreIndexKey(pkgId, { built })` — their cached content
 * depends on whether build scripts ran during fetch (`preparePackage`),
 * so the `built` dimension is part of the key. The integrity-only key
 * would collapse the built/not-built variants into one slot.
 *
 * Tarballs with integrity that aren't git-hosted are addressed by
 * `storeIndexKey(integrity, pkgId)`.
 *
 * Resolutions with neither marker fall through to
 * `gitHostedStoreIndexKey` — these are typically lockfile entries
 * written by older pnpm versions that lacked integrity.
 */
export function pickStoreIndexKey (
  resolution: { type?: string, integrity?: string },
  pkgId: string,
  opts: { built: boolean }
): string {
  if (resolution.type === 'git-tarball' || !resolution.integrity) {
