...
export function getIndexKeyInCafs (
  integrity: string,
  pkgId: string
): string {
  const { hexDigest } = parseIntegrity(integrity)
  const hex = hexDigest.substring(0, 64)
  return `${path.join(hex.slice(0, 2), hex.slice(2))}-${pkgId.replace(/[\\/:*?"<>|]/g, '+')}`
}
...
export function getIndexFilePathInCafs (
  ...
  return path.join(storeDir, `index/${getIndexKeyInCafs(integrity, pkgId)}.mpk`)
}
