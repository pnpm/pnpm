...
const RUST_SOURCES = /^(?:pnpm\/|pnpr\/|\.cargo\/|Cargo\.(?:toml|lock)$|rust-toolchain\.toml$)/
...
export function determineTestScope ({ event, base, cwd = process.cwd() }) {
  ...
  try {
    ...
    // Release notes and Rust sources cannot change the TypeScript tests; other
    // root inputs can affect every package.
    if (files.some(file => !file.startsWith('pnpm11/') && !/^\.changeset\/[^/]+\.md$/.test(file) && !RUST_SOURCES.test(file))) return full
