...
test.each([
  ...
  (editionSpecifier, useNodeVersion, releaseChannel) => {
...
test.each([
  ...
  'invalid Node.js specifier',
  (editionSpecifier, useNodeVersion, releaseChannel) => {
    expect(() => parseNodeSpecifier(editionSpecifier)).toThrow(
      `Node.js version (${useNodeVersion}) must contain the release channel (${releaseChannel})`
    )
  }
)

test.each([['nightly'], ['rc'], ['test'], ['v8-canary']])(
  'invalid Node.js specifier',
  async (specifier) => {
    ...
    await expect(promise).rejects.toHaveProperty(
      'hint',
      `The correct syntax for ${specifier} release is strictly X.Y.Z-${specifier}.W`
    )
  }
)
