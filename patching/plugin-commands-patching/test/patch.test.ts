...
describe('patch and commit', () => {
  ...
  test('patch and commit', async () => {
    ...
    expect(fs.readFileSync(path.join(patchDir, 'license'), 'utf8')).toContain(
      'The MIT License (MIT)'
    )
  ...
  test('patch and commit with a custom edit dir', async () => {
    ...
    const output = await patch.handler({ ...defaultPatchOption, editDir }, [
      'is-positive@1.0.0',
    ])
    const patchDir = getPatchDirFromPatchOutput(output)
  ...
  test('patch and commit with custom patches dir', async () => {
    ...
    expect(
      fs.readFileSync(path.join(patchesDir, 'is-positive@1.0.0.patch'), 'utf8')
    ).toContain('// test patching')
    expect(
      fs.readFileSync('node_modules/is-positive/index.js', 'utf8')
    ).toContain('// test patching')
  ...
  test('patch throws an error if the edit-dir already exists and is not empty', async () => {
    ...
    await expect(() =>
      patch.handler({ ...defaultPatchOption, editDir }, ['is-positive@1.0.0'])
  ...
  test('patch and commit should work when the patch directory is specified with a trailing slash', async () => {
    const editDir =
      path.join(tempy.directory()) + (os.platform() === 'win32' ? '\\' : '/')
  ...
  test('should reuse existing patch file by default', async () => {
    ...
    output = await patch.handler(
      { ...defaultPatchOption, rootProjectManifest: manifest },
      ['is-positive@1.0.0']
    )
    ...
    expect(fs.readFileSync(path.join(patchDir, 'index.js'), 'utf8')).toContain(
      '// test patching'
    )
  })

  test('if the patch file is not existed when patching, should throw an error', async () => {
    const { writeProjectManifest, manifest } = await readProjectManifest(
      process.cwd()
    )
  ...
  test('should ignore patch files with --ignore-patches', async () => {
    ...
    output = await patch.handler(
      { ...defaultPatchOption, ignoreExisting: true },
      ['is-positive@1.0.0']
    )
    patchDir = getPatchDirFromPatchOutput(output)
    ...
    expect(
      fs.readFileSync(path.join(patchDir, 'index.js'), 'utf8')
    ).not.toContain('// test patching')
  })

  test('patch throw an error if no package specified', async () => {
    await expect(() =>
      patch.handler({ ...defaultPatchOption }, [])
  ...
  test('should throw an error if no installed versions found for patched package', async () => {
    await expect(() =>
      patch.handler(defaultPatchOption, ['chalk'])
    ).rejects.toThrow(
      `Can not find chalk in project ${process.cwd()}, did you forget to install chalk?`
    )
  })

  test('should throw an error if no preferred versions found for patched package', async () => {
    await expect(() =>
      patch.handler(defaultPatchOption, ['is-positive@2.0.0'])
    ).rejects.toThrow(
      `Can not find is-positive@2.0.0 in project ${process.cwd()}, you can specify currently installed version: 1.0.0.`
    )
  })
  ...
  test('patch package with installed version', async () => {
    ...
    expect(fs.existsSync(patchDir)).toBe(true)
    expect(
      JSON.parse(fs.readFileSync(path.join(patchDir, 'package.json'), 'utf8'))
        .version
    ).toBe('1.0.0')
  })
  ...
  test('should skip empty patch content and not create patches dir', async () => {
    ...
    expect(result).toBe(
      `No changes were found to the following directory: ${patchDir}`
    )
...
describe('prompt to choose version', () => {
  ...
  test('prompt to choose version if multiple version founded for patched package', async () => {
    ...
    expect(prompt.mock.calls[0][0].choices).toEqual(
    ...
    fs.appendFileSync(
      path.join(patchDir, 'source/index.js'),
      '// test patching',
      'utf8'
    )
    await patchCommit.handler(
    ...
    expect(
      fs.readFileSync(
        'node_modules/.pnpm/ava@5.2.0/node_modules/chalk/source/index.js',
        'utf8'
      )
    ).toContain('// test patching')
  })
...
describe('patching should work when there is a no EOL in the patched file', () => {
  ...
  it('should work when adding content on a newline', async () => {
    ...
    fs.appendFileSync(
      path.join(userPatchDir, 'lib/index.js'),
      '\n// test patching',
      'utf8'
    )
    ...
    const patchContent = fs.readFileSync(
      'patches/safe-execa@0.1.2.patch',
      'utf8'
    )
    expect(patchContent).toContain('diff --git')
    expect(patchContent).toContain('// test patching')
    ...
    expect(
      fs.readFileSync('node_modules/safe-execa/lib/index.js', 'utf8')
    ).toContain('// test patching')
  })
  it('should work fine when new content is appended', async () => {
    ...
    expect(
      fs.readFileSync('node_modules/safe-execa/lib/index.js', 'utf8')
    ).toContain('//# sourceMappingURL=index.js.map// patch without newline')
  })
})
...
describe('patch and commit in workspaces', () => {
  ...
  test('patch commit should work in workspaces', async () => {
    ...
    expect(patchContent).toContain('// test patching')
    expect(
      fs.readFileSync('project-1/node_modules/is-positive/index.js', 'utf8')
    ).toContain('// test patching')
    expect(fs.existsSync('project-1/node_modules/is-positive/license')).toBe(
      false
    )
    expect(
      fs.readFileSync('project-2/node_modules/is-positive/index.js', 'utf8')
    ).toContain('// test patching')
    expect(fs.existsSync('project-2/node_modules/is-positive/license')).toBe(
      false
    )
  })
  ...
  test('patch and patch-commit for git hosted dependency', async () => {
    ...
    prompt.mockResolvedValue({
      version:
        'https://codeload.github.com/zkochan/hi/tar.gz/4cdebec76b7b9d1f6e219e06c42d92a6b8ea60cd',
    })
    prompt.mockClear()
    ...
    expect(patchContent).toContain('// test patching')
    expect(
      fs.readFileSync('./project-2/node_modules/hi/index.js', 'utf8')
    ).toContain('// test patching')
  })
})
...
describe('patch with custom modules-dir and virtual-store-dir', () => {
  ...
  test('should work with custom modules-dir and virtual-store-dir', async () => {
    const manifest = fs.readFileSync(
      path.join(customModulesDirFixture, 'package.json'),
      'utf8'
    )
    const lockfileYaml = fs.readFileSync(
      path.join(customModulesDirFixture, 'pnpm-lock.yaml'),
      'utf8'
    )
    const { allProjects, allProjectsGraph, selectedProjectsGraph } =
      await readProjects(customModulesDirFixture, [])
    await install.handler({
      ...DEFAULT_OPTS,
      cacheDir,
      storeDir,
    ...
    expect(
      fs.readFileSync(
        path.join(
          ...
          'packages/bar/fake_modules/is-positive/index.js'
        ),
        'utf8'
      )
    ).toContain('// test patching')
...
describe('patch-remove', () => {
  ...
  test('patch-remove should work as expected', async () => {
    ...
    fs.writeFileSync(
      path.join(process.cwd(), 'patches/is-positive@1.0.0.patch'),
      'test patch content',
      'utf8'
    )
    ...
    expect(
      fs.existsSync(path.join(process.cwd(), 'patches/is-positive@1.0.0.patch'))
    ).toBe(false)
  ...
  test('prompt to select patches that to be removed', async () => {
    ...
    expect(prompt.mock.calls[0][0].choices).toEqual(
      expect.arrayContaining(['is-positive@1.0.0', 'chalk@4.1.2'])
    )
    prompt.mockClear()
  ...
  test('should throw error when there is no patch to remove', async () => {
    await expect(() =>
      patchRemove.handler(defaultPatchRemoveOption, [])
