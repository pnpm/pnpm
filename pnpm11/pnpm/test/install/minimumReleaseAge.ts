...
describe('minimumReleaseAge resolution fallback', () => {
  ...
  let registry: string
  const maturityEnv = { env: { pnpm_config_minimum_release_age: '1440' } }

  beforeAll(async () => {
    const versions = ['1.2.3', '1.2.4', '1.2.5', '1.2.6']
    const packages = new Map([
      ['rolldown', staggeredPackument('rolldown', versions)],
      ['vite', staggeredPackument('vite', ['8.2.1'])],
      ...['@rolldown/binding-linux-arm64-gnu', '@rolldown/binding-darwin-x64']
        .map(name => [name, staggeredPackument(name, versions)] as const),
    ])
    const mismatched = staggeredPackument('rolldown', versions)
    mismatched.name = 'mismatched-rolldown'
    for (const manifest of Object.values(mismatched.versions)) {
    ...
    registry = `--config.registry=http://127.0.0.1:${(server.address() as { port: number }).port}/`
  })
  ...
  test('backs off to a direct version whose exact-pinned transitive deps are mature', async () => {
    prepare({
    ...
    await execPnpm(
      [registry, 'install', '--lockfile-only'],
      maturityEnv
    )
  ...
  test('update --latest walks past every version of an excluded package whose dependencies are too young', async () => {
    prepare({
    ...
    await execPnpm(
      [registry, 'update', 'rolldown', '--latest', '--lockfile-only'],
      maturityEnv
    )
...
function staggeredPackument (name: string, versions: string[]) {
  ...
  return {
    ...
    versions: Object.fromEntries(versions.map(version => [version, {
      name,
      version,
      dist: { tarball: `https://registry.example/${name}-${version}.tgz`, integrity },
      dependencies: name === 'vite' ? { rolldown: '~1.2.1' } : undefined,
      optionalDependencies: name === 'rolldown'
        ? { '@rolldown/binding-linux-arm64-gnu': version, '@rolldown/binding-darwin-x64': version }
        : undefined,
    }])),
    time: Object.fromEntries(versions.map(version => [version,
      name.startsWith('@rolldown/') && (version === '1.2.5' || version === '1.2.6')
