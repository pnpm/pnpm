  ...
  return runLimited(pkgMirror, async (limit) => {
    ...
    async function persistFreshMeta (fetched: FetchMetadataResult): Promise<{ meta: PackageMeta, pickedPackage: PackageInRegistry | null }> {
      ...
      if (!opts.dryRun) {
        ...
        const etagForDisk = resultToSave === fetched ? fetched.etag : null
        saveMetaBestEffort(pkgMirror, prepareMirrorForDisk(ctx, resultToSave, etagForDisk))
      }
...
function persistUpgradedMeta (
  ...
  saveMetaBestEffort(pkgMirror, prepareMirrorForDisk(ctx, upgradedFrom, null))
  return metaForCache
}
...
function prepareMirrorForDisk (
  ...
  const etag = etagOverride === undefined ? result.etag : (etagOverride ?? undefined)
  if (etag !== result.etag) {
    if (ctx.filterMetadata === true) {
      return prepareJsonForDisk(condenseMetaForCache(ctx, result.meta), etag)
    }
    return prepareIndexedForDisk(result.meta, etag)
  }
