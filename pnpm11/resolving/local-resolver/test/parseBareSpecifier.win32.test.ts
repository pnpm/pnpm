...
const { parseLocalPath, parseLocalScheme } = await import('../src/parseBareSpecifier.js')
...
test('parseLocalPath claims a forward-slash separated path on Windows', () => {
  expect(parseLocalPath({ bareSpecifier: 'some/local/dir' }, projectDir, projectDir, opts)).not.toBeNull()
})
