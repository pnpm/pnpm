...
test('delete override is always treated as applied', () => {
  // Delete overrides remove the dependency key from the manifest before
  // resolution, so the lockfile won't contain it. The scan must treat
  // them as applied to avoid false unused-override warnings.
  const lockfile = {
    lockfileVersion: '6.0',
    importers: {
      '.': {
  ...
  expect(applied.has('foo')).toBe(true)
})
