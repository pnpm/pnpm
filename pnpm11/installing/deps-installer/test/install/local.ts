...
test('install also installs direct file directory dependency projects specified via absolute path', async () => {
  const project = prepareEmpty()
  const localPkgDir = fs.mkdtempSync(path.resolve('local-pkg-install-test-'))
  try {
    fs.writeFileSync(path.join(localPkgDir, 'package.json'), JSON.stringify({
    ...
    await install({
      dependencies: {
        'local-pkg': `file:${normalizePath(path.resolve(localPkgDir))}`,
      },
    }, testDefaults())
