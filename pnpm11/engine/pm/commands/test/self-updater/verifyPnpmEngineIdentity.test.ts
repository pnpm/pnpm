...
const { exePlatformPkgDirName, exePlatformPkgDirNameNext, nativeTargetName, verifyPnpmEngineIdentity } = await import('@pnpm/engine.pm.commands')
...
describe('verifyPnpmEngineIdentity', () => {
  test('resolves when the native engine and its platform binary carry a valid registry signature over the installed bytes', async () => {
    ...
    await expect(verifyPnpmEngineIdentity(envLockfile(), EXE_ENGINE, optsTrusting(key))).resolves.toBeUndefined()
  })
  ...
  test('throws when the installed bytes do not match what the registry signed (tamper)', async () => {
    ...
    await expect(verifyPnpmEngineIdentity(envLockfile(), EXE_ENGINE, optsTrusting(key))).rejects.toThrow(/Refusing to run pnpm/)
  })
  ...
  test('throws when the engine is signed by a key pnpm does not trust', async () => {
    ...
    await expect(verifyPnpmEngineIdentity(envLockfile(), EXE_ENGINE, optsTrusting(createSigningKey()))).rejects.toThrow(/Refusing to run pnpm/)
  })
  ...
  test('throws (fails closed) when the canonical registry is the configured registry and is unreachable', async () => {
    ...
    await expect(verifyPnpmEngineIdentity(envLockfile(), EXE_ENGINE, opts)).rejects.toThrow(/Refusing to run pnpm/)
  })
  ...
  test('warns and proceeds when a user-configured registry is unreachable and no signature is obtainable', async () => {
    ...
    await expect(verifyPnpmEngineIdentity(envLockfile(), EXE_ENGINE, optsTrusting(createSigningKey()))).resolves.toBeUndefined()
  })
  ...
  test('warns and proceeds when a user-configured registry pins a non-sha512 integrity no signature can cover', async () => {
    ...
    await expect(verifyPnpmEngineIdentity(lockfile, EXE_ENGINE, optsTrusting(createSigningKey()))).resolves.toBeUndefined()
  })

  test('throws for a non-sha512 integrity pin when the engine resolves from the canonical registry', async () => {
    const lockfile = envLockfile()
    ;(lockfile.packages as Record<string, { resolution: { integrity: string } }>)['@pnpm/exe@9.1.0'] = { resolution: { integrity: 'sha1-8bee00286a17c00a13c7e6e6dd9a9b389220ee7f' } }

    const opts = { ...optsTrusting(createSigningKey()), registriesByScope: { default: CANONICAL_REGISTRY } }
    await expect(verifyPnpmEngineIdentity(lockfile, EXE_ENGINE, opts)).rejects.toThrow(/Refusing to run pnpm/)
  })

  test('skips (no throw) when no trusted keys are provided', async () => {
    await expect(verifyPnpmEngineIdentity(envLockfile(), EXE_ENGINE, { registriesByScope: { default: REGISTRY }, trustedKeys: [] })).resolves.toBeUndefined()
  })
  ...
  test('throws when an engine component in the lockfile has no integrity metadata', async () => {
    ...
    await expect(verifyPnpmEngineIdentity(lockfile, EXE_ENGINE, optsTrusting(key))).rejects.toThrow(/integrity metadata is missing/)
  })
  ...
  test('resolves when the platform binary carries a valid registry signature', async () => {
    ...
    await expect(verifyPnpmEngineIdentity(lockfile, EXE_ENGINE, optsTrusting(key))).resolves.toBeUndefined()
  })
  ...
  test('throws when the wrapper snapshot lists no platform binaries', async () => {
    ...
    await expect(verifyPnpmEngineIdentity(lockfile, EXE_ENGINE, optsTrusting(key))).rejects.toThrow(/platform binaries are missing/)
  })

  test('throws when the native engine ships no binary for this host', async () => {
    const key = createSigningKey()
    const lockfile = envLockfile()
    ;(lockfile.snapshots as Record<string, unknown>)['@pnpm/exe@9.1.0'] = { optionalDependencies: { '@pnpm/exe.aix-mips': '9.1.0' } }

    await expect(verifyPnpmEngineIdentity(lockfile, EXE_ENGINE, optsTrusting(key))).rejects.toMatchObject({
      code: 'ERR_PNPM_PNPM_ENGINE_NO_NATIVE_BINARY',
      message: `Cannot run @pnpm/exe@9.1.0 on this host: it ships no native binary for ${HOST_TARGET}.`,
    })
  })

  test('verifies only the JavaScript pnpm when that is the engine installed, even though the pinned @pnpm/exe ships no binary for this host', async () => {
    const key = createSigningKey()
    // Only `pnpm` is published: verifying `@pnpm/exe` or a platform binary would
    // reach an unmocked packument and fail closed.
    mockPackument({ name: 'pnpm', integrity: PNPM_INTEGRITY, signatures: [{ keyid: key.keyid, sig: key.sign('pnpm@9.1.0', PNPM_INTEGRITY) }] })

    const lockfile = envLockfile()
    ;(lockfile.snapshots as Record<string, unknown>)['@pnpm/exe@9.1.0'] = { optionalDependencies: { '@pnpm/exe.aix-mips': '9.1.0' } }

    await expect(verifyPnpmEngineIdentity(lockfile, JS_ENGINE, optsTrusting(key))).resolves.toBeUndefined()
  })

  test('throws when the JavaScript pnpm is not pinned by the lockfile', async () => {
    const lockfile = envLockfile()
