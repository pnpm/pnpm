...
async function resolveBinPath (binRelativePath: string, pkgPath: string): Promise<string | null> {
  ...
  if (nodeModulesSpecifier != null) {
    ...
    const resolveFrom = createRequire(path.join(resolveFromPath, 'noop.js'))
    const searchPaths = resolveFrom.resolve.paths(nodeModulesSpecifier) ?? []
    for (const searchPath of searchPaths) {
      const candidatePath = path.join(searchPath, nodeModulesSpecifier)
      const stat = await fs.stat(candidatePath).catch(() => null) // eslint-disable-line no-await-in-loop
      if (stat?.isFile()) {
