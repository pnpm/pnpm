...
export class StoreIndex {
  ...
  protected openDatabase (storeDir: string): void {
    ...
    // SQLite creates a missing database as 0o644 and copies that mode onto
    // the WAL sidecars. Add the store directory's group-write bit before
    // journal_mode=WAL. An existing database is not chmod'd.
    const dbIsNew = process.platform !== 'win32' && !fs.existsSync(dbPath)
...
// store stays group-writable. Owner read and write stay set.
function inheritedFileMode (parentMode: number): number {
  return (parentMode & 0o666) | 0o600
}

function grantInheritedFileMode (filePath: string, parent: string): void {
  let wanted: number
  try {
    wanted = inheritedFileMode(fs.statSync(parent).mode)
  } catch (err: unknown) {
