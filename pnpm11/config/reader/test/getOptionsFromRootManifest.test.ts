...
test('getOptionsFromPnpmSettings() rejects non-object resolutions field', () => {
  expect(() => getOptionsFromPnpmSettings(process.cwd(), {}, {
    resolutions: [1, 2, 3] as unknown as Record<string, string>,
...
test('getOptionsFromPnpmSettings() strips control chars from manifest-sourced selectors in error messages', () => {
  ...
  expect(() => getOptionsFromPnpmSettings(process.cwd(), {}, {
    ...
    code: 'ERR_PNPM_INVALID_RESOLUTIONS',
    message: expect.stringContaining('name?[ERROR] injected??'),
  }))
})
