...
test('prints lockfile breaking change error with the incompatible versions and a --force warning', async () => {
  ...
  const err = new PnpmError(
    ...
    'Lockfile /home/src/pnpm-lock.yaml not compatible with current pnpm: it was generated with lockfileVersion 6.0, but the current pnpm version supports lockfileVersion 9.0'
  )
  ...
  const output = await firstValueFrom(output$.pipe(take(1), map(normalizeNewline)))
  expect(output).toBe(`${formatError('ERR_PNPM_LOCKFILE_BREAKING_CHANGE', 'Lockfile /home/src/pnpm-lock.yaml not compatible with current pnpm: it was generated with lockfileVersion 6.0, but the current pnpm version supports lockfileVersion 9.0')}
${ERROR_PAD}
