...
test('prints lockfile breaking change error with the incompatible versions and a --force warning', async () => {
  const output$ = toOutput$({
    context: { argv: ['install'] },
    streamParser: createStreamParser(),
  })
  ...
  const err = new PnpmError(
    ...
    'Lockfile /home/src/pnpm-lock.yaml not compatible with current pnpm: it was generated with lockfileVersion 6.0, but the current pnpm version supports lockfileVersion 9.x'
  )
  ...
  const output = await firstValueFrom(output$.pipe(take(1), map(normalizeNewline)))
  expect(output).toBe(`${formatError('ERR_PNPM_LOCKFILE_BREAKING_CHANGE', 'Lockfile /home/src/pnpm-lock.yaml not compatible with current pnpm: it was generated with lockfileVersion 6.0, but the current pnpm version supports lockfileVersion 9.x')}
${ERROR_PAD}
${ERROR_PAD}Run with the --force parameter to recreate the lockfile.
${ERROR_PAD}Warning: recreating the lockfile may break your application if it was generated by a different version of pnpm.
${ERROR_PAD}To keep the existing lockfile, install the version of pnpm that generated it.`)
})
