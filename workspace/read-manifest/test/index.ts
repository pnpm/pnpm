...
test('readWorkspaceManifest() throws on array content', async () => {
  await expect(
    readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/array'))
  ).rejects.toThrow('Expected object but found - array')
})

test('readWorkspaceManifest() does not throw on empty packages field', async () => {
  await expect(
    readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/packages-empty'))
  ).resolves.toBeTruthy()
})

test('readWorkspaceManifest() throws on string packages field', async () => {
  await expect(
    readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/packages-string'))
...
test('readWorkspaceManifest() throws on numeric package', async () => {
  await expect(
    readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/packages-contains-number'))
  ).rejects.toThrow('Invalid package type - number')
})

test('readWorkspaceManifest() works when no workspace file is present', async () => {
  const manifest = await readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/no-workspace-file'))

  expect(manifest).toBeUndefined()
})

test('readWorkspaceManifest() works when workspace file is empty', async () => {
  const manifest = await readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/empty'))

  expect(manifest).toBeUndefined()
})

test('readWorkspaceManifest() works when workspace file is null', async () => {
  const manifest = await readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/null'))
...
describe('readWorkspaceManifest() catalog field', () => {
  test('works on simple catalog', async () => {
    await expect(readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/catalog-ok'))).resolves.toEqual({
      packages: ['packages/**', 'types'],
      catalog: {
        foo: '^1.0.0',
      },
  ...
  test('throws on invalid array', async () => {
    await expect(
      readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/catalog-invalid-array'))
    ).rejects.toThrow('Expected catalog field to be an object, but found - array')
  })

  test('throws on invalid object', async () => {
    await expect(
      readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/catalog-invalid-object'))
    ).rejects.toThrow('Expected catalog field to be an object, but found - number')
  })

  test('throws on invalid specifier', async () => {
    await expect(
      readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/catalog-invalid-specifier'))
...
describe('readWorkspaceManifest() catalogs field', () => {
  ...
  test('throws on invalid array', async () => {
    await expect(
      readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/catalogs-invalid-array'))
    ).rejects.toThrow('Expected catalogs field to be an object, but found - array')
  })

  test('throws on invalid value', async () => {
    await expect(
      readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/catalogs-invalid-object'))
    ).rejects.toThrow('Expected catalogs field to be an object, but found - number')
  })

  test('throws on invalid named catalog array', async () => {
    await expect(
      readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/catalogs-invalid-named-catalog-array'))
  ...
  test('throws on invalid named catalog object', async () => {
    await expect(
      readWorkspaceManifest(path.join(import.meta.dirname, '__fixtures__/catalogs-invalid-named-catalog-object'))
