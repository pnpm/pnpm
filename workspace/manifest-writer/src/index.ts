...
import {
  type GLOBAL_CONFIG_YAML_FILENAME,
...
export async function updateWorkspaceManifest (
  ...
  manifest ??= {}
  let shouldBeUpdated =
    opts.updatedCatalogs != null && addCatalogs(manifest, opts.updatedCatalogs)
  if (opts.cleanupUnusedCatalogs) {
    shouldBeUpdated =
      removePackagesFromWorkspaceCatalog(manifest, opts.allProjects ?? []) ||
      shouldBeUpdated
  }
...
function addCatalogs (
  ...
  for (const catalogName in newCatalogs) {
    let targetCatalog: Record<string, string> | undefined =
      catalogName === 'default'
...
function removePackagesFromWorkspaceCatalog (
  manifest: Partial<WorkspaceManifest>,
  packagesJson: Project[]
): boolean {
  let shouldBeUpdated = false
  if (
    packagesJson.length === 0 ||
    (manifest.catalog == null && manifest.catalogs == null)
  ) {
    return shouldBeUpdated
  }
  ...
  if (manifest.catalog) {
    const packagesToRemove = Object.keys(manifest.catalog).filter(
      (pkg) => !packageReferences[pkg]?.has('catalog:')
    )
  ...
  if (manifest.catalogs) {
    ...
    for (const [catalogName, catalog] of Object.entries(manifest.catalogs)) {
      ...
      const packagesToRemove = Object.keys(catalog).filter((pkg) => {
        ...
        return (
          !references?.has(`catalog:${catalogName}`) &&
          !references?.has('catalog:')
        )
      })
      for (const pkg of packagesToRemove) {
        delete catalog[pkg]
