...
test('findWorkspacePackages() output warnings for non-root workspace project', async () => {
  ...
  expect(logger.warn).toHaveBeenNthCalledWith(1, { prefix: barPath, message: `"pnpm" was found in ${barPath}/package.json. It will not take effect, should configure "pnpm" at the root of the project.` })
  expect(logger.warn).toHaveBeenNthCalledWith(2, { prefix: barPath, message: `"resolutions" was found in ${barPath}/package.json. It will not take effect, should configure "resolutions" at the root of the project.` })
  expect(logger.warn).toHaveBeenNthCalledWith(3, { prefix: fooPath, message: `"pnpm" was found in ${fooPath}/package.json. It will not take effect, should configure "pnpm" at the root of the project.` })
})
