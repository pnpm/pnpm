...
test('select only package dependencies (excluding the package itself)', async () => {
  const { selectedProjectsGraph } = await filterWorkspacePackages(
    PKGS_GRAPH,
    [
      {
        excludeSelf: true,
...
test('selection should fail when diffing to a branch that does not exist', async () => {
  let err!: PnpmError
  try {
    await filterWorkspacePackages(
      PKGS_GRAPH,
      [{ diff: 'branch-does-no-exist' }],
      { workspaceDir: process.cwd() }
    )
...
test('should return unmatched filters', async () => {
  const { unmatchedFilters } = await filterWorkspacePackages(
    PKGS_GRAPH,
    [
      {
        excludeSelf: true,
...
test('select all packages except one', async () => {
  ...
  expect(Object.keys(selectedProjectsGraph)).toStrictEqual(
    Object.keys(omit(['/packages/project-1'], PKGS_GRAPH))
  )
})
