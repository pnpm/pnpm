...
- [x] `TypeScript repo: installing/deps-installer/test/hoistedNodeLinker/install.ts:229` `hoistingLimits should prevent packages to be hoisted`. Stubbed in `known_failures::hoisting_limits_prevents_hoisting` — the fresh path threads `hoistingLimits` (proven by the passing `externalDependencies` case), but the e2e assertion exposes a `real-hoist` semantics divergence (yarn hoister borders the subtree; pacquet blocks the named leaf) that's separate from the fresh-path enablement.
