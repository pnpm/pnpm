...
1. **Rust crate** `pacquet-napi` at `pacquet/crates/napi`
...
2. **npm wrapper package** `@pnpm/napi` at `pacquet/npm/napi`
   ...
   - Platform packages follow the same 8-target matrix as `pacquet/npm/pnpm`
...
`pacquet/npm/napi/scripts/generate-packages.mjs` produces the eight
