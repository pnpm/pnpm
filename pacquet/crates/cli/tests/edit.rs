...
#[test]
fn edit_dependency_via_pnpm_config_editor() {
    let CommandTempCwd { root, workspace, npmrc_info, .. } =
        CommandTempCwd::init().add_mocked_registry();
    ...
    let content = fs::read_to_string(&index_js).unwrap();
    assert!(content.contains("config_editor_modified"));
...
#[test]
fn edit_dependency_via_workspace_yaml_editor() {
    let CommandTempCwd { root, workspace, npmrc_info, .. } =
        CommandTempCwd::init().add_mocked_registry();
    ...
    fs::write(workspace.join("pnpm-workspace.yaml"), workspace_yaml_text).unwrap();
    ...
    pacquet(&workspace).with_args(["edit", "is-positive"]).assert().success();

    let content = fs::read_to_string(&index_js).unwrap();
    assert!(content.contains("yaml_editor_modified"));
