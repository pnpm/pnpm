...
use pacquet_resolving_npm_resolver::mirror::{
    encode_pkg_name, get_registry_name, load_meta, ABBREVIATED_META_DIR, FULL_FILTERED_META_DIR,
    FULL_META_DIR,
};
...
impl CacheCommand {
    ...
    fn find_metadata_files(
        ...
        let registry_prefix = get_registry_name(&config.registry).unwrap_or_else(|_| "*".to_string());
        
        let patterns = if packages.is_empty() {
            vec![format!("{}/**", registry_prefix)]
        } else {
            packages.iter().map(|p| format!("{}/{}.jsonl", registry_prefix, encode_pkg_name(p))).collect()
        };
        ...
        for pattern in patterns {
            ...
            for entry in glob.walk(cache_dir).filter_map(std::result::Result::ok) {
                if let Some(path_str) = entry.path().strip_prefix(cache_dir).ok().and_then(|p| p.to_str()) {
    ...
    pub fn run<'a>(
        ...
        match self {
            ...
            CacheCommand::Delete { packages } => {
                ...
                for meta_file in &meta_files {
                    ...
                    if let Ok(rel_path) = file_path.strip_prefix(&cache_dir) {
                        let _ = fs::remove_file(config.cache_dir.join(ABBREVIATED_META_DIR).join(rel_path));
                        let _ = fs::remove_file(config.cache_dir.join(FULL_META_DIR).join(rel_path));
                        let _ = fs::remove_file(config.cache_dir.join(FULL_FILTERED_META_DIR).join(rel_path));
                    }
                }
                if !meta_files.is_empty() {
                    println!("{}", meta_files.join("\n"));
            ...
            CacheCommand::View { package } => {
                ...
                for entry in glob.walk(&cache_dir).filter_map(std::result::Result::ok) {
                    if let Some(path_str) = entry.path().strip_prefix(&cache_dir).ok().and_then(|p| p.to_str()) {
                        meta_file_paths.push((path_str.replace('\\', "/"), entry.path().to_path_buf()));
                    }
                }
                ...
                let store_index = StoreIndex::open_readonly_in(&config.store_dir).into_diagnostic()?;
                ...
                for (file_path, full_path) in meta_file_paths {
                    ...
                    for (version, json_frag) in meta_object.versions.fragments() {
                        let Ok(manifest) = serde_json::from_str::<serde_json::Value>(&json_frag) else { continue };
                        let Some(integrity) = manifest.get("dist").and_then(|d| d.get("integrity")).and_then(|i| i.as_str()) else { continue };
                        
                        let key = pacquet_store_dir::store_index_key(integrity, &format!("{}@{}", meta_object.name, version));
                    ...
                    let registry_name = PathBuf::from(&file_path)
                        .parent().map_or_else(|| ".".to_string(), |p| p.to_string_lossy().into_owned());
