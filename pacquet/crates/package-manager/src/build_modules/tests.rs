...
/// When the slot already matches the overlay (a full reinstall whose slot
/// wasn't re-linked, so the build output a previous install materialized
/// is still present), the gate skips the forced re-import entirely — no
/// stage-and-swap, no rebuild. The overlay here points at non-existent
/// CAS blobs, so any actual import attempt would fail and fall back to
/// running the (failing) postinstall; the install succeeding with no
/// lifecycle event proves the re-import was skipped.
#[cfg(unix)]
#[test]
fn side_effects_cache_skips_reimport_when_slot_already_matches() {
    static EVENTS: Mutex<Vec<LogEvent>> = Mutex::new(Vec::new());
    EVENTS.lock().expect("lock").clear();
    ...
    // A slot that already holds exactly the overlay's files, as if a
    // previous install had materialized them.
    let pkg_dir = create_failing_postinstall_fixture(virtual_store_dir.path(), &pkg_key);
    fs::write(pkg_dir.join("generated-by-postinstall.js"), "built").expect("write side-effect");
    ...
    // Overlay lists exactly what's on disk, but points at CAS blobs that
    // don't exist — so a re-import would error (and fall back to the
    // failing rebuild) if the skip didn't fire.
    let overlay = std::collections::HashMap::from([
        ("package.json".to_string(), virtual_store_dir.path().join("missing-a")),
        ("generated-by-postinstall.js".to_string(), virtual_store_dir.path().join("missing-b")),
    ]);
    let mut side_effects_maps = std::collections::HashMap::new();
    side_effects_maps.insert(
        pkg_key.clone(),
    ...
    .run::<RecordingReporter>()
    .expect("a slot already matching the overlay must skip the re-import, not rebuild");

    let captured = EVENTS.lock().expect("lock").clone();
    let any_lifecycle = captured.iter().any(|e| matches!(e, LogEvent::Lifecycle(_)));
    assert!(
        !any_lifecycle,
        "a matching slot must skip both the rebuild and the re-import: {captured:#?}",
    );
}
