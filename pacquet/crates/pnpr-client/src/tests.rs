...
fn a_package_frame_parses_its_fetch_hint() {
    let line = br#"{"type":"package","id":"acme@1.0.0","name":"acme","version":"1.0.0","integrity":"sha512-abc","tarball":"https://r.test/acme/-/acme-1.0.0.tgz","unpackedSize":123456}"#;
    let Frame::Package { id, name, version, integrity, tarball, unpacked_size } =
...
/// A `package` frame without `unpackedSize` — an older server, or a
/// registry that never published the field — still parses.
#[test]
fn a_package_frame_without_unpacked_size_parses() {
    ...
    let Frame::Package { unpacked_size, .. } = parse_frame(line).expect("frame parses") else {
        panic!("expected a package frame");
    };
