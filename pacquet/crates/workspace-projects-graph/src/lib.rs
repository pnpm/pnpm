...
//! depends on. The projects filter walks this graph to resolve
//! `--filter` selectors that follow dependencies / dependents.
//!
//! The edge computation resolves a dependency specifier to a sibling
//! either by a local path (`file:` / `link:` / a relative or absolute
//! path) or by name plus a semver / `workspace:` version match against
