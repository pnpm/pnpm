...
mod tests {
    ...
    fn identical_frame_produces_empty_diff() {
        ...
        assert!(output.is_empty(), "identical frame should produce no output, got: {output:?}");
    }
    ...
    fn unchanged_sticky_line_not_rewritten() {
        ...
        assert!(
            ...
            "unchanged sticky line must not appear in diff output, got: {output:?}",
        );
    }
    ...
    fn inline_diff_writes_only_changed_chars() {
        ...
        assert!(
            output.contains('1'),
            "inline diff should write the changed character, got: {output:?}",
        );
        assert!(
            !output.contains("Progress"),
            "inline diff should not rewrite unchanged prefix, got: {output:?}",
        );
    }
    ...
    fn first_frame_writes_full_content() {
        ...
        assert!(
            output.contains("Progress"),
            "first frame should contain full content, got: {output:?}",
        );
    }
    ...
    fn added_line_appended() {
        ...
        assert!(
            ...
            "added line should appear in diff output, got: {output:?}",
        );
    }
    ...
    fn removed_line_cleared() {
        ...
        diff.update("Line 1\nLine 2\n");
        let output = diff.update("Line 1\n");
        assert!(output.contains("\x1b[0K"), "removed line should trigger clear, got: {output:?}",);
    }
}
