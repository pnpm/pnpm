...
- **Take the value as a parameter.** A function that needs a path declares `path: &Path`; one that needs an env-var value takes it as a `&str` argument or an `Option<String>`. Use a direct value parameter, not a `FnOnce` closure: callback-shaped "poor man DI" emulates the JS/TS pattern of passing accessor functions, but in Rust the proper seam is a capability trait on `Sys`, so the closure form is reserved for things that genuinely need lazy evaluation (e.g. `Config::current`'s `default: FnOnce() -> Config`, which lets the caller skip computing the SmartDefault when the call site already has one).
