...
> [!NOTE]
> Integration tests that need the local registry mock start it in-process. After dependencies are installed, `cargo test`, `cargo nextest run`, and `just test` should not require a separate registry process.
