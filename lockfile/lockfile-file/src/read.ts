  ...
  if (lockfileFile) {
    ...
    if (
      ...
      opts.wantedVersions.some((wantedVersion) => {
        if (semver.major(lockfileSemver) !== semver.major(comverToSemver(wantedVersion.toString()))) return false
        if (semver.gt(lockfileSemver, comverToSemver(wantedVersion.toString()))) {
          logger.warn({
