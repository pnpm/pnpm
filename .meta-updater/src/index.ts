  ...
  return createUpdateOptions({
    'package.json': (manifest: ProjectManifest & { keywords?: string[] } | null, { dir }: { dir: string }) => {
      ...
      if (manifest.name === 'monorepo-root') {
        manifest.scripts!['release'] = `pnpm --filter=@pnpm/exe publish --tag=${nextTag} --access=public --dry-run && pnpm publish --filter=!pnpm --filter=!@pnpm/exe --access=public --dry-run && pnpm publish --filter=pnpm --tag=${nextTag} --access=public --dry-run`
