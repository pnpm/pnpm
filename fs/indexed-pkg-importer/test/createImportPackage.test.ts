...
jest.unstable_mockModule('@pnpm/fs.graceful-fs', () => {
  ...
  const fsMock = {
    ...
    copyFile: jest.fn().mockReturnValue(Promise.resolve()),
...
beforeEach(() => {
  ...
  jest.spyOn(fs.promises, 'link').mockResolvedValue(undefined);
  jest.spyOn(fs.promises, 'stat').mockResolvedValue({ mode: 0o644 } as fs.Stats);
  jest.spyOn(fs.promises, 'readFile').mockResolvedValue(Buffer.from(''));
  jest.spyOn(fs.promises, 'writeFile').mockResolvedValue(undefined);
  jest.spyOn(fs, 'existsSync').mockReturnValue(true);
  jest.spyOn(fs, 'statSync').mockReturnValue({ ino: 1n } as fs.BigIntStats);
  ...
  jest.mocked(fs.promises.copyFile).mockClear()
  jest.mocked(fs.promises.readFile as jest.Mock).mockClear()
  jest.mocked(fs.promises.writeFile).mockClear()
  jest.mocked(fs.promises.link).mockClear()
  jest.mocked(gfs.mkdirSync).mockClear()
  jest.mocked(gfs.renameSync).mockClear()
  jest.mocked(fs.promises.stat as jest.Mock).mockReset()
...
testOnLinuxOnly('packageImportMethod=auto: clone files by default', async () => {
...
testOnLinuxOnly('packageImportMethod=auto: link files if cloning fails', async () => {
  const importPackage = createIndexedPkgImporter('auto')
  jest.mocked(fs.promises.copyFile).mockImplementation(() => {
  ...
  expect(fs.promises.link).toHaveBeenCalledWith(path.join('hash1'), path.join('project', 'package', 'package.json'))
  expect(fs.promises.link).toHaveBeenCalledWith(path.join('hash2'), path.join('project', 'package', 'index.js'))
  ...
  expect(fs.promises.link).toHaveBeenCalledWith(path.join('hash1'), path.join('project2', 'package', 'package.json'))
  expect(fs.promises.link).toHaveBeenCalledWith(path.join('hash2'), path.join('project2', 'package', 'index.js'))
})

testOnLinuxOnly('packageImportMethod=auto: link files if cloning fails and even hard linking fails but not with EXDEV error', async () => {
  const importPackage = createIndexedPkgImporter('auto')
  jest.mocked(fs.promises.copyFile).mockImplementation(() => {
    throw new Error('This file system does not support cloning')
  })
  let linkFirstCall = true
  jest.mocked(fs.promises.link).mockImplementation(async () => {
    if (linkFirstCall) {
      linkFirstCall = false
      throw new Error()
    }
  ...
  expect(fs.promises.link).toHaveBeenCalledWith(path.join('hash2'), path.join('project', 'package', 'index.js'))
  expect(fs.promises.link).toHaveBeenCalledTimes(2)
...
testOnLinuxOnly('packageImportMethod=auto: chooses copying if cloning and hard linking is not possible', async () => {
  const importPackage = createIndexedPkgImporter('auto')
  jest.mocked(fs.promises.copyFile).mockImplementation(async (src, dest, flags?: number) => {
  ...
  expect(fs.promises.copyFile).toHaveBeenCalledWith(path.join('hash2'), path.join('project', 'package', 'index.js'))
...
testOnLinuxOnly('packageImportMethod=hardlink: fall back to copying if hardlinking fails', async () => {
  const importPackage = createIndexedPkgImporter('hardlink')
  jest.mocked(fs.promises.link).mockImplementation((src, dest) => {
  ...
  expect(fs.promises.link).toHaveBeenCalledTimes(3)
  expect(fs.promises.copyFile).toHaveBeenCalledTimes(2) // One time the target already exists, so it won't be copied
  expect(fs.promises.copyFile).toHaveBeenCalledWith(path.join('hash1'), path.join('project', 'package', 'package.json'))
  expect(fs.promises.copyFile).toHaveBeenCalledWith(path.join('hash2'), path.join('project', 'package', 'index.js'))
})

test('packageImportMethod=hardlink does not relink package from store if package.json is linked from the store', async () => {
  const importPackage = createIndexedPkgImporter('hardlink')
  jest.spyOn(fs, 'statSync').mockReturnValue({ ino: BigInt(1) } as fs.BigIntStats)
  expect(await importPackage('project/package', {
    filesMap: new Map([
      ['index.js', 'hash2'],
      ['package.json', 'hash1'],
...
test('packageImportMethod=hardlink relinks package from store if package.json is not linked from the store', async () => {
  const importPackage = createIndexedPkgImporter('hardlink')
  let ino = 0
  jest.spyOn(fs, 'statSync').mockImplementation(() => ({ ino: ++ino } as unknown as fs.BigIntStats))
  expect(await importPackage('project/package', {
    filesMap: new Map([
      ['index.js', 'hash2'],
      ['package.json', 'hash1'],
...
test('packageImportMethod=hardlink does not relink package from store if package.json is not present in the store', async () => {
  const importPackage = createIndexedPkgImporter('hardlink')
  jest.spyOn(fs, 'statSync').mockImplementation(((file: fs.PathLike) => {
...
testOnLinuxOnly('packageImportMethod=hardlink: falls back to read+write when copyFileSync throws ENOTSUP', async () => {
  const importPackage = createIndexedPkgImporter('hardlink')
  jest.mocked(fs.promises.link).mockImplementation(async () => {
  ...
  jest.mocked(fs.promises.stat as jest.Mock).mockReturnValue({ mode: 0o644 })
  jest.mocked(fs.promises.readFile as jest.Mock).mockReturnValue(Buffer.from('file content'))
  expect(await importPackage('project/package', {
    filesMap: new Map([
      ['index.js', 'hash2'],
      ['package.json', 'hash1'],
...
testOnLinuxOnly('packageImportMethod=copy: falls back to read+write when copyFileSync throws ENOTSUP', async () => {
  const importPackage = createIndexedPkgImporter('copy')
  jest.mocked(fs.promises.copyFile).mockImplementation(() => {
    throw Object.assign(new Error('ENOTSUP: operation not supported on socket'), { code: 'ENOTSUP' })
  })
  jest.mocked(fs.promises.stat as jest.Mock).mockReturnValue({ mode: 0o755 })
  jest.mocked(fs.promises.readFile as jest.Mock).mockReturnValue(Buffer.from('file content'))
  expect(await importPackage('project/package', {
    filesMap: new Map([
      ['index.js', 'hash2'],
...
testOnLinuxOnly('packageImportMethod=auto: ENOTSUP on clone falls through to hardlinks', async () => {
  const importPackage = createIndexedPkgImporter('auto')
  jest.mocked(fs.promises.copyFile).mockImplementation(async (_src, _dest, flags?: number) => {
    if (flags === fs.constants.COPYFILE_FICLONE_FORCE) {
      throw Object.assign(new Error('ENOTSUP: operation not supported on socket'), { code: 'ENOTSUP' })
    }
  })
...
testOnLinuxOnly('packageImportMethod=auto: ENOTSUP on clone uses hardlinks for all subsequent packages too', async () => {
  ...
  jest.mocked(fs.promises.link).mockReset()
  const importPackage = createIndexedPkgImporter('auto')
  jest.mocked(fs.promises.copyFile).mockImplementation(async (_src, _dest, flags?: number) => {
    if (flags === fs.constants.COPYFILE_FICLONE_FORCE) {
      throw Object.assign(new Error('ENOTSUP: operation not supported on socket'), { code: 'ENOTSUP' })
...
testOnLinuxOnly('packageImportMethod=clone: falls back to copy on ENOTSUP, using atomic write for package.json', async () => {
...
testOnLinuxOnly('packageImportMethod=hardlink: rethrows non-ENOTSUP errors from copyFileSync', async () => {
  const importPackage = createIndexedPkgImporter('hardlink')
  jest.mocked(fs.promises.link).mockImplementation(async () => {
    throw new Error('hard link failed')
  })
  ...
  await expect(importPackage('project/package', {
    filesMap: new Map([
      ['index.js', 'hash2'],
    ]),
    force: false,
