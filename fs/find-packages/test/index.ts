...
test('finds packages by patterns', async () => {
  ...
  expect(
    [pkgs[0].manifest.name, pkgs[1].manifest.name].sort(compare)
  ).toStrictEqual(['component-1', 'component-2'])
})
...
test('finds packages by * pattern', async () => {
  ...
  expect(
    [pkgs[0].manifest.name, pkgs[1].manifest.name, pkgs[2].manifest.name].sort(
      compare
    )
  ).toStrictEqual(['component-1', 'component-2', 'many-pkgs-2'])
})
...
test('finds packages by default pattern', async () => {
  ...
  expect(pkgs.map(({ manifest }) => manifest.name).sort(compare)).toStrictEqual(
    ['component-1', 'component-2', 'foo', 'many-pkgs-2']
  )
})
