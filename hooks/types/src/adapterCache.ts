import { type Adapter, type WantedDependency } from './index.js'
...
export function getAdapterCacheKey (wantedDependency: WantedDependency): string {
...
/**
 * Check if an adapter can resolve a wanted dependency, using cache when available
...
export async function checkAdapterCanResolve (
  ...
  const cacheKey = getAdapterCacheKey(wantedDependency)
  ...
  const canResolve = await adapter.canResolve(wantedDependency)
