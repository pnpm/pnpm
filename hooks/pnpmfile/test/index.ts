...
import {
  requireHooks,
  requirePnpmfile,
  BadReadPackageHookError,
} from '@pnpm/pnpmfile'

test('ignoring a pnpmfile that exports undefined', () => {
  const pnpmfile = requirePnpmfile(
    path.join(__dirname, 'pnpmfiles/undefined.js'),
    __dirname
  )
...
test('readPackage hook run fails when returned dependencies is not an object ', () => {
  ...
  return expect(pnpmfile.hooks.readPackage({})).rejects.toEqual(
    new BadReadPackageHookError(
      pnpmfilePath,
      "readPackage hook returned package manifest object's property 'dependencies' must be an object."
    )
  )
})
