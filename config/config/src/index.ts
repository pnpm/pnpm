...
export const types = Object.assign(
  ...
  } satisfies Partial<Record<keyof KebabCaseConfig, any>>,
  npmTypes.types
)
...
export async function getConfig(opts: {
  ...
  const configDir = getConfigDir(process)
  {
    const warn = npmConfig.addFile(
      path.join(configDir as string, 'rc'),
      'pnpm-global'
    )
  ...
  pnpmConfig.rawLocalConfig = Object.assign.apply(Object, [
    ...
    ...npmConfig.list
      .slice(
        3,
        pnpmConfig.workspaceDir && pnpmConfig.workspaceDir !== cwd ? 5 : 4
      )
      .reverse(),
    cliOptions,
  ...
  if (
    ...
    pnpmConfig.only === 'production' ||
    (!pnpmConfig.only && pnpmConfig.production)
  ) {
