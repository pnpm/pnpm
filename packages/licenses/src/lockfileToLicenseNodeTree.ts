...
export type LicenseNodeTree = Omit<LicenseNode, 'path' | 'license' | 'vendorName' | 'vendorUrl' | 'packageManifest'> & {
  install: string[]
  remove: string[]
  metadata: Object
}
 ...
 * private
 */
export async function lockfileToLicenseNode (step: LockfileWalkerStep, options: LicenseExtractOptions) {
  const dependencies = {}
