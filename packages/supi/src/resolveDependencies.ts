...
async function install (
  ...
  if (!options.shamefullyFlatten && !proceed && options.depPath &&
    ...
    await exists(path.join(ctx.shrNModulesDir, `.${options.depPath}`)) && (
      options.currentDepth > 0 || wantedDependency.alias && await exists(path.join(ctx.shrNModulesDir, wantedDependency.alias))
    )) {
