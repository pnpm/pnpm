...
import { addDependenciesToSingleProject } from 'supi'
...
test('tarball from npm registry', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['http://localhost:4873/is-array/-/is-array-1.0.1.tgz'], await testDefaults())
...
test('tarball not from npm registry', async (t) => {
  const project = prepare(t)
  await addDependenciesToSingleProject(['https://github.com/hegemonic/taffydb/tarball/master'], await testDefaults())
...
test('tarballs from GitHub (is-negative)', async (t) => {
  const project = prepare(t)
  await addDependenciesToSingleProject(['is-negative@https://github.com/kevva/is-negative/archive/1d7e288222b53a0cab90a331f1865220ec29560c.tar.gz'], await testDefaults())
