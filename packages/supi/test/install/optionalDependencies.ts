...
import { addDependenciesToSingleProject, install, rebuild } from 'supi'
...
test('successfully install optional dependency with subdependencies', async (t) => {
  ...
  await addDependenciesToSingleProject(['fsevents@1.0.14'], await testDefaults({ targetDependenciesField: 'optionalDependencies' }))
})

test('skip failing optional dependencies', async (t: tape.Test) => {
  const project = prepare(t)
  await addDependenciesToSingleProject(['pkg-with-failing-optional-dependency@1.0.1'], await testDefaults())
...
test('skip optional dependency that does not support the current OS', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['dep-of-optional-pkg'], await testDefaults())
...
test('optional subdependency is skipped', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['pkg-with-optional', 'dep-of-optional-pkg'], await testDefaults({ reporter }))
