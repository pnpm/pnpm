...
test('installing aliased dependency', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['negative@npm:is-negative@1.0.0', 'positive@npm:is-positive'], await testDefaults())
...
test('aliased dependency w/o version spec, with custom tag config', async (t) => {
  ...
  await addDependenciesToSingleProject(['foo@npm:dep-of-pkg-with-1-dep'], await testDefaults({ tag }))
...
test('a dependency has an aliased subdependency', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['pkg-with-1-aliased-dep'], await testDefaults())
