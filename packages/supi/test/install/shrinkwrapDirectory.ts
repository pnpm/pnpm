...
testSkip('subsequent installation uses same shrinkwrap directory by default', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['is-positive@1.0.0'], await testDefaults({ shrinkwrapDirectory: path.resolve('..') }))
...
testSkip('subsequent installation fails if a different shrinkwrap directory is specified', async (t: tape.Test) => {
  ...
  try {
    await addDependenciesToSingleProject(['is-negative@1.0.0'], await testDefaults({ shrinkwrapDirectory: process.cwd() }))
  } catch (_) {
    err = _
  }
