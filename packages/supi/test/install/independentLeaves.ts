...
test('install with --independent-leaves', async (t: tape.Test) => {
  ...
  const pkg = await addDependenciesToPackage({}, ['rimraf@2.5.1'], await testDefaults({ independentLeaves: true }))

  const m = project.requireModule('rimraf')
  t.ok(typeof m === 'function', 'rimraf() is available')
  await project.isExecutable('.bin/rimraf')

  await install(pkg, await testDefaults({ independentLeaves: true, preferFrozenLockfile: false }))
...
test('--independent-leaves throws exception when executed on node_modules installed w/o the option', async (t: tape.Test) => {
  const project = prepareEmpty(t)
  const pkg = await addDependenciesToPackage({}, ['is-positive'], await testDefaults({ independentLeaves: false }))

  try {
    await addDependenciesToPackage(pkg, ['is-negative'], await testDefaults({ independentLeaves: true }))
...
test('--no-independent-leaves throws exception when executed on node_modules installed with --independent-leaves', async (t: tape.Test) => {
  ...
  try {
    await addDependenciesToPackage(pkg, ['is-negative'], await testDefaults({ independentLeaves: false }))
    t.fail('installation should have failed')
  } catch (err) {
