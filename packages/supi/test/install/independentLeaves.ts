...
test('install with --independent-leaves', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['rimraf@2.5.1'], await testDefaults({ independentLeaves: true }))
...
test('--independent-leaves throws exception when executed on node_modules installed w/o the option', async (t: tape.Test) => {
  const project = prepare(t)
  await addDependenciesToSingleProject(['is-positive'], await testDefaults({ independentLeaves: false }))

  try {
    await addDependenciesToSingleProject(['is-negative'], await testDefaults({ independentLeaves: true }))
...
test('--no-independent-leaves throws exception when executed on node_modules installed with --independent-leaves', async (t: tape.Test) => {
  ...
  try {
    await addDependenciesToSingleProject(['is-negative'], await testDefaults({ independentLeaves: false }))
    t.fail('installation should have failed')
  } catch (err) {
