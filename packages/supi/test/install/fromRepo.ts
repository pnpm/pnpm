...
test('from a github repo', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['kevva/is-negative'], await testDefaults())
...
test('from a github repo with different name via named installation', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['say-hi@github:zkochan/hi#4cdebec76b7b9d1f6e219e06c42d92a6b8ea60cd'], await testDefaults({ reporter }))
...
test('a subdependency is from a github repo with different name', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['has-aliased-git-dependency'], await testDefaults())
...
test('from a git repo', async (t: tape.Test) => {
  ...
  const project = prepare(t)
  await addDependenciesToSingleProject(['git+ssh://git@github.com/kevva/is-negative.git'], await testDefaults())
...
if (!isTravis) {
  test('from a non-github git repo', async (t: tape.Test) => {
    ...
    await addDependenciesToSingleProject(['git+http://ikt.pm2.io/ikt.git#3325a3e39a502418dc2e2e4bf21529cbbde96228'], await testDefaults())
