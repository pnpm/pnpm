...
test('preserve subdeps on update', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['foobarqar', 'abc-grand-parent-with-c'], await testDefaults())
...
test('update does not fail when package has only peer dependencies', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['has-pkg-with-peer-only'], await testDefaults())
...
test('update dependency when external shrinkwrap directory is used', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['foo'], await testDefaults({ shrinkwrapDirectory }))
