...
test("shrinkwrap doesn't lock subdependencies that don't satisfy the new specs", async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['react-datetime@2.8.8'], await testDefaults({ save: true }))

  // dependends on react-onclickoutside@0.3.4
  await addDependenciesToSingleProject(['react-datetime@1.3.0'], await testDefaults({ save: true }))
...
test('doing named installation when shrinkwrap.yaml exists already', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['is-positive'], await testDefaults({ reporter }))
  await install(await testDefaults({ reporter }))
...
test('respects shrinkwrap.yaml for top dependencies', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['foo'], await testDefaults({ save: true, reporter }))
  // t.equal(reporter.withArgs(fooProgress).callCount, 1, 'reported foo once')
  await addDependenciesToSingleProject(['bar'], await testDefaults({ targetDependenciesField: 'optionalDependencies' }))
  await addDependenciesToSingleProject(['qar'], await testDefaults({ addDependenciesToSingleProject: 'devDependencies' }))
  await addDependenciesToSingleProject(['foobar'], await testDefaults({ save: true }))
...
test('repeat install with shrinkwrap should not mutate shrinkwrap when dependency has version specified with v prefix', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['highmaps-release@5.0.11'], await testDefaults())
...
test('package is not marked optional if it is also a subdep of a regular dependency', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['pkg-with-1-dep'], await testDefaults())
  await addDependenciesToSingleProject(['dep-of-pkg-with-1-dep'], await testDefaults({ targetDependenciesField: 'optionalDependencies' }))
...
test('scoped module from different registry', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['@zkochan/foo', 'is-positive'], opts)
...
test('repeat install with no inner shrinkwrap should not rewrite packages in node_modules', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['is-negative@1.0.0'], await testDefaults())
...
test('updating package that has a github-hosted dependency', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['has-github-dep@1'], await testDefaults())
  await addDependenciesToSingleProject(['has-github-dep@latest'], await testDefaults())
...
test('updating package that has deps with peers', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['abc-grand-parent-with-c@0'], await testDefaults())
  await addDependenciesToSingleProject(['abc-grand-parent-with-c@1'], await testDefaults())
...
test('dev properties are correctly updated on named install', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['inflight@1.0.6'], await testDefaults({ targetDependenciesField: 'devDependencies' }))
  await addDependenciesToSingleProject(['foo@npm:inflight@1.0.6'], await testDefaults({}))
...
test('optional properties are correctly updated on named install', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['inflight@1.0.6'], await testDefaults({ targetDependenciesField: 'optionalDependencies' }))
  await addDependenciesToSingleProject(['foo@npm:inflight@1.0.6'], await testDefaults({}))
...
test('dev property is correctly set for package that is duplicated to both the dependencies and devDependencies group', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['overlap@2.2.8'], await testDefaults())
...
test('no shrinkwrap', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['is-positive'], await testDefaults({ shrinkwrap: false, reporter }))
...
test("don't remove packages during named install when shrinkwrap: false", async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['is-positive'], await testDefaults({ shrinkwrap: false }))
  await addDependenciesToSingleProject(['is-negative'], await testDefaults({ shrinkwrap: false }))
...
test('save tarball URL when it is non-standard', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['esprima-fb@3001.1.0-dev-harmony-fb'], await testDefaults())
...
test('when package registry differs from default one, save it to resolution field', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['@zkochan/git-config', 'is-positive'], await testDefaults({
    rawNpmConfig: {
...
test('shrinkwrap file has correct format when shrinkwrap directory does not equal the prefix directory', async (t: tape.Test) => {
  ...
  await addDependenciesToSingleProject(['pkg-with-1-dep', '@rstacruz/tap-spec@4.1.1', 'kevva/is-negative#1d7e288222b53a0cab90a331f1865220ec29560c'],
  ...
  await addDependenciesToSingleProject(['is-positive'], await testDefaults({ save: true, shrinkwrapDirectory: path.resolve('..') }))
...
test('use current shrinkwrap.yaml as initial wanted one, when wanted was removed', async (t) => {
  ...
  await addDependenciesToSingleProject(['lodash@4.17.11', 'underscore@1.9.0'], await testDefaults())

  await rimraf('shrinkwrap.yaml')

  await addDependenciesToSingleProject(['underscore@1.9.1'], await testDefaults())
