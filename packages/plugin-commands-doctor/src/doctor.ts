...
export async function handler(opts: Pick<Config, 'failedToLoadBuiltInConfig'>): Promise<void> {
  ...
  if (failedToLoadBuiltInConfig) {
    ...
    logger.warn({
      message:
        'Load npm builtin configs failed. If the prefix builtin config does not work, you can use "pnpm config ls" to show builtin configs. And then use "pnpm config --global set <key> <value>" to migrate configs from builtin to global.',
      prefix: process.cwd(),
    })
  }
}
