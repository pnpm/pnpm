...
const isPositiveMetaWithDeprecated = loadJsonFile.sync<any>(
  f.find('is-positive-with-deprecated.json')
)
const isPositiveMetaFull = loadJsonFile.sync<any>(
  f.find('is-positive-full.json')
)
const isPositiveBrokenMeta = loadJsonFile.sync<any>(
  f.find('is-positive-broken.json')
)
const sindresorhusIsMeta = loadJsonFile.sync<any>(
  f.find('sindresorhus-is.json')
)
...
test('resolveFromNpm()', async () => {
  nock(registry).get('/is-positive').reply(200, isPositiveMeta)
  ...
  const meta = await retryLoadJsonFile<any>(
    path.join(cacheDir, 'metadata/registry.npmjs.org/is-positive.json')
  )
...
test('resolveFromNpm() should save metadata to a unique file when the package name has upper case letters', async () => {
  nock(registry).get('/JSON').reply(200, jsonMeta)
  ...
  const meta = await retryLoadJsonFile<any>(
    path.join(
      cacheDir,
      'metadata/registry.npmjs.org/JSON_0ecd11c1d7a287401d148a23bbd7a2f8.json'
    )
  )
  expect(meta.name).toBeTruthy()
  expect(meta.versions).toBeTruthy()
...
test('resolve to biggest non-deprecated version that satisfies the range', async () => {
  nock(registry).get('/is-positive').reply(200, isPositiveMetaWithDeprecated)

  const resolveFromNpm = createResolveFromNpm({
    cacheDir: tempy.directory(),
  })
  const resolveResult = await resolveFromNpm(
    { alias: 'is-positive', pref: '3' },
    {
      registry,
    }
  )
...
test('can resolve aliased dependency', async () => {
  ...
  const resolveResult = await resolveFromNpm(
    { alias: 'positive', pref: 'npm:is-positive@1.0.0' },
    {
      registry,
    }
  )
...
test('can resolve aliased dependency w/o version specifier', async () => {
  ...
  const resolveResult = await resolveFromNpm(
    { alias: 'positive', pref: 'npm:is-positive' },
    {
      registry,
    }
  )
...
test('can resolve aliased scoped dependency', async () => {
  nock(registry).get('/@sindresorhus%2Fis').reply(200, sindresorhusIsMeta)

  const resolveFromNpm = createResolveFromNpm({
    cacheDir: tempy.directory(),
  })
  const resolveResult = await resolveFromNpm(
    { alias: 'is', pref: 'npm:@sindresorhus/is@0.6.0' },
    {
      registry,
    }
  )
...
test('can resolve aliased scoped dependency w/o version specifier', async () => {
  ...
  const resolveResult = await resolveFromNpm(
    { alias: 'is', pref: 'npm:@sindresorhus/is' },
    {
      registry,
    }
  )
...
test('offline resolution fails when package meta not found in the store', async () => {
  ...
  await expect(
    ...
    new PnpmError(
      'NO_OFFLINE_META',
      `Failed to resolve is-positive@1.0.0 in package mirror ${path.join(cacheDir, 'metadata/registry.npmjs.org/is-positive.json')}`
    )
  )
})
...
test('error is thrown when package is not found in the registry', async () => {
  ...
  nock(registry).get(`/${notExistingPackage}`).reply(404, {})

  const resolveFromNpm = createResolveFromNpm({
    cacheDir: tempy.directory(),
  })
  await expect(
    resolveFromNpm({ alias: notExistingPackage, pref: '1.0.0' }, { registry })
  ).rejects.toThrow(
...
test('error is thrown when registry not responding', async () => {
  ...
  await expect(
    resolveFromNpm(
      { alias: notExistingPackage, pref: '1.0.0' },
      { registry: notExistingRegistry }
    )
  ).rejects.toThrow(
    new PnpmError(
      'META_FETCH_FAIL',
      `GET ${notExistingRegistry}/${notExistingPackage}: request to ${notExistingRegistry}/${notExistingPackage} failed, reason: getaddrinfo ENOTFOUND not-existing.pnpm.io`,
      { attempts: 1 }
    )
  )
})
...
test('error is thrown when there is no package found for the requested version', async () => {
  ...
  await expect(resolveFromNpm(wantedDependency, { registry })).rejects.toThrow(
...
test('error is thrown when package needs authorization', async () => {
  nock(registry).get('/needs-auth').reply(403)

  const resolveFromNpm = createResolveFromNpm({
    cacheDir: tempy.directory(),
  })
  await expect(
    resolveFromNpm({ alias: 'needs-auth', pref: '*' }, { registry })
  ).rejects.toThrow(
    new RegistryResponseError(
      {
...
test('resolve when tarball URL is requested from the registry', async () => {
  ...
  const resolveResult = await resolve(
    {
      alias: 'is-positive',
      pref: `${registry}is-positive/-/is-positive-1.0.0.tgz`,
    },
    {
      registry,
    }
...
test('resolve injected dependency from local directory when it matches the latest version of the package', async () => {
  ...
  const resolveResult = await resolve(
    { alias: 'is-positive', injected: true, pref: '1.0.0' },
    {
      projectDir: '/home/istvan/src',
...
test('resolve from local directory when alwaysTryWorkspacePackages is false but workspace: is used', async () => {
  ...
  const resolveResult = await resolve(
    { alias: 'is-positive', pref: 'workspace:*' },
    {
      alwaysTryWorkspacePackages: false,
      projectDir: '/home/istvan/src',
      registry,
...
test('resolve from local directory when alwaysTryWorkspacePackages is false but workspace: is used with a different package name', async () => {
  ...
  const resolveResult = await resolve(
    { alias: 'positive', pref: 'workspace:is-positive@*' },
    {
      alwaysTryWorkspacePackages: false,
      projectDir: '/home/istvan/src',
      registry,
...
test('resolve from local directory when package is not found in the registry', async () => {
  nock(registry).get('/is-positive').reply(404, {})
...
test('resolve from local directory when the requested version is not found in the registry but is available locally', async () => {
  ...
  const resolveResult = await resolve(
    { alias: 'is-positive', pref: '100.0.0' },
    {
      projectDir: '/home/istvan/src/foo',
      registry,
      workspacePackages: {
...
test('workspace protocol: resolve from local directory even when it does not match the latest version of the package', async () => {
  ...
  const resolveResult = await resolve(
    { alias: 'is-positive', pref: 'workspace:^3.0.0' },
    {
      projectDir: '/home/istvan/src',
      registry,
      workspacePackages: {
...
test('workspace protocol: resolution fails if there is no matching local package', async () => {
  ...
  expect(err.message).toBe(
    `In ${path.relative(process.cwd(), projectDir)}: "is-positive@workspace:^3.0.0" is in the dependencies but no package named "is-positive" is present in the workspace`
  )
})
...
test('workspace protocol: resolution fails if there is no matching local package version', async () => {
  ...
  expect(err.message).toBe(
    `In ${path.relative(process.cwd(), projectDir)}: No matching version found for is-positive@workspace:^3.0.0 inside the workspace`
  )
})
...
test('throws error when package name has "/" but not starts with @scope', async () => {
  ...
  await expect(
    resolveFromNpm({ alias: 'regenerator/runtime' }, { registry })
  ).rejects.toThrow(
    new PnpmError(
      'INVALID_PACKAGE_NAME',
      'Package name regenerator/runtime is invalid, it should have a @scope'
    )
  )
})

test('resolveFromNpm() should always return the name of the package that is specified in the root of the meta', async () => {
  nock(registry).get('/is-positive').reply(200, isPositiveBrokenMeta)
...
test('request to metadata is retried if the received JSON is broken', async () => {
  ...
  nock(registry).get('/is-positive').reply(200, '{')
...
test('request to a package with unpublished versions', async () => {
  ...
  await expect(
    resolve({ alias: 'code-snippet' }, { registry })
  ).rejects.toThrow(
    new PnpmError(
      'NO_VERSIONS',
      'No versions available for code-snippet because it was unpublished'
    )
  )
})

test('request to a package with no versions', async () => {
  nock(registry).get('/code-snippet').reply(200, { name: 'code-snippet' })
  ...
  await expect(
    ...
    new PnpmError(
      'NO_VERSIONS',
      'No versions available for code-snippet. The package may be unpublished.'
    )
  )
})

test('request to a package with no dist-tags', async () => {
  const isPositiveMeta = omit(
    ['dist-tags'],
    loadJsonFile.sync(f.find('is-positive.json'))
  )
  nock(registry).get('/is-positive').reply(200, isPositiveMeta)
  ...
  await expect(resolve({ alias: 'is-positive' }, { registry })).rejects.toThrow(
    new PnpmError(
      'MALFORMED_METADATA',
      'Received malformed metadata for "is-positive"'
    )
  )
})

test('resolveFromNpm() does not fail if the meta file contains no integrity information', async () => {
  nock(registry).get('/is-positive').reply(200, brokenIntegrity)
...
test('resolveFromNpm() fails if the meta file contains invalid shasum', async () => {
  ...
  await expect(
    ...
    'Tarball "https://registry.npmjs.org/is-positive/-/is-positive-1.0.0.tgz" has invalid shasum specified in its metadata: a'
  )
})

test('resolveFromNpm() should normalize the registry', async () => {
  nock('https://reg.com/owner').get('/is-positive').reply(200, isPositiveMeta)
    ...
    .reply(200, {
      versions: {
        '1.0.0-alpha.1': {
          ...
          dist: {
            tarball:
              'https://registry.npmjs.org/is-positive/-/is-positive-1.0.0-alpha.1.tgz',
          },
        },
      },
      'dist-tags': {
