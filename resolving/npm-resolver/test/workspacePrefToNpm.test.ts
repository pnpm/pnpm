...
describe('workspacePrefToNpm', () => {
  ...
  test('resolve package name aliases', async () => {
    expect(workspacePrefToNpm('workspace:is-positive@3.0.0')).toStrictEqual(
      'npm:is-positive@3.0.0'
    )
    expect(workspacePrefToNpm('workspace:is-positive@*')).toStrictEqual(
      'npm:is-positive@*'
    )
    expect(workspacePrefToNpm('workspace:is-positive@^')).toStrictEqual(
      'npm:is-positive@*'
  ...
  test('resolve scoped package name aliases', async () => {
    ...
    expect(workspacePrefToNpm('workspace:@scope/is-positive@*')).toStrictEqual(
