...
test('request metadata when the one in cache does not have a version satisfying the range', async () => {
  ...
  db.queueSet(`${REG}/bad-dates`, JSON.stringify(cachedMeta), {
...
test('do not pick version that does not satisfy the date requirement even if it is loaded from cache and requested by exact version', async () => {
  ...
  const db = new MetadataCache(cacheDir)
  db.queueSet(`${REG}/foo`, JSON.stringify(fooMeta), {
    cachedAt: new Date('2016-08-17T19:26:00.508Z').getTime(),
    isFull: true,
  })
  db.flush()
...
test('should skip time field validation for excluded packages', async () => {
  ...
  const db = new MetadataCache(cacheDir)
  db.queueSet(`${REG}/is-positive`, JSON.stringify(metaWithoutTime), {
    cachedAt: Date.now(),
    isFull: true,
  })
  db.flush()
...
test('use cached metadata based on cachedAt when publishedBy is set', async () => {
  ...
  const db = new MetadataCache(cacheDir)
  db.queueSet(`${REG}/is-positive`, JSON.stringify(isPositiveAbbreviatedMeta), {
    cachedAt: Date.now(),
  })
  db.flush()
  db.close()
