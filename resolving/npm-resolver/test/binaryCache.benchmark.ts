...
function loadFixture (name: string): PackageMeta {
  ...
  try {
    return JSON.parse(fs.readFileSync(fixturePath, 'utf-8')) as PackageMeta
  } catch {
    ...
    return {
      ...
      versions: {
        '1.0.0': { name: 'is-positive', version: '1.0.0' } as any,
        '2.0.0': { name: 'is-positive', version: '2.0.0' } as any,
        '3.0.0': { name: 'is-positive', version: '3.0.0' } as any,
        '3.1.0': { name: 'is-positive', version: '3.1.0' } as any,
      },
    } as PackageMeta
  }
}
...
function createLargePackageMeta (versionCount: number): PackageMeta {
