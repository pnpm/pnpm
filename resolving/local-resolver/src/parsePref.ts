...
import type { LocalPackageSpec, WantedLocalDependency } from '@pnpm/types'
...
function fromLocal(
  ...
  if (/^~[/]/.test(spec)) {
    ...
    normalizedPref = isAbsolute(spec) ? `${protocol}${spec}` : `${protocol}${path.relative(projectDir, fetchSpec)}`;
  }
