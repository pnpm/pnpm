...
skipOnWindowsCI(
  'pack packages with workspace LICENSE if no own LICENSE is present',
  async () => {
    preparePackages(
      [
        {
    ...
    crossSpawn.sync(pnpmBin, [
      'add',
      '../project-1/project-1-1.0.0.tgz',
      '../project-2/project-2-1.0.0.tgz',
    ])

    expect(await exists('node_modules/project-1/LICENSE')).toBeTruthy()
    expect(await fs.readFile('node_modules/project-1/LICENSE', 'utf8')).toBe(
      'workspace license'
    )
    expect(await exists('node_modules/project-2/LICENSE')).toBeTruthy()
    expect(await fs.readFile('node_modules/project-2/LICENSE', 'utf8')).toBe(
      'project-2 license'
    )
...
test('publish packages with workspace LICENSE if no own LICENSE is present', async () => {
  ...
  expect(await fs.readFile('node_modules/project-100/LICENSE', 'utf8')).toBe(
    'workspace license'
  )
  expect(await fs.readFile('node_modules/project-200/LICENSE', 'utf8')).toBe(
    'project-200 license'
  )
...
test('publish: package with all possible fields in publishConfig', async () => {
  ...
  const { default: originalManifests } = await import(
  ...
  const { default: publishedManifest } = await import(
    path.resolve('node_modules/test-publish-config/package.json')
  )
...
test('publish: package with publishConfig.directory', async () => {
  const packages = preparePackages([
    {
      ...
      scripts: {
        prepublishOnly:
          "node --eval=\"const fs=require('fs');fs.mkdirSync('dist',{recursive:true});fs.writeFileSync('dist/prepublishOnly', '', 'utf8')\"",
      },
  ...
  expect(
    JSON.parse(
      await fs.readFile(
        'node_modules/publish_config_directory_dist_package/package.json',
        { encoding: 'utf-8' }
      )
    )
  ).toStrictEqual({
  ...
  expect(
    existsSync(
      'node_modules/publish_config_directory_dist_package/prepublishOnly'
    )
  ).toBeTruthy()
})
...
test.skip('convert specs with workspace protocols to regular version ranges', async () => {
  ...
  await expect(
    ...
    .rejects // It would be great to match the exact error message
  ...
  crossSpawn.sync(pnpmBin, [
    'add',
    '--store-dir=store',
    'workspace-protocol-package',
    '--no-link-workspace-packages',
    `--registry=http://localhost:${REGISTRY_MOCK_PORT}`,
  ])

  const { default: publishedManifest } = await import(
    path.resolve('node_modules/workspace-protocol-package/package.json')
  )
...
test.skip('convert specs with relative workspace protocols to regular version ranges', async () => {
  ...
  const { default: publishedManifest } = await import(
    path.resolve(
      'node_modules/relative-workspace-protocol-package/package.json'
    )
  )
  expect(publishedManifest.dependencies).toStrictEqual({
    'file-type': '12.0.1',
  ...
  await publish.handler(
    {
      ...DEFAULT_OPTS,
      argv: {
        original: ['publish', '--publish-branch', branch, ...CREDENTIALS],
      },
      dir: process.cwd(),
...
test('publish: use basic token helper for authentication', async () => {
  ...
  const file = os === 'win32' ? 'tokenHelperBasic.bat' : 'tokenHelperBasic.js'
...
test('publish: use bearer token helper for authentication', async () => {
  ...
  const os = process.platform
  const file = os === 'win32' ? 'tokenHelperBearer.bat' : 'tokenHelperBearer.js'
  const tokenHelper = path.join(__dirname, 'utils', file)
