...
async function execPnpmInstall(opts) {
    await execa('node', [
        pnpmBin,
        'install',
...
async function approveSomeBuilds(opts) {
...
async function approveNoBuilds(opts) {
    await execPnpmInstall();
    const config = await getApproveBuildsConfig();
    prompt.mockResolvedValueOnce({
        result: [],
...
test('approve selected build', async () => {
    ...
    const workspaceManifest = readYamlFileSync(path.resolve('pnpm-workspace.yaml')); // eslint-disable-line
...
test('approve no builds', async () => {
    ...
    const manifest = readYamlFileSync(path.resolve('pnpm-workspace.yaml')); // eslint-disable-line
...
test('positional arguments with unknown package throws error', async () => {
    ...
    await execPnpmInstall();
    const config = await getApproveBuildsConfig();
    await expect(approveBuilds.handler(config, ['@pnpm.e2e/nonexistent-package'])).rejects.toThrow('not awaiting approval');
});
...
test('contradictory arguments throw error', async () => {
    ...
    await execPnpmInstall();
    const config = await getApproveBuildsConfig();
    await expect(approveBuilds.handler(config, [
        '@pnpm.e2e/pre-and-postinstall-scripts-example',
        '!@pnpm.e2e/pre-and-postinstall-scripts-example',
...
test('--all with positional arguments throws error', async () => {
    ...
    await execPnpmInstall();
    const config = await getApproveBuildsConfig();
    await expect(approveBuilds.handler({ ...config, all: true }, ['@pnpm.e2e/pre-and-postinstall-scripts-example'])).rejects.toThrow('Cannot use --all with positional arguments');
});
test('positional args preserve existing allowBuilds entries', async () => {
    ...
    const manifest = readYamlFileSync(workspaceManifestFile); // eslint-disable-line
...
//# sourceMappingURL=approveBuilds.test.js.map