...
3. Resolve any conflicts with the base branch by running './shell/resolve-pr-conflicts.sh $pr_number'. This force-fetches the base branch (avoiding stale refs), rebases, auto-resolves lockfile conflicts, and force-pushes. If it reports manual resolution needed, fix those files and re-run it. Do NOT skip this step. Do NOT assume the branch is up to date.
